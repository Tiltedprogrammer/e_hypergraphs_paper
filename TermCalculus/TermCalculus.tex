...
\begin{abstract}
Just as the simply typed lambda calculus is the internal language of cartesian
closed categories, many categorical structures admit a type theory whose
derivations are precisely their morphisms. In this work we give such a type
theory for \emph{semilattice-enriched multicategories}. We define a type theory
for (representable) $\catname{SLat}$-multicategories and show that it enjoys the
standard desirable properties: terms uniquely determine their derivations, and
substitution is admissible and satisfies associativity and interchange. We prove
that this type theory correctly presents the free (representable)
$\catname{SLat}$-multicategory over a multigraph.

Our motivating instance is the \emph{e-graphs}, the data structures powering
...
render e-graphs as terms in our type theory, under which the semilattice join
corresponds directly to e-class membership and equality saturation becomes a
term rewriting problem modulo the structural equations of the theory. In
particular, we show how the explicit \textbf{let} and recursive \textbf{letrec}
constructs capture, respectively, the sharing structure of e-graphs and cyclic
e-graphs modelled by traced cartesian $\catname{SLat}$-categories. This may lay
the foundations for programming languages with equivalences built in.
\end{abstract}
\maketitle
...
The key syntactic insight is that the semilattice join $+$ of the type theory corresponds directly to e-class membership: a term $M + N : A$ represents an equivalence class containing both $M$ and $N$ as terms of type $A$.
The explicit \textbf{let} construct then captures the node-sharing structure of e-graphs, and the recursive \textbf{letrec} construct captures cyclic e-graphs.
A further advantage over practical implementations such as \texttt{egg}~\cite{EggPaper}, where terms are essentially closed, is that the type theory treats open and closed terms uniformly: an e-graph with free variables is simply a term with a non-empty context $\Gamma$, requiring no special treatment.
...
We develop the type theory in the \emph{linear} (multicategorical) setting first, as it is the most presentationally clean; Cartesian structure --- required for conventional e-graphs, where variables may be used multiple times --- is then straightforwardly added following the general recipe of~\citet{shulman2016categorical}.
...
\begin{itemize}
  ...
  categorically by traced cartesian $\catname{SLat}$-categories
