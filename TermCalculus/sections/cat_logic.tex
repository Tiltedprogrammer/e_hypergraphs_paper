...
\[
\inferrule*[]{\Gamma, x_1 : A_1, \ldots, x_n : A_n \vdash M : A \quad \Gamma, x_1 : A_1, \ldots, x_n : A_n \vdash N : B}
{\Gamma \vdash \textbf{letrec } (x_1, \ldots, x_n) = M \textbf{ in } N : B},
\]
...
\begin{gather}\label{eq:let_equations}
  \textbf{letrec } x = M \textbf{ in } N \equiv \textbf{letrec } x = M \textbf{ in } N[M / x]\\
  \textbf{letrec } y = (\textbf{letrec } x = L \textbf{ in } M) \textbf{ in } N \equiv \textbf{letrec } x = L \textbf{ in } (\textbf{letrec } y = M \textbf{ in } N)\\
  \textbf{letrec } D_1 \textbf{ in } \textbf{letrec } D_2 \textbf{ in } M \equiv \textbf{letrec} D_1, D_2 \textbf{ in } M\\
  \textbf{letrec } D_1, D_2, D_3 \textbf{ in } M \equiv \textbf{letrec } D_2, D_1, D_3 \textbf{ in } M\\
  \textbf{letrec } x = M \textbf{ in } F(..., N, ...) \equiv F(..., \textbf{letrec } x = M \textbf{ in } N, ...)\\ 
  \textbf{letrec } x = M \textbf{ in } N \equiv N,\;x \not \in FV(N).
\end{gather}

We then define trace as
\[
Tr_{\Gamma,B}^{X}(\llbracket \Gamma, x : X \vdash M : B \otimes X \rrbracket) = \llbracket \Gamma \vdash \textbf{match } M \{uv . \textbf{letrec } x = v \text{ in } M \} \rrbracket
\]
...
The construction
\[
\text{Tr}_{\Gamma,B}^{X}(\llbracket \Gamma, x : X \vdash M : B \otimes X \rrbracket) = \llbracket \Gamma \vdash \textbf{letrec } x = \textbf{ match}(x, uv.v) \textbf{ in } \textbf{match}(M, uv.u)\rrbracket
\]
...
And therefore,
\begin{align*}
\textbf{match}(f(h(a),x), uv.\{g(u),v\}) &\equiv \{g(\textbf{match}(u', \alpha\beta.\alpha)), \textbf{match}(u', \alpha\beta.\beta)\}[f(h(a),x) / u']\\
