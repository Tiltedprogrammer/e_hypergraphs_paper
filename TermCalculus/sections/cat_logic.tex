...
\[
\inferrule*[]{\Gamma, x : A \vdash M : A \quad \Gamma, x : A \vdash N : B}
{\Gamma \vdash \textbf{letrec } x = M \textbf{ in } N : B},
\]

note that $M$ and $N$ use the same context $\Gamma$.

We then define the substitution on a let as
\[
(\textbf{letrec } x = M \textbf{ in } N) [P/y] = \textbf{ letrec } x = M[P/y] \textbf{ in } N[P/y].
\]
and use a syntactic sugar for multi-variable let as follows:
\[
\textbf{letrec } (x,y) = M \textbf{ in } N \coloneqq \textbf{letrec } z = \textbf{match}(z, xy.M) \textbf{ in } \textbf{match}(z, xy.N).
\]
Then we require the following equations to hold:
\begin{gather}
  \textbf{letrec } x = M \textbf{ in } N \equiv \textbf{letrec } x = M \textbf{ in } N[M / x]\label{eq:let_equations:subst}\\
\textbf{letrec } x = N(M(x)) \textbf{ in } M(x) \equiv \textbf{letrec } y = M(N(y)) \textbf{ in } y\label{eq:let_equations:dinaturality}\\
\textbf{letrec } x = (letrec y = Q \textbf{ in } P) \textbf{ in } N \equiv \textbf{letrec } (x,y) = \{P,Q\} \textbf{ in } N\label{eq:let_equations:merge}\\
...
\textbf{letrec } z = \textbf{match}(z, xy. \{w, Q(w, y)\}) \textbf{ in } M[w/x]  \equiv  \textbf{letrec } y = Q(w, y) \textbf{ in } M \qquad w \not \in \{x,y,z\}\label{eq:let_equations:qvar}
\end{gather}
...
\begin{align*}
  ...
  &\text{by \autoref{eq:let_equations:congruence}}\\
  &\equiv \textbf{match}(f(a,\textbf{letrec } x = g(\textbf{match}(f(a,x), \alpha\beta.\beta)) \textbf{ in } x), \alpha\beta.\alpha) : B\\
  &\text{by \autoref{eq:let_equations:subst}}\\
  &\equiv \textbf{match}(f(a,\textbf{letrec } x = g(\textbf{match}(f(a,x), \alpha\beta.\beta)) \textbf{ in } g(\textbf{match}(f(a,x), \alpha\beta.\beta))), \alpha\beta.\alpha) : B\\
  &\text{by \autoref{eq:let_equations:congruence}}\\
  &\equiv \textbf{match}(f(a, g(\textbf{letrec } x = g(\textbf{match}(f(a,x), \alpha\beta.\beta)) \textbf{ in } \textbf{match}(f(a,x), \alpha\beta.\beta))), \alpha\beta.\alpha) : B\\
  &\text{by \autoref{eq:let_equations:dinaturality}}\\
  &\equiv \textbf{match}(f(a,g(\textbf{letrec } y = \textbf{match}(f(a,g(y)), \alpha\beta.\beta) \textbf{ in } \textbf{y}), \alpha\beta.\alpha) : B\\
  &\text{by \autoref{eq:let_equations:congruence}}\\
...
Let $A_x = \textbf{match}(A,\gamma\delta.\delta)$ and $A_B = \textbf{match}(A, \gamma\delta.\gamma) : B$.
