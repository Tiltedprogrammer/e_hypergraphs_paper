...
\begin{gather*}
  \textbf{letrec } x = M \textbf{ in } N \equiv N[M/x], x \not \in FV(M)\\
...
\[
Tr_{\Gamma,B}^{X}(\llbracket \Gamma, x : X \vdash M : B \otimes X \rrbracket) = \llbracket \Gamma \vdash \textbf{match} M \{uv . \textbf{letrec} \} \rrbracket
\]
...
\[
\text{Tr}^{X}((g \otimes \id_{X}) \circ f \circ (h \otimes \id_{X})) = g \circ Tr^{X}(f) \circ h
\]
...
\[
\text{Tr}^{\I}(f) = f, \forall f : A \to B
\]
and
\[
\text{Tr}^{X \otimes Y}(f) = \text{Tr}^{X}(\text{Tr}^{Y}(f)), \forall f : A \otimes X \otimes Y \to B \otimes X \otimes Y
\]
...
\]
\paragraph{Yanking} For all $A,X \in \mathcal{C}$
\[
...
\paragraph{Vanishing II} For all $A,B,X,Y \in \mathcal{C}$ and $f : (A,X,Y;B \otimes X \otimes Y)$
\[
\text{Tr}^{X \otimes Y}(f') = \text{Tr}^{X}(\text{Tr}^{Y}(f)),
\]
where $f' : (A, X \otimes Y; B \otimes X \otimes Y)$ is the unique morphism such that $f' \circ (id_{A}, \xi_{X,Y}) = f$.

\paragraph{Strength} For all $A,B,C,D,X \in \mathcal{C}$, $f : (C,X;D \otimes X)$, $g : (A;B)$
\[
\text{Tr}^{X}(\xi_{B, D \otimes X} \circ (g,f)) = \xi_{B,D} \circ (g, \text{Tr}^{X}(f))
\]
