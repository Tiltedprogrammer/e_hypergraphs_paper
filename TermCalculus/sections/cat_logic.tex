...
\label{def:substitution}
  ...
  \begin{enumerate}
    ...
    \begin{gather*}
     ...
     = \sum_{k_1=1}^{i_1}\ldots\sum_{k_n=1}^{i_n}f(N_{1k_1}, \ldots, N_{j,k_j}[M_1 + \ldots + M_m / x], \ldots, N_{nk_n})\\
     = \sum_{k_1=1}^{i_1}\ldots\sum_{k_n=1}^{i_n}f(N_{1k_1}, \ldots, N_{nk_n})[M_1 + \ldots + M_m / x]
    \end{gather*}
  \item Suppose $N$ is the conclusion of the rule
  \[
...
\begin{example}
  ...
  \[
  \vdash (a * 2) / 2 \equiv ((a * 2) + (a * 2)) / 2 : \mathbb{N},
  \]
  then, using
  \[
  \vdash (a * 2) \equiv a <\!\!< 1 : \mathbb{N},
  \]
  and the substitution congruence, we have that
  \[
...
\label{ex:recursive_let}
  ...
  The $+$-introduction is induced by the equation $x * 2 \equiv x <\!\!< 1$, so its premises must be the literal-$2$ terms $x * 2$ and $x <\!\!< 1$.
  We first derive the term with $2$ as a literal constant, then use the $\beta$-let equation in reverse to abstract the two occurrences of $2$ into a single \textbf{let}-binding, making the sharing of the constant explicit in the syntax.
  ...
  \inferrule*[right=intro $+$, left=$\pi_E$]{\pitri{\pi_{x * 2}} \quad \pitri{\pi_{x <\!\!< 1}}}
  {x : \mathbb{N} \vdash (x * 2) + (x <\!\!< 1) : \mathbb{N}}\\
  \inferrule*[right=intro $/$, left=$\pi_{E/2}$]{
    \pitri{\pi_{E}} \quad
    x : \mathbb{N} \vdash 2 : \mathbb{N} \quad
    (/) \in \mathcal{G}(\mathbb{N}, \mathbb{N}; \mathbb{N})}
  {x : \mathbb{N} \vdash ((x * 2) + (x <\!\!< 1)) / 2 : \mathbb{N}}
\end{gather*}
By the $\beta$-let equation $\textbf{let } z = 2 \textbf{ in } N \equiv N[2/z]$ applied in reverse, the derived term is equivalent to a form in which the two occurrences of $2$ are shared by a single binder:
\[
x : \mathbb{N} \vdash ((x * 2) + (x <\!\!< 1)) / 2 \;\equiv\; \textbf{let } z = 2 \textbf{ in } ((x * z) + (x <\!\!< 1)) / z : \mathbb{N}.
\]
The right-hand side may equivalently be derived using the typing rule for \textbf{let} with body $x : \mathbb{N}, z : \mathbb{N} \vdash ((x * z) + (x <\!\!< 1)) / z : \mathbb{N}$.
Wrapping the outer binding for $a$ gives the final term:
\[
\inferrule*[left=$\pi_{x = a}$]{\vdash a : \mathbb{N} \quad x : \mathbb{N} \vdash \textbf{let } z = 2 \textbf{ in } ((x * z) + (x <\!\!< 1)) / z : \mathbb{N}}
{\vdash \textbf{let } x = a \textbf{ in } (\textbf{let } z = 2 \textbf{ in } ((x * z) + (x <\!\!< 1)) / z) : \mathbb{N}}
\]
Substituting $x \mapsto a$ and $z \mapsto 2$ yields $((a * 2) + (a <\!\!< 1)) / 2$, which matches the term derived in example~(a) after applying the equation $x * 2 \equiv x <\!\!< 1$.
...
We then define the substitution on a let as
\[
(\textbf{letrec } D \textbf{ in } N) [P/y] = \textbf{ letrec } D[P/y] \textbf{ in } N[P/y].
\]
...
E-graphs are similar to sharing graphs (with the addition of equivalence classes) and the latter were given models in traced monoidal categories by~\citet{hasegawa_models_1999}.
We recall a notion of a traced (symmetric) monoidal category, according to~\citet{Joyal_Street_Verity_1996}, next.
...
\begin{proposition}
Traced (symmetric) representable multicategories are equivalent to traced (symmetric) monoidal categories.
The monoidal structure is not affected by the trace and therefore the monoidal part of the equavalence is given by the same argument as in~\autoref{prop:representable_multicategories}.
...
Note that we denote the monoidal tensor as $\text{tr}$.
\end{proposition}
\begin{proof}
  ...
  \begin{itemize}
   ...
   \alpha_{A,C,X} \circ \xi_{A \otimes C, X} \circ (\xi_{A,C}, \id_{X}) = \xi_{A, C \otimes X} \circ (\id_{A}, \xi_{C,X}),
    ...
    \end{align*}
  \end{itemize}
  By definition, we also have that $g \otimes \text{tr}^{X}(f)$ is the unqiue morphism such that
  \[
...
\[
g(M) \equiv \textbf{letrec } x = M \textbf{ in } g(x), y \not \in FV(M,g)
\]
\end{lemma}
\begin{proof}
\begin{align*}
...
Using the $\eta$-eqution we can derive the following
\[
...
Trace, however, is an iterative operation, and additivity in its argument --- $\text{Tr}^X(f \vee g) = \text{Tr}^X(f) \vee \text{Tr}^X(g)$ --- is not implied by $\catname{SLat}$-enrichment.

The canonical $\catname{SLat}$-enriched traced monoidal category $\catname{Rel}$ of sets and relations (with disjoint union as tensor) witnesses this failure.
Take $X = \{x_0, x_1\}$ and consider the relations $R_1, R_2 : X \to X$ given by $R_1 = \{(x_0, x_1)\}$ and $R_2 = \{(x_1, x_0)\}$.
Neither $R_1$ nor $R_2$ admits a fixed point in $X$, so $\text{Tr}^X(R_1) = \text{Tr}^X(R_2) = \varnothing$, and hence $\text{Tr}^X(R_1) \cup \text{Tr}^X(R_2) = \varnothing$.
However, $R_1 \cup R_2$ contains the 2-cycle $x_0 \mapsto x_1 \mapsto x_0$, so $\text{Tr}^X(R_1 \cup R_2)$ is non-empty.
Operationally, a trace path through $R_1 \cup R_2$ may interleave steps from $R_1$ and $R_2$, producing fixed-point trajectories that neither summand admits in isolation.
Composition cannot exhibit this behaviour because it is a single-step operation, but iteration can, and so additivity of trace is a strictly stronger requirement than $\catname{SLat}$-enrichment of composition.
...
As before, we keep $2$ and $1$ as literal constants in the derivation so that the $+$-introductions are justified directly by the equation $x * 2 \equiv x <\!\!< 1$, and we apply the $\beta$-let equation in reverse at the end to share the four occurrences of $2$ and the two occurrences of $1$.
\begin{gather*}
...
}{x:\mathbb{N} \vdash x*2 + x\mathbin{<\!\!<}1 : \mathbb{N}}\\
%
\inferrule*[left=$\pi_{E/2}$]{
  \pitri{\pi_E} \quad
  x:\mathbb{N} \vdash 2 : \mathbb{N} \quad
  {/} \in \mathcal{G}(\mathbb{N},\mathbb{N};\mathbb{N})
}{x:\mathbb{N} \vdash (x*2+x\mathbin{<\!\!<}1)/2 : \mathbb{N}}\\
%
\inferrule*[left=$\pi_W$]{
  \inferrule*[]{
    x:\mathbb{N} \vdash 2:\mathbb{N} \quad
...
}{x:\mathbb{N} \vdash 2/2+1 : \mathbb{N}}\\
%
\inferrule*[left=$\pi_{x \cdot W}$]{
  x:\mathbb{N} \vdash x:\mathbb{N} \quad
  \pitri{\pi_W} \quad
  {*} \in \mathcal{G}(\mathbb{N},\mathbb{N};\mathbb{N})
}{x:\mathbb{N} \vdash x*(2/2+1) : \mathbb{N}}\\
%
\inferrule*[left=$\pi_Z$]{
  \inferrule*[]{
    \pitri{\pi_{E/2}} \qquad \pitri{\pi_{x \cdot W}}
  }{x:\mathbb{N} \vdash (x*2+x\mathbin{<\!\!<}1)/2 + x*(2/2+1) : \mathbb{N}} \quad
  x:\mathbb{N} \vdash a:\mathbb{N}
}{x:\mathbb{N} \vdash (x*2+x\mathbin{<\!\!<}1)/2 + x*(2/2+1) + a : \mathbb{N}}\\
%
\inferrule*[left=$\pi_x$]{
  \pitri{\pi_Z} \qquad x:\mathbb{N} \vdash x : \mathbb{N}
}{\vdash \textbf{letrec}\ x\ =\ (x*2+x\mathbin{<\!\!<}1)/2 + x*(2/2+1) + a\ \textbf{in}\ x : \mathbb{N}}
\end{gather*}
The four occurrences of $2$ in the body --- one in $x*2$ (the LHS of the equation), one in the outer division $/\,2$, and two in $2/2$ --- all refer to the same constant morphism $\vdash 2 : \mathbb{N}$ in the multicategorical interpretation; likewise, the two occurrences of $1$ --- one in $x \mathbin{<\!\!<} 1$ and one in $2/2 + 1$ --- both refer to $\vdash 1 : \mathbb{N}$.
Applying the $\beta$-let equation $\textbf{let}\ v = M\ \textbf{in}\ N \equiv N[M/v]$ in reverse twice, the derived term is equivalent to a form in which this sharing is made explicit by single binders for $2$ and $1$:
\[
\vdash \textbf{letrec}\ x\ =\ (x*2+x\mathbin{<\!\!<}1)/2 + x*(2/2+1) + a\ \textbf{in}\ x \;\equiv\; \textbf{let}\ z = 2\ \textbf{in}\ \textbf{let}\ w = 1\ \textbf{in}\ \textbf{letrec}\ x = (x*z+x\mathbin{<\!\!<}w)/z + x*(z/z+w) + a\ \textbf{in}\ x : \mathbb{N}.
\]
\end{example}