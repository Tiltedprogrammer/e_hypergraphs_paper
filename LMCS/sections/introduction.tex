...
E-graphs were first introduced in the automated theorem proving literature in the 1980s~\cite{nelson1980techniques}, but recent years have seen renewed interest, particularly for optimisation and synthesis using the technique of \emph{equality saturation}~\cite{EqualitySaturation2009,EggPaper,flatt_small_2022}, wherein rewrites to an e-graph are repeated until a fixpoint (or timeout) is reached.
...
In the textual representation of $\lambda$-terms, $\alpha$-equivalent terms may be distinct and thus cannot be shared, which adds a layer of bookkeeping and complexity.
...
The two different occurrences of the same bound variable $f$ are represented differently by $\%0$ and $\%1$, and cannot be shared in an e-graph.
...
Terms are identified up to permutations of non-interacting substitutions, which is called \enquote{graph equivalence}~\cite{accattoli2014nonstandard} since such identification becomes obvious in certain diagrammatic representations.
...
String diagrams are a direct bridge from the language of category theory, which in turn can be used to give semantic models to a variety of programming languages and logics, to the concrete language of graphs, which are a fundamental data structure in compiler design.
...
The particular context of string diagrams for $\lambda$-calculi is given in~\citet{ghica2024stringdiagramslambdacalculifunctional} (see~\autoref{fig:de-brujin-string}), while string diagrams for e-graphs without bindings are defined in~\citet{tiurin2025equivalencehypergraphsdporewriting}.
\begin{figure}
	\centering
	\adjustbox{scale=0.6}{
...
For instance, hypergraph isomorphism rigorously expresses what it means for string diagrams to \enquote{look the same} --- topologically, that one may be deformed continuously into the other without obstruction\footnote{This is the informal notion of the \enquote{equality of string diagrams}.}; secondly, string-diagrammatic equational reasoning can be formalised as (categorical) graph rewriting over these hypergraphs using the technique of double-pushout rewriting (DPO)~\cite{bonchi_string_2022-1}.
...
These boxes require extra structure in their hypergraph representation in the form of a layering of graphs (\emph{hierarchical hypergraphs})~\cite{fscd}.
...
The scope of a $\lambda$-abstraction is represented by a rounded box with the bound variable represented as a wire attached to the frame, the application node by a half-circle
...
and sharing by a \emph{copy node} \comonoid.

\subsection{E-graphs}

E-graphs can efficiently simultaneously represent several equivalent terms of an algebraic theory, generalising the \emph{term graph} representation of terms to include equivalence classes of subterms (or subgraphs).
...
\begin{figure}
	...
	\end{center}
	\caption{E-graph rewriting example (top) and its equivalent string diagram representation (bottom)~\cite{tiurin2025equivalencehypergraphsdporewriting}}
	\label{fig:e-graph-example}
\end{figure}

Each column represents a term (or term rewrite rule), its conventional e-graph, and the equivalent string diagram (for Cartesian symmetric monoidal categories) representation using the \emph{e-hypergraphs} used by~\citet{tiurin2025equivalencehypergraphsdporewriting}.
...
By making a sequence of choices for e-nodes in e-classes, we can extract terms from the graph; a different sequence of choices results in an equivalent term.
Term graphs embed in e-graphs, where each e-class has a single e-node.

In~\autoref{fig:e-graph-example}, for the first rewrite rule, replacing multiplication by \emph{shift-left} creates the first non-singleton e-class, which includes the multiplication ($*$) and shift-left ($<\!\!<$) nodes, now sharing the e-class containing the node $a$.
...
In the latter, edges connect directly to e-nodes inside the e-class, whereas in the former edges connect to the equivalence class itself.
...
Let us compare our proposal with how substitution works in conventional e-graphs, which lack specialised support for binding.
...
We will see how in our categorical representation these nodes are not necessary, because explicit substitution is simply modelled categorically as composition.
...
The same example is then given again with string diagrams in~\autoref{fig:e-string-substitution}.
...
The explicit occurrences of variable names in the conventional approach raise obvious questions of scope, binding and explicit substitution, whereas the use of terms inside the explicit substitution nodes also raises many technical difficulties which adds significant complexity to proving various mathematical properties.
...
Recent work on \emph{slotted} e-graphs~\cite{slotted-egraphs} addresses the sharing issues when representing terms that are equivalent up to renaming of variables (even free ones) by incorporating variables as first-class data of an e-graph and making binding a built-in operation.
...
For example, in~\autoref{fig:slotted}, an e-class $e_1$ containing $+$ that is parameterised by variable names $s_2$ and $s_3$ gets instantiated once with $a,b$ (via $s_4 \mapsto a, s_2 \mapsto s_4$ and $s_5 \mapsto b, s_3 \mapsto s_5$) and once with $x,c$.
...
A slotted e-graph version of~\autoref{fig:e-graph-substitution} would be able to introduce sharing for the variable names $x$ and $y$ but must still represent abstraction, application, and substitution explicitly, and so would look much the same\footnote{The framework of slotted e-graphs also provides a built-in substitution operation, however the explicit substitution remains the preferred way of dealing with variables and binding due to its flexibility.}.
...
We defer a case study to~\autoref{sec:application}, where we argue that our approach has advantages if one is particularly interested in e-graphs for (variations of) $\lambda$-calculus, and, by extension, programming languages.
...
Finally, the development of e-graph applications and algorithms outpaces the development of its semantics with only two works dedicated to the former~\cite{zhang_relational_2022,tree_automata_e_graphs}.
We hope that our work might serve as a foundation for semantics of e-graphs with bindings.
...
Then we define what closed e-hypergraphs are and provide the interpretation of closed enriched $\Sigma$-terms as cospans of e-hypergraphs (and by doing this we also formalise the corresponding string diagrams as concrete combinatorial objects).
...
We conclude the paper with a description of what benefits the e-hypergraph representation may bring for equality saturation in the context of $\lambda$-calculus with explicit substitution compared to conventional (slotted) e-graphs.
