\section{\texorpdfstring{$\catname{SLat}$}{SLat}}
...
\begin{defi}[Semilattice homomorphism]
...
\begin{defi}[Category of semilattices]
...
\begin{prop}[Special case of Proposition 6.4.6~\cite{Borceux_1994}]
...
\begin{defi}[Category of MDA Hypergraphs with Interfaces~\cite{bonchi_string_2022-2}]%
...
\begin{defi}[Boundary complement]%
...
\begin{exa}[Example 29~\cite{bonchi_string_2022-2}]
	Consider the following signature $\Sigma = \{\alpha_1 : 0 \to 1, \alpha_2 : 1 \to 0, \alpha_3 : 1 \to 1\}$ and the rewrite rule $id \to \alpha_3$ for the example term $\alpha_1;\alpha_2$.
...
\begin{defi}[Convex DPOI rewriting]%
...
\begin{exa}[Example 32~\cite{bonchi_string_2022-2}]
	Consider the following signature $\Sigma \{e_1 : 1 \to 2, e_2 : 2 \to 1, e_3 : 1 \to 1, e_4 : 1 \to 1\}$ and the following rewrite rule
	\[
		id \otimes e_{1};\sym \otimes id; id \otimes e_{2} \quad \to \quad \sym;e_4 \otimes e_4
	\]
	...
	\]
	Now consider the cospan of hypergraphs corresponding to the following $\Sigma$-term $e_1;e_3\otimes id;e_2$.
	\[
		\adjustbox{scale=0.65}{
	...
	That is, there is no decomposition of $e_1;e_3\otimes id;e_2$ as $c_1;id_{n}\otimes(id \otimes e_1;\sym\otimes id;id \otimes e_2);c_2$.
	There is, however, a match modulo traced symmetric monoidal structure --- $\text{tr}(e_3\otimes id;\textcolor{red}{e_1 \otimes id;\sym \otimes id; id \otimes e_2})$.
	Requiring convex matches prevents such situations from arising.
\end{exa}

\begin{figure}
	\[
		\adjustbox{scale=0.7, width=\linewidth}{
...
\section{Pushout in \texorpdfstring{$\catname{EHyp}(\Sigma)$}{EHyp(Σ)}}
...
\begin{thm}[Existence of pushouts in $\catname{EHyp}(\Sigma)$]
...
\section{Interpretation of \texorpdfstring{$\Sigma$}{Σ}-terms as cospans of hypergraphs}
...
For example, the EDPOI rule corresponding to the distributivity of $(;)$ over $+$ is given by the following rewrite rule.
\[
...
Consider the example of EDPOI rewriting in~\autoref{fig:dpoi-example} as defined in~\autoref{def:dpoi-e}.
...
\section{Modelling \texorpdfstring{$\lambda_{\text{lsub}}$-}{linear substitution }calculus in slotted e-graphs}%
...
These can be directly encoded in the framework of slotted e-graphs of ~\citet{slotted-egraphs} as can be seen in \autoref{listing:define_language}, \autoref{listing:lsub_beta_rule}, \autoref{listing:lsub_ls_rules}, \autoref{listing:gc_rule}, and \autoref{listing:graphical_equivalence}.
...
\begin{rem}
	...
	The deletion morphisms for $\Gamma_1, \Gamma_2$ that follow the copy can be absorbed by the e-hypergraph representation by following the approach of~\citet{zanassi_comonoid}.
\end{rem}
