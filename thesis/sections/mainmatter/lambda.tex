...
It is well-known that different variations of $\lambda$-calculi are modeled by different flavours of closed monoidal categories.
The morphisms in the latter can be represented by string diagrams, a particular kind as used by Ghica and Zanasi~\cite{ghica2024stringdiagramslambdacalculifunctional}.
We recapitulate the key properties of such string diagrams with respect to variables and binding, develop a combinatorial representation of the version of these diagrams extended with $\catname{SLat}$-structure, prove the equivalence of rewriting of closed terms and these combinatorial objects.
We then argue that this representation may be efficient for performing equality saturation on the $\lambda$-calculus with explicit substitution terms by showing that particular equalities of such calculus are absorbed by our representation.

Different flavours of monoidal closed categories are well-known models of different variations of lambda-calculi.
\begin{definition}%
	\label{def:closed}
	...
	\begin{equation}
		f = \Lambda_{A,X,B}(f) \otimes \id_{X} \fatsemi \ev_{X,A},
  \label{eq:lambda_1}
  \tag{1}
  \end{equation}
  \begin{equation}
...
If the tensor is symmetric, then the monoidal category is \emph{closed}.
Note the assymetry, $\ev_{X,A} : (X \multimap A) \otimes X \to A$ is the counit of the adjunction, whereas $\Lambda_{A,X,B}$ is the \emph{currying} isomorphism of the adjunction given by $\Lambda_{A,X,B}(f) = \eta_{A} \fatsemi (X \multimap f)$.
...
String diagrams for closed monoidal categories are given by the combined syntax in~\cref{fig:lambda-string-combined}.
...
\end{subfigure}
\caption{String diagrams for closed monoidal categories.}
\label{fig:lambda-string-combined}
\end{figure}

The equations \eqref{eq:lambda_1}--\eqref{eq:lambda_3} are repesented as the equations between string diagrams in~\cref{fig:lambda-string-eq},
\begin{figure}
  \[
\adjustbox{max width=\textwidth}{
...
If one wish to further model $\lambda$-calculi where the context admits contraction and weakening, the string diagrams are extended with natural copy $\comonoid$ and $\counit$ delete generators.

The first representational advantage of string diagrams is that they are a \emph{nameless} represenattion, that is, $\alpha$-equivalent terms are represented by the same string diagram.
...
\]
It is worth noticing, that the same class of terms may be represented by a single $\lambda$-term using de Bruijn indices, namely the term $\lambda . \%0((\lambda . \%1 \%0) 2)$, however the different occurences of the binder corresponding to $f$ use different indices, which might not be desirable.
...
Below we consider the \emph{linear substitution calculus} $\lambda_{\mathrm{sub}}$ of Accattoli et al.~\cite[Sec.~2]{accattoli2014nonstandard}.
...
In slotted-egraphs, each e-class is parameterised by a set of free variables it uses and each e-node may invoke different instances of such an e-class by supplying concrete varible names for slots.
...
E-nodes are matched to their corresponding e-classes by computing a normal form with respect to the free varibles, by canonically renaming them.
...
Yet, the practically preffered way of performing substitution is still to use explicit substitution.

We arggue that string diagrams for $\catname{SLat}$-enriched closed symmetric monoidal categories, provide a more principled support for varible binding and substitution.
...
\begin{figure}
	...
	\caption{String diagrams for closed symmetric monoidal $\catname{SLat}$-categories; dashes stand for arbitrary number of wires and appropriate labels.}
	\label{fig:egraph-strings-closed}
\end{figure}
String diagrams keep varibles and e-classes (dashed boxes) as separate entities, variables are just wires connecting the diagram.
Substitution then does not require any special treatment, it is still just a rewiring of the diagram, regardless whether we connecting dashed boxes or not.
Finally, as we noted above, string diagrams have some notation advantages.
...
In contrast, in the string diagram, $\beta$-reduction is perfomed by a single rewrite step; the other steps perform structural $\catname{SLat}$-transformations to make the correspondence between the two structures more evident.
...
Yet, explicit substitution might be unavoidable when one wants to represent, for example, the equational theory of $\lambda_{\mathrm{sub}}$.
Before we demonstrate the advantages of string diagrams with respect to representing $\lambda_{\mathrm{lsub}}$, we need to be precise about what the concrete combinatorial representation of string diagrams is, because it is the latter that determines what structure is absorbed on the nose and what must be additionally enforced as rewrite rules.

We next present the combinatorial representation of string diagrams for closed symmetric monoidal $\catname{SLat}$-categories, which is a reconciliation of our development in~\cref{chapter:monoidal} and hypernets introduced by Ghica et al.~\cite{fscd}.

We start with the notion of closed $\Sigma$-terms.
...
\begin{enumerate}
  ...
  \item For all $A \in \operatorname{At}^{\Lambda}_{\Sigma}$ and $X,B \in \operatorname{Ob}^{\Lambda}_{\Sigma}$, if $h : X \otimes A \to B$ is a term, then
  \[
    \Lambda_{X,A,B}(h) : X \to A \multimap B
  \]
  is a term.
\end{enumerate}
\end{definition}
...
\begin{remark}
  Above we assume that the evaluation and abstraction happens with respect to atomic types.
  This is to simplify the necessary bookkeeping in the combanatorial definitions.
\end{remark}
...
\begin{definition}[Raw closed terms with joins]
The families
...
\[
  \frac{f_1 : A \to B \qquad \ldots  f_n : A \to B, \qquad n \geq 2}
       {f_1 \join \cdots \join f_n : A \to B}.
\]
All constructors act on terms of the enlarged families;
in particular, joins may occur inside abstractions.
No equations are imposed at this stage.
\end{definition}
\begin{remark}
In this chapter we are concerned with raw $\Sigma^{\join}$-terms of $\operatorname{Tm}^{\Lambda, \join}$, rather than with morphism as was the case with $\mathbf{S}^{\join}_{\Sigma}$ and $\mathbf{H}^{\join}_{\Sigma}$.
...
closed e-hypergraph interpretations.
...
\begin{enumerate}
  ...
  $f_1,\ldots,f_n:A\to B$, every permutation $\sigma$ of $[n]$ gives
  \[
    f_1 \join \cdots \join f_n
    \equiv_{\mathsf{str}}
    f_{\sigma(1)} \join \cdots \join f_{\sigma(n)},
  \]
  ...
  \]
  Here we use the convention that a one-element join denotes that term
  itself, so contracting a binary join of equal arguments gives
  $f\join f\equiv_{\mathsf{str}}f$. Together, these are the $n$-ary
  forms of commutativity, associativity and idempotence. The arity
  bound $n\geq2$ matches the hierarchical-arity condition of
  \cref{def:lambda-rooted-graph}: a $\boxsort$-edge has at least two
  components, and contracting to a single argument removes the box
  altogether.
...
\end{enumerate}
These are precisely the equations of \cref{fig:string-equations}, read
as equations between raw terms.
Neither congruence includes the $\beta$-, $\eta$-, or
currying naturality equations among its generators.
These rules perform computations, that is, effects, so we introduce them as rewrite rules instead.
\end{definition}
...
Slotted e-graphs extend the traditional signature employed by e-graphs with an explicit construct for binding ($\text{bind}\; \$x\; tc$) and variables (\emph{slots}).
	...
	\end{alignat*}
	The traditional congruence relation maintained by e-graphs for a set of equations $E$ is given below (plus the obvious rules for transitivity, symmetry and reflexivity of $\cong$):
	\begin{mathpar}
  ...
  The rule $tc \cong tc' \implies f(tc) \cong f(tc')$ is then exactly the property of composition in $\catname{SLat}$-category, namely $f(tc \join tc') = f(tc) \join f(tc')$.
  ...
  We did not have to introduce this rule in ad hoc manner, nor did we have to justify it---it is provided by the axioms of the category we work in and thus the rule is sound with respect to semantics.

\section{The carrier and its interfaces}
...
\cref{def:rooted_ehypergraph} is unchanged. In particular,
\(\top\in E_{\cmpsortsubscript}\), it has no parent, and every other
vertex or edge has a unique immediate parent. The remaining conditions
...
\[
  ...
  (i\in[n]).
\]
No order-preservation condition is imposed on \(\varphi\).
...
\[
  \varphi + \psi:[n+m]\longrightarrow[n'+m']
\]
defined by
\[
  (\varphi + \psi)(i)
...
The symmetry
\[
  \sigma_{U,V}:U + V\longrightarrow V + U
\]
...
\begin{example}[Type words of interfaces]
...
\begin{definition}[Typed discrete-interface functor]
...
\[
  ...
  \{\top_U\}.
\]
Its root is the component edge \(\top_U\), and its remaining structure
is given by
\[
  s(\top_U)=t(\top_U)=\varepsilon,
  \qquad
  l_E(\top_U)=\cmpsort,
\]
\[
  ...
  \quad (i\in[n]).
\]
There are no other relations in \(<^\mu\).
...
\(D_\Lambda(\varphi+\psi)\), and the symmetry is the image under
...
\[
  ...
  \catname{CEHyp}_{\mathrm{s}}(\Sigma)
\]
is fully faithful. Henceforth, we identify a typed finite interface
\[
  U=([n],\tau)
\]
...
That is, we will abbreviate $D([n], \tau)$ by writing $n$, so that the latter
...
\begin{enumerate}
  ...
  \item
  every strictly internal interface vertex is mapped below a
  non-root component of \(\mathcal G\).
...
The closed extended cospan is \emph{monogamous and acyclic}, or an
...
\[
X,B\in\operatorname{Ob}^{\Lambda}_{\Sigma} \qquad \text{and} A \in \operatorname{At}^{\Lambda}_{\Sigma}
\] 
such that
\[
  l_V^*\bigl(s(e)\bigr)
...
type
\[
  X\otimes A\longrightarrow B
\]
to a morphism with type
\[
  X\longrightarrow A\multimap B.
\]
Since \(A\) is atomic,
\(\operatorname{wd}(A)=\langle A\rangle\); hence the input interface
...
\begin{example}
Let the below extended cospan represent
\[
...
\]
Thus we take $X' \in \operatorname{Ob}^{\Lambda}_{\Sigma}$ to be $A \otimes B$, take $A' \in \operatorname{Ob}^{\Lambda}_{\Sigma}$ to be $X$, and take $B' \in \operatorname{Ob}^{\Lambda}_{\Sigma}$ to be $A \otimes B \otimes Y$.
...
Finally, the symmetry is induced by the symmetry of $+$ in $\mathbb{F}^{\Lambda}_{\Sigma}$, and therefore
...
\begin{definition}[Currying a closed extended cospan]
...
Let
\[
  \mathbf 1_{B\multimap C}
  \defeq
  ([1],\chi_{B,C}),
  \qquad
  \chi_{B,C}(1)\defeq B\multimap C,
\]
be the singleton typed interface labelled by \(B\multimap C\).
...
\[
  ...
  \mathbf 1_{B\multimap C}\coprod k'
  \xleftarrow{\iota_{\mathbf 1_{B\multimap C}}}
  \mathbf 1_{B\multimap C}
  \right),
\]
...
\[
  \Lambda(\mathbf G):
  \bar{n}\longrightarrow\mathbf 1_{B\multimap C}
\]
...
\begin{lemma}[Closed recursive convex decomposition]
\label{lemma:lambda-convex-decomposition}
Let \(\mathbf L\) and \(\mathbf F\) be well-typed MDA closed extended
cospans, and let
\[
...
cospan \(\mathbf D\), constructed from \(\mathbf L\) and other
well-typed MDA closed extended cospans by composition, tensor product,
join and abstraction, and the isomorphism
\[
  \mathbf{\Phi}:\mathbf D\xrightarrow{\cong}\mathbf F
\]
such that, for the canonical occurrence
\[
  \iota_{\mathcal L}:
...
\(\lambdasort\)-edge. 
Then \(a\) is the unique component child of \(p\).
It determines a well-typed component cospan
\[
  \mathbf F_a:X\otimes A\longrightarrow B,
\]
where
\[
...
\[
  \mathbf C_1^a:
  X\otimes A\longrightarrow q\coprod i,
  \qquad
  \mathbf C_2^a:
  q\coprod j\longrightarrow B,
\]
and the isomorphism
\[
  \mathbf{\Phi}_{a}:
...
\(\Lambda(\mathbf F_a)\), abstraction induces the isomorphism
\[
  \mathbf{\Phi}_p:
  \Lambda(\mathbf D_a)
  \xrightarrow{\cong}
...
\[
  ...
  \Lambda(\mathcal F_a)
\]
be the canonical inclusions of the respective component carriers.
...
\(p\) gives \(\phi_p\) and preserves this equation.
...
\subsection{Operations and concrete interpretation}

\begin{definition}[Concrete interpretation of closed terms]
...
We define the interpretation of raw closed terms recursively. A term
\[
...
and
\[
  \widehat{\Lambda_{X,A,B}(h)}
  \defeq
  \Lambda(\widehat h).
\]
Here \(A\in\operatorname{At}^{\Lambda}_{\Sigma}\), so the final input
position of
\[
  \widehat h:
  \partial X\coprod\partial A\longrightarrow\partial B
\]
...
cospans of \cref{def:join_of_extended_cospans}. The arity bound
matches the hierarchical-arity condition of
\cref{def:lambda-rooted-graph}, so the result is again a well-typed
MDA closed extended cospan.
...
\label{lemma:lambda-join-free-faithfulness}
For raw closed terms
\[
...
hypergraph interpretation therefore gives
\[
  \bar s
  \equiv_{\mathsf{SMC}}^{\Gamma_\phi}
  \bar t.
\]
...
Let \(\mathbf S_{\Gamma_\phi}\) be the free symmetric monoidal category
on the multi-sorted signature \(\Gamma_\phi\), and let
...
\]
Let \(t^\phi\) be the closed \(\Sigma\)-term obtained by interpreting
each occurrence of \(x_e\) in \(\bar t\) as \(\Lambda(h_e)\). Then
\[
  F_\phi([\bar t]_{\mathsf{SMC}})=[t^\phi]_0.
\]
On the other hand, \(t\) is recovered from \(\bar t\) by interpreting
each \(x_e\) as \(\Lambda(h'_e)\). Since
\[
  \Lambda(h_e)\equiv_0\Lambda(h'_e)
\]
...
\subsection{Normal forms of raw terms}
...
\[
 N_{A,B}:
 \operatorname{Tm}^{\join,\Lambda}_{\Sigma}(A,B)
 \longrightarrow
...
For raw closed terms with joins
\[
  s,t\in
  \operatorname{Tm}^{\Lambda,\join}_{\Sigma}(A,B),
\]
...
\[
  ...
  \tag{\ref{lemma:lambda-term-normal-forms}.2}
...
The point of using a finite set of $\equiv_{0}$-classes, rather than an
expression $t_1\join\cdots\join t_n$, is that the former is canonical.
No orientation of the structural equations, and hence no termination or
confluence argument for such an orientation, is needed.

\subsection{Complete resolutions}

A \emph{complete $\catname{SLat}$-resolution} removes every
$\boxsort$-edge by replacing it by one selected component, with its
boundary glued to the box's outer ports. More precisely, define the set
of resolutions by recursion on box depth. Resolve each child component
recursively. At a $\lambda$-edge retain that edge and its unique component
with the chosen resolved body. At a $\boxsort$-edge select one of its
resolved components and replace the \emph{whole} box cospan by that
component using \cref{lemma:lambda-cut-replace}. Discard the other
component subtrees. In each frame perform these replacements for its
immediate boxes and retain all ordinary edges.

The source of such a replacement consists of the box, its ports, a fresh
component root and all its component subtrees. Its canonical occurrence
is a convex match in the sense of \cref{def:lambda-convex-match}. Its
replacement has the same external type.
This is the mixed version of the selection operation in
\cref{def:flat_resolutions}, not a structural equation. In particular,
one does not leave a unary $\boxsort$-edge behind. The selected component's
root is identified with the ambient component, its former external
boundary is glued in order to the cut ports, and its deeper interface
blocks are retained. A $\lambda$-edge survives unless it belongs to a
discarded component subtree.
...
\end{lemma}
\begin{proof}
Neither composition nor tensor introduce new $\boxsort$-edges, so the sets of complete resolutions are given by those of component combined using either composition or tensor.
...
For every raw $\Sigma^{\join}$-term $t \in \operatorname{Tm}^{\Lambda,\join}_{\Sigma}$,
\[
...
Suppose $\mathcal L,\mathcal R:A\to B$ have the same external type and
$\operatorname{Flat}(\mathcal L)=\operatorname{Flat}(\mathcal R)$.
If a direct closed EDPOI replacement of $\mathcal L$ by $\mathcal R$
takes $\mathcal G$ to $\mathcal H$, then
\[
 \operatorname{Flat}(\mathcal G)=\operatorname{Flat}(\mathcal H).
\]
Their strictly internal interfaces need not be the same before resolution.
\end{lemma}
\begin{proof}
Since the match $m : \mathcal{L} \to \mathcal{G}$ is convex, there is a decomposition of $\mathcal{G}$ as in~\cref{lemma:lambda-convex-decomposition}.
Under such a decomposition, $\mathcal{G}$ is obtained by repeated composition, tensor, join and currying around $\mathcal{L}$.
Since $\operatorname{Flat}(\mathcal L) = \operatorname{Flat}(\mathcal R)$, \cref{lemma:lambda-resolution-operations} gives us $\operatorname{Flat}(\mathcal{G}) = \operatorname{Flat}(\mathcal{H})$.
\end{proof}
...
\label{lemma:lambda-resolution-invariance}
...
%   \operatorname{Can}(\operatorname{Flat}(\widehat t)).
% \]
As in~\cref{lemma:canonical_representative}, we can turn $\widehat{t}$ into the form $\bigjoin \mathcal{F}_{i}$ such that $\mathcal{F}_{i}$ is box-free and $\mathcal{F}_{i} \not \cong \mathcal{F}_{j}$ for $i \neq j$.
Since $\operatorname{Flat}(\widehat{t})$ is invariant under $\sim_{\mathfrak{R}_{\Lambda}}$ and since each $\mathcal{F}_{i}$ is box-free, we have
that 
\[
\operatorname{Flat}(\bigjoin \mathcal{F}_{i}) = \bigcup_{i} \operatorname{Flat}(\mathcal{F}_{i}) = \bigcup_{i} \{[\mathcal{F}_{i}]_{\cong}\} = \operatorname{Flat}(\widehat{t})~,
\]
therefore
\[
\widehat{t} \sim_{\mathfrak{R}_{\Lambda}} \mathcal{F}_{i} \cong \operatorname{Can}(\operatorname{Flat}(\widehat{t}))~.
\]
\end{proof}
...
For raw closed $\Sigma^{\join}$-terms $s,t \in \operatorname{Tm}^{\Lambda, \join}(A, B)$,
\[
 s\equiv_{\mathsf{str}}t
 \quad\Longleftrightarrow\quad
...
Conversely, suppose
$\widehat s\sim_{\mathfrak R_{\Lambda}}\widehat t$. 
By~\cref{lemma:lambda-resolution-invariance}, structural rules preserve
...
\cref{lemma:lambda-join-free-faithfulness}.  Therefore $N(s)=N(t)$, and
\cref{lemma:lambda-term-normal-forms} yields
$s\equiv_{\mathsf{str}}t$.
\end{proof}

\subsection{Role of normal forms in operational rewriting}

The normal forms above are used only to prove
\cref{theorem:lambda-structural-adequacy}.  They do not restrict the
representatives on which an operational rule may be matched.  This
distinction matters because distributivity can duplicate a redex.  For a
rule $l\to r$, for example, the single step
\[
 l\fatsemi(g\join h)
 \longrightarrow
 r\fatsemi(g\join h)
\]
has expanded source and target
\[
 (l\fatsemi g)\join(l\fatsemi h)
 \qquad\text{and}\qquad
 (r\fatsemi g)\join(r\fatsemi h).
\]
Rewriting the two displayed residuals separately uses two directed
applications, whereas the factored representative admits one. Thus
expanding representatives need not preserve the length of a direct
rewrite sequence. Modulo structural equality the expanded source can
instead be factored, rewritten once, and expanded again.

For the subsequent soundness and completeness theorem, structural adequacy
is instead used before and after one genuine rule application:
\[
 \widehat a
 \sim_{\mathfrak R_{\Lambda}}
 \mathcal A'
 \mathrel{\widetilde{\rightsquigarrow}_{\widehat l\to\widehat r}}
 \mathcal B'
 \sim_{\mathfrak R_{\Lambda}}
 \widehat b.
\]
The operational step itself is handled by the mixed-box context-extraction
lemma.  Keeping these two arguments separate avoids any residual or
commutation theorem between structural normalisation and operational
rewriting.

\section{The rewriting correspondence}
\label{sec:lambda-rewriting-correspondence}

We now compare directed rewriting of raw closed terms modulo
$\equiv_{\mathsf{str}}$ with directed EDPOI rewriting modulo
$\mathfrak R_{\Lambda}$.  Throughout this section a rule
\[
 \rho=(l\longrightarrow r):A\longrightarrow B
\]
is ground: $l$ and $r$ contain no term metavariables.  A schematic rule may
instead be treated by replacing it with all of its well-typed substitutional
instances.  Nothing in the argument below otherwise changes.
...
For a rewrite rule $\langle l, r \rangle$ of $\Sigma$-terms $l,r \in \operatorname{Tm}^{\Lambda, \join}_{\Sigma}(P,Q)$ and $a,b \in \operatorname{Tm}^{\Lambda, \join}_{\Sigma}(A,B)$ , write
\[
 a\longrightarrow_{\langle l, r \rangle/\mathsf{str}}b
\]
if there are raw terms $a',b'$ and a typed one-hole context $C[-]$:
\begin{equation}
\begin{aligned}
  &[-], C[-]\fatsemi t,
  \quad
  ...
  \quad
  t\otimes C[-],\\
  &t_1\join\cdots\join C[-]\join\cdots\join t_n,
  \quad
  \Lambda_{X,A,B}(C[-])
\end{aligned}
...
The corresponding graph relation must make the same distinction between
reversible structure and directed computation.
...
\[
 \mathcal G
 \mathrel{\widetilde{\rightsquigarrow}^{\,\mathfrak R_{\Lambda}}_{\widehat\rho}}
 \mathcal H
\]
...
\[
 A\defeq\left\lVert\operatorname{ty}_{n}\right\rVert_{\otimes},
 \qquad
 B\defeq\left\lVert\operatorname{ty}_{k}\right\rVert_{\otimes},
\]
...
and
 ...
 gives raw terms $u_i:P\to Q$ with
 $\widehat{u_i}\cong\mathbf G_{c_i}$.  
 Put
 \[
  w\defeq u_1\join\cdots\join u_s.
 \]
 This is the step where we utilise n-ary joins in the syntax, otherwise we would have to reify terms up to structural equivalence.
 ...
 \[
  ...
  Q=D\multimap C,
  \qquad
  \mathbf G_c:\partial(X\otimes D)\longrightarrow\partial C.
 \]
 Therefore,
 \[
...
%  \cref{lemma:lambda-convex-decomposition}.  
 ...
 raw term $v:X\otimes D\to C$ with $\widehat v\cong\mathbf G_c$.  Put
 \[
  w\defeq\Lambda_{X,D,C}(v).
 \]
\end{enumerate}
...
\[
 ...
 \nu(\mathbf C_2)<\nu(\mathbf G).
\]
The induction hypothesis gives raw terms $c_1$ and $c_2$ with
$\widehat{c_1}\cong\mathbf C_1$ and $\widehat{c_2}\cong\mathbf C_2$.
Write $K\defeq\lVert\operatorname{ty}_q\rVert_{\otimes}$, and put
\[
...
\[
 \widehat u
 =
 \widehat{c_1}\fatsemi(\id_{q}\otimes\widehat w)\fatsemi\widehat{c_2}
 \cong
 \mathbf C_1\fatsemi(\mathbf 1_q\otimes\mathbf L)\fatsemi\mathbf C_2
 \cong
 \mathbf G.
...
\cref{theorem:lambda-structural-adequacy}, it yields an isomorphism
\[
...
\label{prop:lambda-one-step-rewriting}
Let $l, r \in \operatorname{Tm}^{\Lambda,\join}_{\Sigma}(A,B)$ be raw
terms, and let $\rho = \langle l, r \rangle$ be the corresponding
rewrite rule. For raw terms
...
of~\cref{theorem:convex_edpoi_rewriting_slat} with the only exception
that we work exclusively with raw terms.
...
forms~\eqref{eq:typed-one-hole-context}. Say
...
For every fixed closed extended cospan occurring in this finite
