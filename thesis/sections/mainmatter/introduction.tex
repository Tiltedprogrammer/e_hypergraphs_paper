...
\begin{example}\label{example:egraph_example_introduction}
The e-graph presentation of rewriting in~\cref{example:term_rewrite} can be seen in~\cref{fig:e-graph_example}.
...
\begin{figure}
    ...
    \]
    \caption{E-graph example due to Willsey et al.~\cite{EggPaper}.}
...
We can build terms from these generators by composing them horizontally ($\fatsemi$) and vertically ($\otimes$) with each other and with additional generators $\sym : 2 \to 2$ and $\id : 1 \to 1$---symmetries and identities.
...
These constitute a symmetric monoidal theory and, together with the structural equations of a symmetric monoidal category, induce a symmetric monoidal category with commutative comonoid structure.
\end{example}
...
Given the setting of~\cref{example:comonoids}, and the term $((\mu;\sym);(\id \otimes \eta)) \otimes (\mu ; \id \otimes \id)$, we can get the following e-graph:
\[
\tikzfig{./figures/introduction/e-graph-comonoid-example}
\]
By saturating the e-graph with the equations $\mathcal{E}$ and the structural equations of a symmetric monoidal category (defined formally in Appendix~\ref{appendix:smce-rules}), we can establish that the term is equivalent to $(\mu \otimes \mu);(\id \otimes \eta \otimes \id \otimes \id)$, or to $\id \otimes \mu$.
The e-graph grew from $10$ to $8{,}546$ nodes before reaching saturation.
\cref{tab:comonoid-rules} shows the number of times each rule was applied in each direction during saturation; the tensor-structural rules (tensor associativity and the tensor unit) account for the majority of applications, reflecting the overhead of encoding the monoidal structure in the flat e-graph representation, while the comonoid-specific rules (commutativity and the counit law) were applied only once each.
\end{example}
...
\bottomrule
\end{tabular}
\caption{Rewrite rule applications during saturation (\cref{example:sat-comonoid}); $n \in \{1,2,3,4\}$. Columns $(\to)$ and $(\leftarrow)$ count applications in each direction; --- indicates zero. Purely definitional rules (e.g.\ $\id_2 = \id_1 \otimes \id_1$) are omitted; see Appendix~\ref{appendix:smce-rules}.}
...
We refer the interested reader to Appendix~\ref{appendix:smce-rules} for details on comonoid symmetric monoidal theory encoding in \egg.

This suggests that perhaps the choice of encoding is wrong for the comonoids within the e-graph framework.
Is it somehow possible to get rid of the extra rewrites so that they are absorbed by the encoding?
One alternative is an \emph{internal language for monoidal categories}, in which monoidal terms are written in a form of type theory whose substitution corresponds to composition, requiring only a subset of rules that we had to include above~\cite{shulman2016categorical}.
...
The structure of symmetric monoidal categories is absorbed into the graphical syntax, with the composition and tensoring represented by juxtaposition, and symmetries as crossings of strings.
...
The basic building blocks of string diagrams are given in~\cref{fig:string-diagram-syntax}.
Ticks on the wires are a shorthand for an arbitrary number of wires.
These string diagrams are topological objects and two diagrams represent the same morphism if one diagram can be continuously deformed into the other, by sliding the boxes along the wires and untangling the wires that cross twice:
\[
\scalebox{0.8}{
...
With this \enquote{enhanced} syntax, we can write down the term from~\cref{example:sat-comonoid} and apply the rewrites to it with redexes outlined by dashed boxes:
\[
...
Some information is lost, however, when we apply the rewrites, namely we cannot recover the original term from the rightmost diagram (without applying the rewrites).
...
This box must satisfy the equations from~\cref{fig:e-box-equations} to ensure that it correctly captures the equivalences.
\begin{figure}
  \[
...
On top of this, we recover a fragment of traditional e-graphs as a special case of our framework; this constitutes a categorical semantics for e-graphs.

We next give an overview of another domain, $\lambda$-calculi, where traditional e-graphs underperform, to see if string diagrams can provide any insight into mitigating the issues there.
\section*{Lambda Calculi}
It is hard to overstate the importance of $\lambda$-calculi---an umbrella
term for variants of the $\lambda$-calculus---in computer science,
particularly as foundations for functional programming languages.
...
The equivalence of the $\lambda$-abstractions above would then be justified by the following derivation:
\[
\inferrule*[]{
    \inferrule*[]{
        \inferrule*[]{\inferrule*[]
...
\]
The introduction of variable transposition would pollute the e-graph and lead to the same behaviour that we observed in~\cref{example:sat-comonoid}.
...
So $[v/x]t$ must be read as \enquote{apply the substitution of the e-class $v$ for the variable $x$ throughout the entire equivalence class represented by $t$.} 
...
Consider the example in~\cref{fig:explicit-substitution-egg}, where the first e-graph represents the application of a $\lambda$-abstraction to $1$.
...
The last e-graph represents the result of fully propagating the substitution, once we have reached the e-classes with the actual variables.
...
An example of a slotted e-graph for a term $\lambda f .\, f ((\lambda x .\, f x) 2)$ is given in~\cref{fig:slotted-example}. 
...
For example, when computing the substitution $(\lambda x . f x)[x / f]$, the bound $x$ is distinct from the $x$ being substituted in, as a globally unique name is picked for the bound variable.
\begin{figure}[t!]
    ...
    \]
		\subcaption{Slotted e-graph containing $\lambda f .\,f((\lambda x .\, f x)2)$.}%
    ...
    \]
    \subcaption{String diagrammatic representation of $\lambda f .\,f((\lambda x .\, f x)2)$.}
    \label{fig:closed-string-diagram-example}
    \end{subfigure}
    \caption{Slotted e-graph (a) and string diagram (b) representations of the same term.}
\end{figure}

Remarkably, the challenges we mentioned above can be resolved by using string diagrams in place of textual syntax.
Consider the traditional string diagrammatic syntax from \cref{fig:string-diagram-syntax} and extend it with the constructs for $\lambda$-abstraction and application as in~\cref{fig:lambda-string}, plus the ability to fork wires $\comonoid$ and to delete wires $\delete$.
...
\begin{example}
The string diagrammatic representation of the term $\lambda f .\, f ((\lambda x .\, f x) 2)$ is given in~\cref{fig:closed-string-diagram-example}.
We label the wires with variables to see the correspondence more easily, but only the connectivity matters: $\lambda g .\, g ((\lambda y .\, g y) 2)$ would give the same string diagram.
...
This can be observed in action in~\cref{fig:beta-reduction-string-diagrams}, which is a string diagrammatic version of~\cref{fig:explicit-substitution-egg}.
...
Then we can pull some nodes out of the dashed box, by using the deleting nodes $\delete$ to properly connect the inputs of the components inside the box with the nodes outside: a diagram that terminates with the $\delete$ node can be completely discarded.
This illustrates that we can achieve a compact representation of equivalences using string diagrams.
Also note that unlike in traditional e-graphs we completely avoid the substitution into $y$ as it simply does not use the necessary wire.

If we were to perform the same $\beta$-reduction using slotted e-graphs, we would end up with a very similar result, so at first glance it may seem that there is not much benefit in string diagrams over slotted e-graphs.
However, the substitution in string diagrams is semantically well-defined---it is still a simple rewiring of diagrams even if the diagrams of interest have multiple components (i.e. if they are dashed boxes), which semantically corresponds to an equation between two morphisms in a closed semilattice-enriched monoidal category.
In slotted e-graphs, the substitution is a mere operation on the data structure.
Another, less obvious, advantage of string diagrams is connected to explicit substitution.
...
% In particular, it makes it possible to write down e-graphs as terms in a principled way as we do in~\cref{lst:terms-e-graph}.
...
% \caption{Terms corresponding to e-graphs in \cref{fig:e-graph_example}}
% \label{lst:terms-e-graph}
% \end{listing*}
\section*{Contributions of this Thesis}
This thesis aims to compensate the disproportion between the number of works devoted to the applications (or algorithms) of e-graphs~\cite{EggPaper,slotted-egraphs,singher2023colored,koehler2022sketchguided,Caviar,EqualitySaturationTensorGraphSuperoptimization} and the works devoted to the study of their semantics~\cite{zhang_relational_2022,tree_automata_e_graphs, EggsAreAdhesive}, which it does from a categorical point of view here.
Below we summarise the contributions of this thesis and give an overview of its structure.

\paragraph{Semilattice enriched categories.}
We propose \emph{semilattice-enriched categories} as a categorical setting for studying e-graphs.
While enriched category theory is a well-established field of mathematics, its application to the semantics of e-graphs appears to be new.
We recapitulate the necessary properties of semilattice-enriched categories in~\cref{chapter:enriched}, showcasing that the structure of semilattice-enriched categories is suitable for modelling a fragment of e-graphs.
We also identify a tension between the categorical structure and the usual semantics of e-graphs: in particular, free semilattice enrichment does not preserve binary products in the required sense.
This prevents the framework from capturing the full generality of traditional e-graphs, but also suggests a categorical foundation for \emph{substructural} variants.
More broadly, semilattice enrichment may provide a semantic core upon which additional structure can be built.

\paragraph{Monoidal e-graphs.}
The second contribution, presented in~\cref{chapter:monoidal}, is a generalisation of e-graphs for \emph{monoidal} theories~\cite{tiurin2025equivalencehypergraphsdporewriting}.
We characterise these e-graphs as morphisms in a free semilattice-enriched symmetric monoidal category and give them a concrete combinatorial representation as extended cospans of hierarchical hypergraphs, which we call \emph{e-hypergraphs}.
The main technical results are the soundness and completeness of this combinatorial representation with respect to the class of morphisms it represents, and
the equivalence theorem for DPO rewriting of e-hypergraphs with respect to the equational theory of semilattice-enriched symmetric monoidal categories.
This correspondence provides a graph-rewriting account of equality saturation for symmetric monoidal theories.
It also opens the framework to theories arising in areas such as quantum computing, signal flow, and digital circuits, while providing a basis for substructural e-graphs.


\paragraph{Categorical e-graphs for $\lambda$-calculi.}
The final main contribution, presented in~\cref{chapter:lambda}, extends the categorical framework to languages with variable binding, specifically monoidal closed theories and the $\lambda$-calculus.
We work with free semilattice-enriched symmetric monoidal closed categories: the closed structure provides the internal hom needed to model abstraction and application, while the graphical wiring accounts for substitution and the semilattice enrichment continues to model e-class membership.
We introduce \emph{closed e-hypergraphs} as a concrete representation of the resulting terms, accounting for the interaction between hierarchical e-boxes and closed structure.
We prove that this representation is sound and complete modulo the relevant structural equations, and that convex closed EDPOI rewriting modulo structural rules corresponds exactly to rewriting of closed-monoidal terms.
This yields a categorical account of e-graphs for languages with binding, grounded in an established semantic framework.

\section*{Structure of the Thesis}

\Cref{chapter:algebras} recalls the necessary background on algebraic theories and provides an algebraic treatment of traditional e-graphs.
\Cref{chapter:enriched} recapitulates the required enriched category theory, specialising it to semilattice enrichment and examining both its suitability and its limitations as a semantics for e-graphs.
\Cref{chapter:string_diagrams} reviews string diagrams, their representation by hypergraphs, and DPOI rewriting, providing the technical background for the later constructions.
\Cref{chapter:monoidal} introduces monoidal e-graphs, develops their representation by e-hypergraphs, and establishes the correspondence between enriched term rewriting and EDPOI rewriting.
\Cref{chapter:lambda} extends this framework to variable binding through closed e-hypergraphs and applies it to $\lambda$-calculi.

The reader is assumed to be familiar with basic category theory and monoidal categories.