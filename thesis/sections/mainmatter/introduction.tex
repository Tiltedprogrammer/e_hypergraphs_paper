...
\]
By saturating the e-graph with the equations $\mathcal{E}$ and the structural equations of a symmetric monoidal category (defined formally in~\cref{appendix:smce-rules}), we can establish that the term is equivalent to $(\mu \otimes \mu);(\id \otimes \eta \otimes \id \otimes \id)$, or to $\id \otimes \mu$.
...
\bottomrule
\end{tabular}
\caption{Rewrite rule applications during saturation (\cref{example:sat-comonoid}); $n \in \{1,2,3,4\}$. Columns $(\to)$ and $(\leftarrow)$ count applications in each direction; --- indicates zero. Purely definitional rules (e.g.\ $\id_2 = \id_1 \otimes \id_1$) are omitted; see \autoref{appendix:smce-rules}.}
...
We refer the interested reader to~\cref{appendix:smce-rules} for details on comonoid symmetric monoidal theory encoding in \egg.
...
We next give an overview of another domain, \emph{lambda calculi}, where traditional e-graphs underperform, to see if string diagrams can provide any insight into mitigating the issues there.
...
Finally, \cref{chapter:type_theory} sketches a complementary type-theoretic perspective based on semilattice-enriched multicategories.
