...
At its core, the idea is simple: a set of equations is oriented into directed \emph{rewrite rules}, and these rules are applied to transform an expression into an equivalent but more desirable form---simpler, more normal, or cheaper to evaluate.
...
Consider a \emph{monoidal} signature $\Sigma = \{\mu : 1 \to 2, \eta : 1 \to 0\}$---comultiplication and counit---where numbers indicate the number of inputs and outputs of each generator.
...
This raises the question of whether we chose the wrong encoding of comonoids within the e-graph framework.
Is it somehow possible to get rid of the extra rewrites so that they are absorbed by the encoding?
One alternative is the \emph{internal language} of monoidal categories, in which monoidal terms are written in a form of type theory whose substitution corresponds to composition, requiring only a subset of rules that we had to include above~\cite{shulman2016categorical}.
...
Another alternative is to enter the realm of two-dimensional syntax of \emph{string diagrams} that were specifically designed to represent morphisms of monoidal categories.
...
Ticks on the wires are a shorthand for arbitrary number of wires.
...
This thesis provides a positive answer to this question by augmenting the string diagrammatic syntax with \emph{e-boxes} drawing the inspiration from Melliès's functorial boxes~\cite{mellies_functorial_2006}, though e-boxes do not correspond to functors.

We introduce the following construct
\[
\tikzfig{./figures/introduction/e-box}
\]
which applies an operation that we denote by $\join$, to distinguish it from arithmetical $+$ which we may occasionally use and formally defined later, to sub-diagrams.
...
This problem was addressed is a series of works by Bonchi, Gadducci, Kissinger, Sobociński, and Zanasi~\cite{bonchi_string_2022-1,bonchi_string_2022-2,bonchi_string_2022-3}.
...
It is hard to overestimate the importance of lambda calculi---an umbrella term for different variants of lambda calculus---in computer science and the number of construction it underlies, in particular, functional programming languages.
...
The challenges above we first noted by Koehler et al.~\cite{koehler2022sketchguided} in the context of e-graphs.
...
The substitution $t[v / x]$ is then can be built into the e-graph by traversing all e-nodes in the e-class of $t$ replacing each reference to the e-class of $x$ with the e-class of $v$.
This is automatically capture-avoiding, since for each lambda abstraction the bound variable has a unique name this is not used anywhere else in the e-graph.
...
The very fist step introduces a new component into the dashed box which is the second argument of the evaluation node connected to the dangling wire of the erased box.
...
The translation of other e-classes into terms follow the same pattern.
