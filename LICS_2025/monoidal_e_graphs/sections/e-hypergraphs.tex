...
These edges can be designated as \enquote{parents} to sub-e-hypergraphs,  which interpret the various components of the e-boxes.  
...
The condition $\consistency_{p} \not = (E_{p} \sqcup V_{p}) \times (E_{p} \sqcup V_{p})$, in particular, does not allow hierarchical edges with no delimiting lines.
...
\] and mono $m : \mathcal{L} \to \mathcal{G}$ in $\catname{EHyp(\Sigma)}$, a pushout complement $i \sqcup j \to \mathcal{L}^{\bot} \to \mathcal{G}$
...
\begin{enumerate} 
    ...
    \item for all $v,w$ in $\mathcal{G}$ in the image of $i \sqcup j$,  $v$ and $w$ share the same set of parents, and either $v,w$ are top-level or else $v \consistency w$;
    \item for all $v,w$ in $\mathcal{L}^{\bot}$ in the image of $i \sqcup j$,  $v$ and $w$ share the same set of parents, and either $v,w$ are top-level or else $v \consistency w$;
...
\begin{tikzcd}[cramped, row sep=mysize, column sep = tiny]
	{n \sqcup j} &&&&&& {k \sqcup i} \\
	& {n' \setminus (i' \setminus i) \sqcup j} & \vspace{16em} & {\mathcal{L}^{\bot}} & \vspace{16em} & {k' \setminus (j' \setminus j) \sqcup i}
    \arrow["{f_1 \sqcup id_{j}}"{xshift=-0.7ex}, from=1-1, to=2-2]
	\arrow["{f_2 \sqcup id_{i}}"'{xshift=1ex}, from=1-7, to=2-6]
...
\]
    ...
    \[
    \hspace{-2em} n \xrightarrow{f_1} n' \setminus (i' \setminus i) \sqcup j \xrightarrow{[g_1,c_2]} \mathcal{L}^{\bot} \xleftarrow{[g_2,c_1]} k' \setminus (j' \setminus j) \sqcup i \xleftarrow{f_2} k
    \]
\end{enumerate}
...
Note that,  in the above definition,  the extended cospan 
    \[
    \vspace{-1mm}
    n \xrightarrow{} n' \setminus (i' \setminus i) \sqcup j \xrightarrow{[g_1,c_2]} \mathcal{L}^{\bot} \xleftarrow{[g_2,c_1]} k' \setminus (j' \setminus j) \sqcup i \xleftarrow{} k
    \vspace{-0.5mm}
    \]
...
\[
    \mathcal{G} \xleftarrow{} n' \sqcup k' \xleftarrow{} n \sqcup k \xrightarrow{} n'' \sqcup k'' \xrightarrow{} \mathcal{H}
\]
in $\catname{EHyp(\Sigma)}$, we say $\mathcal{G}$ \textit{rewrites (convexly) to} $\mathcal{H}$ (\textit{with external interface} $n \sqcup k$ and \textit{taking internal interface} $n' \sqcup k'$ \textit{to} $n'' \sqcup k''$) --- denoted by $\mathcal{G} \Rrightarrow \mathcal H$  --- \textit{via a rewrite rule} 
\[
    \mathcal{L} \xleftarrow{} i' \sqcup j' \xleftarrow{} i \sqcup j \xrightarrow{} i'' \sqcup j'' \xrightarrow{} \mathcal{R}
\] 
...
such that the two marked squares are pushouts and the following conditions hold:
	...
	\[
		n'' = n' \setminus (i' \setminus i) \sqcup (i'' \setminus i) \qquad k'' = k' \setminus (j' \setminus j) \sqcup  (j'' \setminus j)\\
	\]
