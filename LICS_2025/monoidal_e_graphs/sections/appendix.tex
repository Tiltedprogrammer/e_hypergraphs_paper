...
\end{remark}
  ...
  \begin{definition}
    ...
    \begin{remark}
        One could think about this free construction in the following way.
        Given a set $X$ a free (unbounded) semilattice over $X$ is a set of all non-empty subsets of $X$.
        The $+$ is then given by a union of sets.

        We will next illustrate the difference between $\otimes$ and $\times$ when it comes to the free construction.
        Consider two sets $X$ and $Y$ and their tensor product $X \times Y$
        ...
        \end{align*}
        The isomorphism between $F(X) \otimes F(Y)$ and $F(X \times Y)$ is constructed as follows.
        Given a set of subsets of $X \times Y$, e.g., $\{(1,b),(2,b),(3,a)\}$ we map it to $\{\{1,2\},b\} + \{3,a\}$, essentially grouping by the components of the pairs.
        Different ways to group things are then identified.
        If we were to use $\times$ as the tensor, we would face a problem when mapping, \textit{e.g.}, $\{(1,a), (2,a), (1,b), (2,b)\}$ and $\{(1,a),(2,b)\}$ as both would be identified as $\{\{1,2\},\{a,b\}\}$.
    \end{remark}
...
\begin{remark}
    Both $<$ and $\consistency$ can be considered as (partial) functions defined on $V_{\mathcal{F}}  \sqcup  E_{\mathcal{F}}$, \textit{i.e.}, on the coproduct of vertices and edges.
...
\begin{proof}
    ...
    \begin{align*}
          ...
          &\qquad y_j = g_{V};\iota_{2,V}(z) \text{ where $x_i \in V_{X}$ and $y_j \in V_{Y}$ }
        \}\\
        &\cup \;\{(x,x)\;\text{where}\; x \in V_{X}  \sqcup  V_{Y}\}\\
    ...
    % \begin{align*}
    % S_{E} &= \{\\
    % &(x_i,y_j) \in (E_{X}  \sqcup  E_{Y}) \times (E_{X}  \sqcup  E_{Y})\; |\\
...
\begin{align*}
    \sfrac{X  \sqcup  Y}{\sim (R_{V},R_{E})} = \{
        &\sfrac{V_{X}  \sqcup  V_{Y}}{\sim (R_{V},R_{E})}, \sfrac{E_{X}  \sqcup  E_{Y}}{\sim (R_{V},R_{E})}, \sfrac{s_{X \sqcup Y}}{\sim (R_{V},R_{E})},\\
        &\sfrac{t_{X \sqcup Y}}{\sim (R_{V},R_{E})}, \consistency_{\sfrac{X \sqcup Y}{\sim (R_{V},R_{E})}}, <_{\sfrac{X  \sqcup  Y}{\sim (R_{V},R_{E})}}\}    
\end{align*}
\else
    \begin{align*}
        \sfrac{X  \sqcup  Y}{\sim (R_{V},R_{E})} = (&
            ...
            &\sfrac{t_{X \sqcup Y}}{\sim (R_{V},R_{E})},\\
            &\consistency_{\sfrac{X \sqcup Y}{\sim (R_{V},R_{E})}},\\
            &<_{\sfrac{X  \sqcup  Y}{\sim (R_{V},R_{E})}})    
    \end{align*}
\fi
...
% So far, the construction matches the construction of the pushout in $\catname{Hyp_{\Sigma}}$ which can be found in~\cite{bonchi_string_2022-1} and hence $s_{\sfrac{X \sqcup Y}{\sim}}$ and $t_{\sfrac{X \sqcup Y}{\sim}}$ are well-defined and $[]$ is a homomorphism with respect to sources and targets and therefore $\sfrac{\iota_1}{\sim}$ and $\sfrac{\iota_{2}}{\sim}$ are.

Recall that $<$ is essentially a transitive closure of $<^{\mu}$ and hence we can first define 
    \[<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}[e])
    \] and 
    \[
        <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[u])
    ...
    \begin{enumerate}
        \item If there exists $v$ such that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))$ is defined and $u \sim v$
              we let
              \[
                <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([u])) = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))]
              \]
        \item \label{def:child_respects_connectivity} $u$ has no pre-image in $V_{Z}$ and $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(u))$ is undefined (\textit{i.e.}, $[u) = \varnothing$).
              If there exists $v$ such that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) = e'$ and such that there is an \textit{undirected} path from $[u]$ to $[v]$, then we define
              \[ 
                <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[u]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v])
              \]
    \end{enumerate}
    ...
    There is no case when $u$ has a pre-image in $V_{Z}$ and yet $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(u))$ is undefined and there exists $v$ such that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) = e$ and there is a path from $[u]$ to $[v]$.
    ...
    Otherwise, we leave $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([u]))$ undefined.
    ...
    Consider two cases.
    \begin{enumerate}
    \item  $<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e)) = e'$.
            Then we define
        \[
            <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}[e]) = [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e))]
        \]
    \item $[e) = \varnothing$ and there exists $v$ such that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) = e'$ and such that there is an undirected path from $[e]$ to $[v]$, we define 
        \[
            <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}[e]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v])
        \] 
    \end{enumerate}
    Otherwise we leave $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}{\sim}([e]))$ undefined.
    ...
    Then, it must be the case that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1)) = <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_2))$.
    Consider cases.
    \begin{itemize}
    ...
    Suppose, $[u) = \varnothing$ and there exist $v_1$ and $v_2$ such that $<_{X \sqcup Y}^{\mu}(v_1) = e_1$ and $<_{X \sqcup Y}^{\mu}(v_2) = e_2$ and $e_1 \not = e_2$ and such that there is a path from $[u]$ to $[v_1]$ and from $[u]$ to $[v_2]$.
    ...
    \begin{itemize}
            ...
            \item Suppose $v_1 \sim_{S} v_1'$ which means there exists $z_1 \in V_{Z}$ such that $v_1 = f_{V};\iota_{1,V}(z_1)$ and $v_1' = g_{V};\iota_{2,V}(z_1)$ and suppose $v_2 \sim_{S} v_2'$ which further means there exists $z_2 \in V_{Z}$ such that $v_2 = f_{V};\iota_{1,V}(z_2)$ and $v_2' = g_{V};\iota_{2,V}(z_2)$.
                  Then we have
                  \begin{align*}
                    <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1)) &= <_{X \sqcup Y}^{\mu}(f_{V};\iota_{1,V};\iota_{V_{X}  \sqcup  V_{Y}}(z_1))\\
                                                              &= \iota_{1,E}(<_{X}^{\mu}(f_{V};\iota_{V_{X}}(z_1)))
                \end{align*}
                  and
                  \begin{align*}
                    <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_2)) &= <_{X \sqcup Y}^{\mu}(f_{V};\iota_{1,V};\iota_{V_{X}  \sqcup  V_{Y}}(z_2))\\
            ...
            \item Suppose $v_1 \sim_{S} v_1'$ and $v_2 \sim v_2'$ via $w = (x_1, \ldots, x_n)$ such that there is a path from $u$ to $v_1'$ and from $u$ to $x_n = v_2'$.
                  ...
                  By the same reasoning as above it must be the case that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1)) = <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_2))$.
    ...
    Suppose $v_1, v_2$ are vertices and $v_1 \sim v_2$, then $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_1]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_2])$.
    \begin{itemize}
        \item If $[v_2) \not = \varnothing$, then, by definition we have
                \begin{align*}
                    <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_1]) &= [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_2))]\\
                     &= \text{ as $v_2 \sim v_2$ and $[v_2) \not = \varnothing$ }\\
                     &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_2])
                \end{align*}
        ...
        \begin{align*}
            <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_1]) &= [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_3))]\\
             &= \text{ as $v_3 \sim v_3$ and $[v_3) \not = \varnothing$ }\\
             &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_3])\\
             &= \text{ as $v_2 \sim v_1 \sim v_3$}\\
             &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_2])
        \end{align*}
    \end{itemize}
    % $v_1 \sim v_2$ implies that either $v_1 = v_2$ and the well-definedness is trivial, or there exists a sequence $w = (x_1, \ldots, x_n)$ such that $v_1 = x_1$, $v_2 = x_n$ and $x_i \sim_{S} x_{i \sqcup 1}$ for $i < n$.
    ...
    %                   Then, 
    %                   \[
    %                     <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)]
    %                   \]
    %                   and
    %                   \[
    %                     <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_2])) = [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)]
    %                   \]
    %                   and we have well-definedness by definition.
    %             \item $f_{V}(z) = \varnothing$ and $g_{V}(z) \not = \varnothing$ is symmetric to the above.
    %           \end{itemize}
    %     \item Assume that if $v_1 \sim v_2$ via $w = (x_1, \ldots, x_n)$ then $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_1]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_2])$.
    %     \item Now suppose $v_1 \sim v_2$ via $w = (x_1, \ldots, x_n, x_{n \sqcup 1})$.
    %           We know that 
    %           \[
    %             <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_1]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[x_n])
    %           \]
    %           and that
    %           \[
    %             <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[x_n]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_2])
    %           \]
    %           by the same argument as in the base case,
    %           which implies
    %           \[
    %             <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} \sqcup V_{Y}}{\sim}}[v_1]) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_2])
    %           \]
    % \end{itemize}
    ...
    That is, we need to check if $[v) \not = \varnothing$, then $[<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))$.
    Since $v \sim v$ this is homomorphic by definition.
    Similarly for edges.
    
    Then $<_{\sfrac{X \sqcup Y}{\sim}} : V_{\sfrac{X \sqcup Y}{\sim}}  \sqcup  E_{\sfrac{X \sqcup Y}{\sim}} \to (E_{\sfrac{X \sqcup Y}{\sim}})^{*}$ is defined as a transitive closure of $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}$.
    ...
    \begin{itemize}
        ...
        \]
        % \item if $[e] \in \consistency^{\hashtag}_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))$ then 
        % \[
        %     [v] \in (\consistency^{\hashtag}_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e])))^{c}
        % \]
    ...
    \ifdefined \ONECOLUMN
    \[
        [[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v)))
        \subseteq
        \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))
    \]
    \else
    ...
    \begin{enumerate}
        \item The first case is when $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) = \varnothing$.
              Then the property trivially holds as the empty set is a subset of any set.
        \item Suppose $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) \not = \varnothing$.
              Then, 
              \[
                \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}})(v))
              \]
              since $v \sim v$,
              and hence
              \ifdefined \ONECOLUMN
              \begin{align*}
                [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}})(v))
                &\subseteq
                \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))\\
                &\subseteq
                (\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])))^{c} = \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))\\
              \end{align*}
              \else
              \begin{align*}
                &[[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}})(v))\\
              ...
              by recalling that 
              \[
              [[]_{V}^{\consistency}, []_{E}^{\consistency}] = [[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}, []_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]
              \].
    \end{enumerate}
    ...
    % \item If there exists $v' \in V_{X \sqcup Y}$ such that $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v')) \not = \varnothing$ and $v' \sim v$, then
    % \begin{align*}
    %     &\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]_{V}))\\
    %     &= [[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v' \in V_{X}  \sqcup  V_{Y} | [v']_{V} = [v]_{V}} \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v')))
    % \end{align*}
    % \item If for all $v' \in V_{X \sqcup Y}$ such that $v' \sim v$, $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v')) = \varnothing$ and there exists $[u]$ such that there is an undirected path from $[v]$ to $[u]$ and $\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([u])) \not = \varnothing$, then
    % \[
    %     \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]_{V})) = \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{E_{X}  \sqcup  E_{Y}}([u]_{V}))
    % \]
    % \end{enumerate}
    % where $\iota_*$ are injections into $\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}  \sqcup  \sfrac{E_{X}  \sqcup  E_{Y}}{\sim}$.
    ...
    % We then define
    % \[
    %   \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = (\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])))^{c}
    % \]
    % \[
    %   \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e])) = (\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e])))^{c}  
    % \]
    ...
    %         \item  Assume that $v_1, v_2 \in V_{X \sqcup Y}$ and $v_1 \sim v_2$ then we need to show that
    %         \[
    %             \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{E_{X}  \sqcup  E_{Y}}[v_1]) = \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{E_{X}  \sqcup  E_{Y}}[v_2])
    %         \]
    %           $v_1 \sim v_2$ implies there exists $z \in V_{Z}$ such that $v_1 = f_{V};\iota_{1,V}(z)$ and $v_2 = g_{V};\iota_{2,V}(z)$ and supposing that for any of $v_i$ $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_i)) \not = \varnothing$, then
    %           \begin{align*}
    %             &\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1]))\\
    %             & = ([[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v_i \in \{v_1, v_2\}} \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_i))))^{c}
    %           \end{align*}
    %           Similarly,
    %           \begin{align*}
    %             &\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_2]))\\
    %             &= ([[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v_i \in \{v_1, v_2\}} \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_i))))^{c}
    %         \end{align*}
    ...
    %         \[\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1)) = \varnothing
    %         \quad 
    %         \text{and} 
    %         \quad\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_2)) = \varnothing
    %         \]
    %         This would mean that for all $z \in V_{Z}$, $\consistency(f_{V}(z)) = \varnothing$ and $\consistency(g_{V}(z)) = \varnothing$ and automatically
    %               \[
    %                 \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_2]))
    %               \]
    %         \end{itemize}
    %     \item Putting $\phi = ([]_{V},[]_{E})$ into the definition of a homomorphism gives us
    %         \begin{align*}
    %          &[[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}, []_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(x)))\\
    %          &\subseteq
    %          \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x]))
    %         \end{align*}
    %         By definition, 
    %         \begin{align*}
    %             &\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x])) =\\
    %              [[]_{V}^{\consistency}, []_{E}^{\consistency} ]^{*} &(\bigcup_{x' \in V_{X}  \sqcup  V_{Y} | [x']_{V} = [x]_{V}} \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(x)))
    %         \end{align*}
    %         and by recalling that $[]_V^{\consistency} = []_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}$ and similarly for $[]_{E}^{\consistency}$ we get the inclusion 
    %         \begin{align*}
    %             &[[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}, []_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(x)))\\
    %             &\subseteq \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x]))
    %         \end{align*}
    %          and the latter is the subset of $\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x]))$ because it is a closure.
    %         If $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(x)) = \varnothing$ then homomorphismness holds because empty set is a subset of any set.
    %         Similarly for edges.
    % \end{itemize}

    So far we have shown that relations $<_{\sfrac{X \sqcup Y}{\sim}}$ and $\consistency_{\sfrac{X \sqcup Y}{\sim}}$ are well-defined and $([]_{V},[]_{E})$ is a homomorphism.
    By the Proposition~\ref{prop:pushout_is_e_hypergraph} the constructed $\sfrac{X \sqcup Y}{\sim}$ is an e-hypergraph.
    ...
    $v_1 \sim v_2$ means there is a sequence $w = (x_1, \ldots, x_n)$ such that $v_1 = x_1$, $v_2 = x_n$ and for all $i < n$ $x_i \sim_{S} x_{i \sqcup 1}$. We will proceed by induction on the length of $w$.
    \begin{itemize}
        ...
        \item Suppose $v_1 \sim v_2$ via $w = (x_1, \ldots, x_n, x_{n \sqcup 1})$.
              By hypothesis, we know that $u_{V}([v_1]) = u_{V}([x_1]) = u_{V}([x_n])$ and that $u_{V}([x_n]) = u_{V}([x_{n \sqcup 1}]) = u_{V}([v_2])$ and hence $u_{V}([v_1]) = u_{V}([v_2])$.
    \end{itemize}
    ...
    \begin{enumerate}
        \item The first condition to check is if $u_{V}^{*}(s_{\sfrac{X \sqcup Y}{\sim}}([e])) = s_{Q}(u_{E}([e]))$. 
              By construction of $s_{\sfrac{X \sqcup Y}{\sim}}$ we have
              \[
                u_{V}^{*}(s_{\sfrac{X \sqcup Y}{\sim}}([e])) = u_{V}^{*}([s_{X \sqcup Y}(e)]^{*})
              \]
              Suppose $e = \iota_{E_{X}}(e')$ and then
              \begin{align*}
                u_{V}^{*}([s_{X \sqcup Y}(e)]^{*}) &= u_{V}^{*}([s_{X \sqcup Y}(\iota_{1,E}(e'))]^{*})\\
    ...
    \[
        <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([v]))) = u_{E}(<^{\mu}_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v]))
    \]
    We need to consider a few cases.
    \begin{itemize}
        \item Suppose $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(u))]$ for $u \sim v$.
              Then assume $u = \iota_{1,V}(u_{x})$ and then 
              \begin{align*}
                [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(u))] &= [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(\iota_{1,V}(u_{x})))]\\
        ...
        \item Suppose $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v']))$ such that there is a path from $[v]$ to $[v']$ and $v$ has no pre-image in $V_{Z}$.
              ...
              and by definition
              \[
                u_{E}(<^{\mu}_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v'])) = u_{E}(<^{\mu}_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v]))
              \]
    ...
    \begin{align*}
    &[u_{V};\iota_{V_{Q}}, u_{E};\iota_{E_{Q}}]^{*}(\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])))\\
    &\;\subseteq\\
    &\consistency_{Q}(u_{V};\iota_{V_{Q}}([v]))
    \end{align*}
    Recall that
    \[
      \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = (\consistency^{\hashtag}_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])))^{c}
    \]
    and first we will check if
    \begin{align*}
      &[u_{V};\iota_{V_{Q}}, u_{E};\iota_{E_{Q}}]^{*}(\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])))\\
      &\;\subseteq\\
      &\consistency_{Q}(u_{V};\iota_{V_{Q}}([v]))
    \end{align*}
    Consider cases
    \begin{itemize}
        \item If $\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = \varnothing$ then the inclusion is trivial and hence we assume that
        \[
            \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) \not = \varnothing
        \]
        \item Suppose $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(u)) \not = \varnothing$ and $v \sim u$ and then
        \[
            \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X} \sqcup V_{Y}}(u)))
        \]
        Assume $u = \iota_{1,V}(u_{x})$, then
        \ifdefined \ONECOLUMN
        \begin{align*}
            [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X} \sqcup V_{Y}}(u)))
            &=
            [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X} \sqcup V_{Y}}(\iota_{1,V}(u_{x}))))\\
            &=
        ...
        \fi
        The case when $u = \iota_{2,V}(u_{y})$ is symmetric.
        \item Suppose that $v$ has no pre-image in $V_{Z}$ and $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) = \varnothing$ and there exists $u$ such that $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(u)) \not = \varnothing$ and such that there is an undirected path from $[v]$ to $[u]$ and then
        \[
            \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([u]))    
        \]
        By the argument above it follows that
        \begin{align*}
            &[u_{V};\iota_{Q_{V}},u_{E};\iota_{Q_{E}}]^{*}(\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([u])))\\
            &\;\subseteq\\
            &\consistency_{Q}(\iota_{V_{Q}}(u_{V}([u])))
        \end{align*}
        \end{itemize}
    ...
    \fi
       ...
       and
       \begin{align*}
        &[u_{V};\iota_{V_{Q}}, u_{E};\iota_{E_{Q}}]^{*}(\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e])))\\
        &\;\subseteq\\
       ...
       \begin{itemize}
        ...
        \item $\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([u]) \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))$ then 
        \[
        ...
        \[
            \iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_i]) \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_{i \sqcup 1}]))
        \] 
        
        or 
        \[
            \iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_{i \sqcup 1}]) \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_{i}]))
        \]
        ...
        \begin{itemize}
            \item Base case is when $w = (x_1,x_2)$.
                  ...
                  \begin{align*}
                  &[u_{V};\iota_{V_{Q}},u_{E};\iota_{E_{Q}}]^{*}(\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_2])))\\
                  &\subseteq\\
                  &\consistency_{Q}(u_{V};\iota_{V_{Q}}([x_2]))
                  \end{align*}
            \item Suppose the statement is true for all $w$ such that $|w| \leq n$.
            \item Consider a sequence $w = (x_1, \ldots, x_{n}, x_{n \sqcup 1})$.
                  By assumption, we know that $u_{V};\iota_{V_{Q}}([x_1]) \in \consistency_{Q}(u_{V};\iota_{V_{Q}}([x_n]))$.
                  The fact that $\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_{n}]) \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([x_{n \sqcup 1}]))$ (or its symmetric counterpart) implies that $u_{V};\iota_{V_{Q}}([x_{n}]) \in \consistency_{Q}(u_{V};\iota_{V_{Q}}([x_{n \sqcup 1}]))$ and by the transitivity and symmetry of $\consistency_{Q}$ it follows that
                \[
                    u_{V};\iota_{V_{Q}}([x_1]) \in \consistency_{Q}(u_{V};\iota_{V_{Q}}([x_{n \sqcup 1}]))
                \]
                and
                \[
                    u_{V};\iota_{V_{Q}}([x_{n \sqcup 1}]) \in \consistency_{Q}(u_{V};\iota_{V_{Q}}([x_{1}]))
                \]
        \end{itemize}
       \end{itemize}
       Therefore, we have
...
\begin{remark}
    If in a span $Y \xleftarrow{g} Z \xrightarrow{f} X$ of morphisms in $\catname{EHyp}(\Sigma)$ the morphisms $f$ and $g$ are monos, then the arrows $f'$ and $g'$ of the pushout $Y \xrightarrow{g'} X  \sqcup _{f,g} Y \xleftarrow{f'} X$ are also monos.
\end{remark}
...
% So far we have shown that the constructed $\sfrac{X \sqcup Y}{\sim}$ is a pushout but we are yet to show that it is an e-hypergraph.
...
% By construction we have the following corollaries for $<_{\sfrac{X \sqcup Y}{\sim}}$ and $\consistency_{\sfrac{X \sqcup Y}{\sim}}$.

\begin{lemma}
\label{lemma:child_irreflexive}
    $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}$ is irreflexive.
\end{lemma}
\begin{proof}
    Suppose there exist $[e]$ such that $[e] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e]))$.
    Then we have two cases.
    \begin{itemize}
      \item $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e])) = [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e))] = [e]$
      This would mean that $<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e)) = e$ as $[]_{E}$ is injective, which would mean that the relation $<_{X \sqcup Y}^{\mu}$ is not irreflexive because $e$ is the predecessor of $e$.
      \item Suppose $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e])) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))]$ such that $<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))$ is defined and there is a path from $[e]$ to $[v]$.
            This implies
            \[
                [e] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))]
            \]
            and
            \[
               e = <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))
            ...
            \begin{itemize}
                \item If $v' = v$, then there is a path from $e$ to $v$ which contradicts $e = <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))$
                \item Consider the case when $v' \sim v$.
                      $e = <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))$ implies that both $e$ and $v$ are in the image of $(\iota_{1,V},\iota_{1,E})$ or $(\iota_{2,V},\iota_{2,E})$.
                      ...
                      \begin{align*}
                        <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) &= \iota_{1,E}(<_{X}^{\mu}(\iota_{V_{X}}(v_{x})))\\
...
\label{lemma:child_assymetric}
    $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}$ is asymmetric.
\end{lemma}
\begin{proof}
    Let $e_1,e_2$ be edges. 
    Then, if $[e_1] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_2]))$ it must not be the case that $[e_2] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))$.
    For $[e_1] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_2]))$ we have two cases.
    \begin{itemize}
      \item 
      \[
        [e_1] = [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_2))]
      \]
      which implies $e_1 = <_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_2))$.
      Having at the same time $[e_2] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))$ gives us also two cases.
      \begin{itemize}
        \item $[e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))]$ and $e_2 = <_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))$ which implies that $e_1$ is the predecessor of $e_2$ and vice versa which contradicts e-hypergraphness of $X \sqcup Y$.
        \item
            \[
                [e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))]
            \]
            ...
            \begin{itemize}
                ...
                \begin{align*}
                    <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) &= \iota_{1,E}(<_{X}^{\mu}(\iota_{V_{X}}(v_{x})))\\
                                                            ...
                                                            &= <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v'))\\
                                                            &= <_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))
                \end{align*}
                which contradicts $e_1 = <_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_2))$.
                The case when $e_2 = \iota_{2,E}(e_{2,y})$ and $v = \iota_{2,V}(v_{y})$ is symmetric.
            \end{itemize}
        \end{itemize}
            \item The other case is when          
            \[
                [e_1] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1))]
            \]
            such that there is a path from $e_2$ to $v'_1$ and $v_1 \sim v'_1$ and $[e_2) = \varnothing$.
            \begin{itemize}
                \item The case when $[e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))]$ is analogous to the last case above.
                \item Suppose 
                \[
                    [e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_2))]
                \] such that there is a path from $e_1$ to $v'_2$ and $v_2 \sim v'_2$ and $[e_1) = \varnothing$.
                    ...
                    \begin{align*}
                        e_1 &= <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1))\\
                            ...
                            &= <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v'_2))\\
                            &= <_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))
                    \end{align*}
                    And we get a contradiction to irreflexivity of $<_{X \sqcup Y}^{\mu}$.
            \end{itemize}
      \end{itemize} 
...
\label{lemma:child_respect}
    $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}$ respects connectivity.
\end{lemma}
\begin{proof}
        We need to show that if $[v] \in s_{\sfrac{X \sqcup Y}{\sim}}([e_1])$ and $[e_2] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))$ then $[e_2] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v])$.
        We have two cases.
        \begin{itemize}
            \item \[
            [e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(e_1))]
            \]
            By definition, $s_{\sfrac{X \sqcup Y}{\sim}}([e_1]) = [s_{X \sqcup Y}(e_1)]^{*}$ and $[v] \in [s_{X \sqcup Y}(e_1)]^{*}$ means there exists $v' \sim v$ such that $v' \in s_{X \sqcup Y}(e_1)$.
            \begin{itemize}
                \item $v' = v$ and then $v \in s_{X \sqcup Y}(e_1)$ and therefore $e_2 = <_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))$ and $[e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v))]$.
                \item $v' \sim v$ and then 
                \begin{align*}
                    [e_2] &= [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v'))]\\
                          &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v']))\\
                          &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))
                \end{align*}
            \end{itemize}
            \item \[
                [e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1))]    
            \]
            such that there is $v_1' \sim v_1$ and there is a path from $e_1$ to $v_1'$ and $[e_1) = \varnothing$.
            $[v] \in [s_{X \sqcup Y}(e_1)]^{*}$ means there exists $v' \sim v$ such that $v' \in s_{X \sqcup Y}(e_1)$.
            \begin{itemize}
                \item $v' = v$ and then $v \in s_{X \sqcup Y}(e_1)$. Since there is a path from $e_1$ to $v_1'$, there is also a path from $v'$ to $v_1'$ and by definition
                \[
                    <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v])) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = [e_2]
                \]
                \item $v' \sim v$ and then there is a path from $v'_1$ to $v'$. 
                Suppose $v_1 = f_{V};\iota_{1,V}(z_1)$ and since $v_1' \in s_{X \sqcup Y}(e_1)$, $[e_1) = \varnothing$, both $v_1'$ and $v'$ should be in the image of $g_{V};\iota_{2,V}$.
                ...
                \begin{align*}
                    [e_2] &= [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1))]\\
                          ...
                          &= [<_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(f_{V};\iota_{1,V}(z'')))]\\
                          &= [<_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v''))]\\
                          &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v'']))\\
                          &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))
                \end{align*}
            ...
            \end{itemize}
            \end{itemize}
            \item Finally, we need to check that if $[v] \in s([e_1])$ and $[e_2] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))$ then $[e_2] = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))$.
            Once again, $[v] \in s_{\sfrac{X \sqcup Y}{\sim}}([e_1])$ means that there exists $v' \in s_{X \sqcup Y}(e_1)$ such that $v' \sim v$.
            \begin{itemize}
                \item \[
                [e_2] = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v''))]
                \] such that $v'' \sim v$ and $[v'') \not = \varnothing$.
                \begin{itemize}
                    \item If $v' = v$ and $v'' = v$ then $v \in s_{X \sqcup Y}(e_1)$ and since $[v'') = [v) \not = \varnothing$ and $v \in s_{X \sqcup Y}(e_1)$, it must be the case that
                    $<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1)) = <_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(v))$ and further
                    \begin{align*}
                    <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1])) &= [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))]\\
                                                                                            &= [<_{X \sqcup Y}^{\mu}(\iota_{E_{X}  \sqcup  E_{Y}}(v))]\\
                                                                                            &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))\\
                                                                                            &= [e_2]
                    \end{align*}
                    \item If $v' = v $ and $v'' \sim v$ then $v \in s_{X \sqcup Y}(e_1)$.
                          ...
                          \begin{itemize}
                            ...
                            \item $v = g_{V};\iota_{2,V}(z)$, then $g_{V}(z) = \varnothing$ and $[e_1) = \varnothing$.
                                  ...
                                  \begin{align*}
                                    <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1])) &= <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v'']))\\
                                    &= [e_2]
                                  \end{align*}
                          \end{itemize}
                          Case when $v'' = g_{V};\iota_{2,V}(z'')$ is symmetric.
                    \item If $v' \sim v$ and $v'' = v$ then $v' \in s_{X \sqcup Y}(e_1)$. This implies that $[v) \not = \varnothing$
                          \begin{itemize}
                    ...
                    \item If $v' \sim v$ and $v'' \sim v$ then $v' \in s_{X \sqcup Y}{e_1}$, $[v'') \not = \varnothing$ and $v'' \sim v'$.
                    \begin{itemize}
...
If
\[
\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_1])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_2]))
\]
where $\iota_{\sfrac{*}{\sim}}$ is either $\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}$ or $\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}$ depending on whether $[x_i]$ is in $V_{X}  \sqcup  V_{Y}$ or $E_{X}  \sqcup  E_{Y}$.
Then
\[
\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_1])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_2]))
\]
...
\begin{proof}
    ...
    Apart from $\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_1]))$ the set on the left also contains
    \begin{itemize}
        \item $[x_1]$ as per the reflexivity.
        So we need to check if $[x_1] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_2]))$.
              Because 
              \[
              \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_1])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_2]))
              \]
              ...
              \[
              [y] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_1]))
              \]
               and
            \[
                [y] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_2]))
            \]
            that is, we have a sequence $([x_1],[y],[x_2])$ and by transitivity
               \[
               [x_1] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_2]))
               \]
                and
            \[
            [x_2] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_1]))
            \]
        \item $[y]$ for all $[y]$ such that $[x_1] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([y]))$.
              Let's check if $[y] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_2]))$.
              As above, there exists $[z]$ such that
              \[
              [z] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_1]))
              \]
               and
              \[
                [z] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_2]))
              \]
              By a sequence $([y],[x_1],[z],[x_2])$ we get that
              \[
                [y] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_2]))
              \]
              Similarly for $[y]$ such that $[x_2] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([y]))$.
        \item For any sequence $([x_1],\ldots,[x_n])$ such that $[x_i] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_{i \sqcup 1}]))$ or $[x_{i \sqcup 1}] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_{i}]))$ for $i < n$ the set on the left contains $[x_n]$.
              Let's check if $[x_n] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_2]))$.
              Since there exists $[y]$ such that $[y] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_1]))$ and $[y] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x_2]))$ we can extend the sequence to $([x_2],[y],[x_1],\ldots,[x_n])$ and hence $[x_n] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_2]))$.
              Similarly, given a sequence $([x_2],\ldots, [x_n])$ and extending it to $([x_1],[y],[x_2],\ldots,[x_n])$ we get $[x_n] \in \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x_1]))$.
    \end{itemize}
\end{proof}
...
$\consistency_{\sfrac{X \sqcup Y}{\sim}}$ respects connectivity.
\end{lemma}
\begin{proof}
    Suppose $[v_1] \in s_{\sfrac{X \sqcup Y}{\sim}}([e_1])$ and $[[v_1]) = [[e_1]) \not = \varnothing$, then it must be the case that 
    \[
    \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))
    \]
    By the lemma above, to prove this, it is sufficient to show that

    \[
    \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))
    \]

    $[v_1] \in s_{\sfrac{X \sqcup Y}{\sim}}([e_1])$ means there exists $v_1' \in s_{X \sqcup Y}(e_1)$ such that $v_1 \sim v_1'$.
    We then have two cases.
    \begin{itemize}
      \item $v_1 = v_1'$ and then $v_1 \in s_{X \sqcup Y}(e_1)$.
      \begin{itemize}
         \item $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1''))]$ such that $v_1'' \sim v_1$ and $[v_1'') \not = \varnothing$.
            \begin{itemize}
                \item $v_1'' = v_1$ and then $[v_1) \not = \varnothing$ and $[e_1) \not = \varnothing$.
                      Furthermore, necessarily $\consistency_{X \sqcup Y}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1)) = \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1))$.
                      By applying $([]_{V},[]_{E})$ we get
                      \ifdefined \ONECOLUMN
                      \begin{align*}
                        \text{by functionality}\\
                          [[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]^{*}(\consistency_{X \sqcup Y}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1))) &=
                          [[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1)))\\
                          \text{by unfolding the definition}\\
                          \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1])) &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1]))\\
                          \end{align*} 
                      \else
                      \begin{align*}
                      \text{by functionality}\\
                        ...
                        [[]_{V};\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1))) &\\
                        \text{by unfolding the definition}\\
                        \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1])) &=\\
                        \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1]))&
                        \end{align*} 
                      \fi
                \item $v_1'' \sim v_1$. If $v_1'' = f_{V};\iota_{1,V}(z_1'')$ and $v_1 = f_{V};\iota_{1,V}(z_1)$ we get
                    \begin{align*}
                        \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}&(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) =\\
                                                                                                                &= [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v'')))\\
                                                                                                                &= [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(f_{V};\iota_{1,V}(z_1''))))\\
                                                                                                                &= [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(f_{V};\iota_{1,V}(z_1))))\\
                                                                                                                &= [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X \sqcup Y}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1)))\\
                                                                                                                &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))
                    \end{align*}
                    If $v_1'' = f_{V};\iota_{1,V}(z_1'')$ and $v_1 = g_{V};\iota_{1,V}(z_1)$ then $[v_1) = [e_1) = \varnothing$ and 
                    \[
                        \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1)) = \consistency_{X \sqcup Y}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1)) = \varnothing
                    \]
                    \begin{align*}
                    \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1])) &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1'']))\\
                                                                                                            &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1]))
                    \end{align*}
                \end{itemize}
                \item $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_2]))$ such that $[v_2) \not = \varnothing$ and there is a path from $[v_1]$ to $[v_2]$. 
                By definition, this implies $[v_1) = \varnothing$ and $[e_1) = \varnothing$.
                Again, by definition, we have
                \begin{align*}
                    \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_2]))\\
                                                                                                    &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))
                \end{align*}
                \end{itemize}
                \item $v_1 \sim v_1'$ and then $v_1' \in s_{X \sqcup Y}(e_1)$.
                \begin{itemize}
                    \item $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1])) = [<_{X \sqcup Y}^{\mu}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1''))]$ such that $v_1 \sim v_1''$ and $[v_1'') \not = \varnothing$.
                    \begin{itemize}
                        \item $v_1 = v_1''$ and then $[v_1) \not = \varnothing$.
                            ...
                            \begin{align*}
                              \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}&(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_1']) =\\
                              &\;= [[]_{V}^{\hashtag},[]_{E}^{\hashtag}]^{*}(\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1')))\\
                                                                                                                     &\;= [[]_{V}^{\hashtag},[]_{E}^{\hashtag}]^{*}(\consistency_{X \sqcup Y}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1)))\\
                                                                                                                     &\;= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1]))\\
                                                                                                                     &\;= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_1])
                            \end{align*}
                        ...
                        \item $v_1 \sim v_1''$ and then $v_1' \sim v_1''$. Suppose $v_1'' = f_{V};\iota_{1,V}(z_1'')$.
                              ...
                              \begin{align*}
                                \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1')) &= \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1''))\\ 
                                                                               &= \consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(e_1'))
                              \end{align*}
                              and
                              \begin{align*}
                                \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_1]) &=
                                \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}[v_1'])\\ &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}[e_1])
                              \end{align*}
                              If $v_1'$ is in the image of $g_{V};\iota_{2,V}$, then $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v_1')) = \consistency_{X \sqcup Y}(\iota_{E_{X}  \sqcup  E_{Y}}(e_1)) = \varnothing$ and by definition as there is a path from $[e_1]$ to $[v_1'']$
                              \begin{align*}
                                \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}&(\iota_{\sfrac{E_{X}  \sqcup  E_{Y}}{\sim}}([e_1])) =\\
                                &\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1'']))\\ 
                                                                                                                        &= \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v_1]))
...
\label{lemma:consistency_child}
    $\consistency_{\sfrac{X \sqcup Y}{\sim}}$ is defined only for vertices and edges that are not top-level.
\end{lemma}
\begin{proof}
  $\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x])) \not = \varnothing$ if and only if $<_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{*}{\sim}}([x]))$ is defined.
  Let's first show from left to right. $\consistency_{\sfrac{X \sqcup Y}{\sim}}(\iota_{\sfrac{*}{\sim}}([x])) \not = \varnothing$ implies $\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x])) \not = \varnothing$.
  \begin{itemize}
    \item $\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x])) = [[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\iota_{*}{\sim}(x'))$ such that $x \sim x'$.
          By e-hypergraph-ness of $X \sqcup Y$ this implies that $[x') \not = \varnothing$ and hence $<_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x'])) = <_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x])) = [<_{X \sqcup Y}^{\mu}(\iota_{*}(x))]$
    \item $\consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{*}{\sim}}([x])) = \consistency_{\sfrac{X \sqcup Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X}  \sqcup  V_{Y}}{\sim}}([v]))$ such that there is a path from $[x]$ to $[v]$ and
          $\consistency_{X \sqcup Y}(\iota_{V_{X}  \sqcup  V_{Y}}(v)) \not = \varnothing$. This implies that $[v) \not = \varnothing$ and by definition
          \[
            <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{*}{\sim}}([x])) = <_{\sfrac{X \sqcup Y}{\sim}}^{\mu}(\iota_{\sfrac{*}{\sim}}([v]))  
          \]
  \end{itemize}
...
\begin{lemma}
$\consistency_{\sfrac{X \sqcup Y}{\sim}}^{p}$ is not total.
\end{lemma}
\begin{proof}
...
\begin{proposition}[$\sfrac{X \sqcup Y}{\sim}$ is an e-hypergraph]
\label{prop:pushout_is_e_hypergraph}
For this we need to check that the defined relations --- $<_{\sfrac{X \sqcup Y}{\sim}}$ and $\consistency_{\sfrac{X \sqcup Y}{\sim}}$ --- satisfy all the needed properties as per Definition~\ref{def:e-homo}.
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof}
        The pushout in~\ref{def:boundary_new} satisfies the assumptions in Definition~\ref{pushout:assumptions} (note that since $i \to i' \to \mathcal{L} \xleftarrow{} j' \xleftarrow{} j$ is an MDA cospan, the image of $i  \sqcup  j$ in $\mathcal{L}$ consists of top-level vertices only) and therefore is isomorphic to the pushout constructed in~\ref{pushout:assumptions} and by construction should also be a pushout on the underlying sets of vertices and hyperedges.
    ...
    %     \[
    %         \begin{tikzcd}
    %             & \mathcal{L} \arrow[d] \arrow[ldd, bend right] & i \sqcup j \arrow[l, "h"'] \arrow[d, "c"]           &                 & V_{\mathcal{L}} \arrow[d] \arrow[ldd, bend right] & i \sqcup j \arrow[l, "h_{V}"] \arrow[d, "c_{V}"]      \\
    ...
    %     Suppose there exist $i \sqcup j \to V_{\mathcal{L}} \to V_{Q} = i \sqcup j \to V_{\mathcal{C}} \to V_{Q}$ and we need to check that there exists a unique $u_{V}$ such that these morphisms factors out through $u_{V}$.
    ...
    %     \begin{itemize}
    %         \item $V = V_{Q}$
    %         \item $E = \sfrac{E_{\mathcal{L}}  \sqcup  E_{\mathcal{C}}}{\mathcal{R}}$ where $e_1 \mathcal{R} e_2$ if there exists $v \in V_{Q}$ such that $v = i_{V}(v_1)$ and $v = i_{V}(v_2)$ and $e_1 = <^{\mu}_{\mathcal{L}}(\iota_{V_{\mathcal{L}}}(v_1)), e_2 = <^{\mu}_{\mathcal{C}}(\iota_{V_{\mathcal{C}}}(v_2))$
    %     \end{itemize}
        In particular,
    \[
    ...
    % Because $i \sqcup j$ is discrete, $E_{\mathcal{G}}$ is the disjoint union of $E_{\mathcal{L}}$ and $E_{\mathcal{L}^{\bot}}$ and hence $E_{\mathcal{L}^{\bot}} = E_{\mathcal{G}} \setminus E_{\mathcal{L}}$.
...
\[\begin{tikzcd}
	{a  \sqcup  y} && {i  \sqcup  j  \sqcup  x} && {i \sqcup j} \\
	\\
	{a  \sqcup  y  \sqcup  z} &&&& {\begin{matrix} i  \sqcup  j  \sqcup  w \\ \cong \\ i  \sqcup  j  \sqcup  z \end{matrix}}
...
\end{tikzcd}\]
    Where $y$ is the image of $i  \sqcup  j$. Clearly, $i  \sqcup  j  \sqcup  z$ is a pushout complement, as computing the pushout identifies $i  \sqcup  j$ with its image within $y$ and leaves $z$ as is.
    Suppose there is another pushout complement $i  \sqcup  j  \sqcup  w$.
    Because $a  \sqcup  y  \sqcup  w$ also yields a pushout it must be the case that $a  \sqcup  y  \sqcup  w \cong a  \sqcup  y  \sqcup  z$ and therefore $w \cong z$ and pushout complement for vertices is $i  \sqcup  j  \sqcup  z$ up to isomorphism.
    ...
    One can observe that $g_{V} = h_{V,ext};h_{V,int}  \sqcup  id_{n}$ and that $h_{V,ext}$ is mono and $h_{V,int} = [h_1,h_2]$ where $h_1,h_2$ are mono.
    ...
    $(h_{V,ext};[h_{1},h_{2}]  \sqcup  id_{n})(s_{1}(e)) = (h_{V,ext};[h_{1},h_{2}]  \sqcup  id_{n})(s_{2}(e))$.
    ...
    % Since $g_{V} = h_{V,ext};[h_1,h_2]  \sqcup  id_{n}$ the inverse image of $g_{V}$ contains at most two elements: one with the pre-image in $i$ and another with the pre-image in $j$.
    ...
    This implies that both $v''$ and $v'$ have a pre-image in $i  \sqcup  j$ and according to the definition of a boundary complement it must be $[[c_1,c_2](z_1)) = [[c_1,c_2](z_2))$ for all $z_1, z_2 \in V_{i  \sqcup  j}$.
    For the same reason there is also a morphism from $\mathcal{L}$ to $\mathcal{G}'$.
    The only case when such a morphism may not exist is when $v'' \in V_{\mathcal{L}}$ such that $[v'') \not = \varnothing$ is mapped to $m_{V}(v'') = g_{V}(v')$ such that $[v') = \varnothing$ which implies $v'$ and $v''$ share a pre-image in $i  \sqcup  j$.
    This leads to a contradiction as the image of $i  \sqcup  j$ in $\mathcal{L}$ should consist of top-level vertices exclusively.
    This construction results in a commutative diagram below
    \[
        \begin{tikzcd}
            & \mathcal{L} \arrow[d, "m"'] \arrow[ldd, "m'"', bend right=49] & i' \sqcup j' \arrow[l] & i \sqcup j \arrow[d] \arrow[l]                                           \\
...
\end{tikzcd}
    ...
    \begin{itemize}
        ...
        \item Suppose that $\consistency_{\mathcal{G}}(g_{V};\iota_{V_{\mathcal{G}}}(v)) \not = \varnothing$ which further means that $[g_{V};\iota_{V_{\mathcal{G}}}(v)) \not = \varnothing$.
              ...
              This implies $v, v'$ have a pre-image in $i  \sqcup  j$ and by definition it must be $v' \consistency v$.
              ...
              It implies $v_1, v_1'$ have pre-image in $i  \sqcup  j$ and by definition of a boundary-complement the image of $i  \sqcup  j$ should consist of top-level vertices exclusively.
...
% \begin{remark}
%     Consider morphisms $i  \sqcup  j \xrightarrow{[f_{ext},g_{ext}];[f_{int},g_{int}]} \mathcal{L} \xrightarrow{m} \mathcal{G}$.
%     If both of these morphisms are mono, then the boundary complement exists. 
%     Intuitively, it exists for the same reason the corresponding boundary complement for plain hypergraphs exists: it is constructed by removing from $\mathcal{G}$ everything that has no pre-image in $i  \sqcup  j$.
% \end{remark}
...
\begin{remark}
    ...
    \[
    \mathcal{F}_1 \; +\; \ldots\; +\; \mathcal{F}_{k}~.
    \]    
\end{remark}
...
\begin{proof}
    ...
    \begin{itemize}
        \item If $\mathcal{C}$ is empty, then either $f = g$ syntactically and $\llbracket f \rrbracket = \llbracket g \rrbracket$ or $f = l = r = g$ for some $l = r$ or $r = l$.
              ...
              \adjustbox{scale=0.8}{
                \begin{tikzcd}
                    {\mathcal{L}} & {i' \sqcup j'} & {i \sqcup j} & {i'' \sqcup j''} & {\mathcal{R}} \\
                    \\
              ...
              \]
              \fi
              where $i \to i' \to \mathcal{L} \xleftarrow{} j' \xleftarrow{} j = \llbracket l \rrbracket = \llbracket f \rrbracket$ and $i \to i'' \to \mathcal{R} \xleftarrow{} j'' \xleftarrow{} j = \llbracket r \rrbracket = \llbracket g \rrbracket$ and $i  \sqcup  j \to i  \sqcup  j \to \mathcal{L}$ is a boundary complement as per~\ref{def:boundary_new}.
              ...
              \begin{tikzcd}
                ...
                & {n_1' \sqcup m_1'} & {n_1 \sqcup m_1} & {n_1'' \sqcup m_1''} \\
                {\llbracket f \rrbracket} && {\mathcal{L}_2^{\bot}} && {\llbracket g \rrbracket} \\
                & {n_2'  \sqcup  m_2'} && {n_2'' \sqcup m_2''} \\
                && {n_2  \sqcup  m_2}
                \arrow[from=1-1, to=3-1]
                \arrow[from=1-2, to=1-1]
                \arrow[from=1-3, to=1-2]
            ...
            \]
            \fi
            Clearly the occurrence of $\llbracket \mathcal{C}[l] \rrbracket$ in $\llbracket f \rrbracket$ is convex and down-closed and by following the argument in Theorem 35~\cite{bonchi_string_2022-2}, there exists $n_1  \sqcup  m_1 \xrightarrow{} n_1  \sqcup  m_1 \xrightarrow{} \mathcal{L}_2^{\bot} \xleftarrow{} n_3  \sqcup  m_3 \xleftarrow{} n_2  \sqcup  m_2$ such that
            \ifdefined \ONECOLUMN
            \begin{align*}
                \llbracket f \rrbracket &= \;
                (0 \to 0 \to \llbracket C[l] \rrbracket \xleftarrow{} n_1'  \sqcup  m_1' \xleftarrow{} n_1  \sqcup  m_1)
                ;
                (n_1  \sqcup  m_1 \xrightarrow{} n_1  \sqcup  m_1 \xrightarrow{} \mathcal{L}_2^{\bot} \xleftarrow{} n_3  \sqcup  m_3 \xleftarrow{} n_2  \sqcup  m_2)\\
                \llbracket g \rrbracket &= \;
                (0 \to 0 \to \llbracket C[r] \rrbracket \xleftarrow{} n_1'  \sqcup  m_1' \xleftarrow{} n_1  \sqcup  m_1)
                ;
                (n_1  \sqcup  m_1 \xrightarrow{} n_1  \sqcup  m_1 \xrightarrow{} \mathcal{L}_2^{\bot} \xleftarrow{} n_3  \sqcup  m_3 \xleftarrow{} n_2  \sqcup  m_2)
                \end{align*}
            \else
            ...
            \fi
            and
            % $n'_2  \sqcup  m'_2 = n_3  \sqcup  m_3  \sqcup  (n_1'  \sqcup  m_1' \setminus (n_1  \sqcup  m_1))$
            \[
                n_2' = n_3  \sqcup  (n_1' \setminus n_1) \qquad m_2' = m_3  \sqcup  (m_1' \setminus m_1)
            \]
            and
            % fix overflowing
            % \[
            % n_2  \sqcup  m_1 \xrightarrow{} n_2' \setminus (n_1' \setminus n_1)  \sqcup  m_1 \xrightarrow{} \mathcal{L}_{2}^{\bot} \xleftarrow{} m'_2 \setminus (m_1' \setminus m_1)  \sqcup  n_1 \xleftarrow{} m_2  \sqcup  n_1    
            % \]
            \tikzcdset{row sep/mysizerow/.initial=0.3ex}
            \tikzcdset{column sep/mysizecol/.initial=0.1ex}
            \[\
\begin{tikzcd}[cramped, row sep=mysizerow, column sep = mysizecol]
                n_2  \sqcup  m_1\hspace{-2em} &&&&&&&& \hspace{-2em}m_2  \sqcup  n_1\\
                & \hspace{-2em} n_2' \setminus (n_1' \setminus n_1)  \sqcup  m_1 &\; & & \mathcal{L}_{2}^{\bot} &\; && m'_2 \setminus (m_1' \setminus m_1)  \sqcup  n_1\hspace{-2em} &\\
...
\]
            is an MDA cospan.
            By denoting the pushout of $\mathcal{L}_1^{\bot} \xleftarrow{} n_1  \sqcup  m_1 \xrightarrow{} \mathcal{L}_{2}^{\bot}$ with $\mathcal{L}_3^{\bot}$,
            one can observe that $\llbracket f \rrbracket$ is the pushout object of $\mathcal{L} \xleftarrow{} i \sqcup j \xrightarrow{} \mathcal{L}_{3}^{\bot}$ as per the diagram below
            \[\begin{tikzcd}
                && {n_1 \sqcup m_1} && {\mathcal{L}^{\bot}_2} \\
                \\
                {i \sqcup j} && {\mathcal{L}^{\bot}_1} && {\mathcal{L}^{\bot}_3} \\
                \\
            ...
            The corestriction of $n_1  \sqcup  m_1 \xrightarrow{[f_1,f_2]} \mathcal{L}_1^{\bot}$ to the image of $i  \sqcup  j \xrightarrow{[g_1,g_2]} \mathcal{L}_{1}^{\bot}$ is mono: suppose it is not mono, then there exist $z_1$, $z_2$, $z_3$ such that $f_1(z_1) = f_2(z_2) = g_1(z_3)$, then $[f_2,g_1]$ would not be mono, similarly if $f_1(z_1) = f_2(z_2) = g_2(z_3)$.\
            By construction of the pushout the restriction of $\mathcal{L}_{1}^{\bot} \to \mathcal{L}_{3}^{\bot}$ is mono and hence the arrow $i  \sqcup  j \to \mathcal{L}_{3}^{\bot}$ is mono.
            ...
            \else
                ...
                % \textcolor{red}{Consider cases when $\mathcal{L}$ has no-input and no-output sub-hypergraph and when it does not. Basically, repeat the above reasoning here as well, but say that if $n \sqcup m$ is empty, then there is no pushout.}
                \ifdefined \ONECOLUMN
                \[\adjustbox{width=0.95\linewidth}{
                    \begin{tikzcd}
                    {\mathcal{L}} & {i' \sqcup j'} & {i \sqcup j} & {i'' \sqcup j''} & {\mathcal{R}} \\
                    \\
                    {\llbracket f_1 \rrbracket \;  + \; \ldots  +  \; \llbracket\mathcal{C}[l]\rrbracket \;  +  \; \ldots \;  +  \; \llbracket f_n \rrbracket} && {\llbracket f_1 \rrbracket \;  +  \; \ldots \;  +  \; \mathcal{L}_{1}^{\bot} \;  +  \; \ldots \;  +  \; \llbracket f_n \rrbracket} && {\llbracket f_1 \rrbracket \;  +  \; \ldots \;  +  \; \llbracket\mathcal{C}[r]\rrbracket \;  +  \; \ldots \;  +  \; \llbracket f_n \rrbracket} \\
                    & {n_1'  \sqcup  m_1'} && {n_1''  \sqcup  m_1''} \\
                    && {n  \sqcup  m}
                    \arrow[from=1-1, to=3-1]
                    \arrow[from=1-2, to=1-1]
                    \arrow[from=1-3, to=1-2]
...
\begin{align*}
    ...
    n \xrightarrow{} n_{1}' \xrightarrow{} \mathcal{C}_{1} \xleftarrow{} m_{1}'  \sqcup  i  \sqcup  k \xleftarrow{} i  \sqcup  k; \qquad\qquad\qquad &\otimes \qquad\qquad\qquad ; j  \sqcup  k \xrightarrow{} j  \sqcup  k  \sqcup  n'_{2} \xrightarrow{} \mathcal{C}_{2} \xleftarrow{} m'_{2} \xleftarrow{} m\\
...
\begin{itemize}
	\item Suppose the image of $\mathcal{L}$ in $\mathcal{F}$ is top-level.
		...
		\begin{align*}
		&\llbracket c_{1} \rrbracket = n \xrightarrow{} n_{1}' \xrightarrow{} \mathcal{C}_{1} \xleftarrow{} m_{1}'  \sqcup  i  \sqcup  k \xleftarrow{} i  \sqcup  k\\
        &\llbracket c_{2} \rrbracket = j  \sqcup  k \xrightarrow{} j  \sqcup  k  \sqcup  n'_{2} \xrightarrow{} \mathcal{C}_{2} \xleftarrow{} m'_{2} \xleftarrow{} m
        \end{align*}
...
\end{itemize}
    ...
    % Then necessarily vertices in $\mathcal{C}$ in the image of $i \sqcup j$ share the same set of parents and are pairwise consistent as they do so in $\mathcal{G}$.
    % The same is true for the image of $i  \sqcup  j$ in $\mathcal{R}$ and therefore the necessary pushout exists which is $\mathcal{C}$ with $\mathcal{R} = \llbracket r \rrbracket$ glued in and it is equal to $\llbracket h \rrbracket$.
...
\begin{proof}
    ...
    \begin{align*}
        n \to n_{1}' \to &\mathcal{C}_{1} \xleftarrow{} m_{1}'  \sqcup  k  \sqcup  i \xleftarrow{} k  \sqcup  i\\
        i \to i' \to &\mathcal{L} \xleftarrow{} j' \xleftarrow{} j\\
        k  \sqcup  j \to n_{2}'  \sqcup  k  \sqcup  j \to &\mathcal{C}_{2} \xleftarrow{} m_{2}' \xleftarrow{} m\\
    \end{align*}
    and $n \xrightarrow{f} n' \xrightarrow{f'} \mathcal{G} \xleftarrow{g'} m' \xleftarrow{g} m$ is the co-limit of the following diagram
    % \[
    %     n \to n_{1}' \to \mathcal{C}_{1} \xleftarrow{} m_{1}'  \sqcup  k \xleftarrow{} i  \sqcup  k \to i'  \sqcup  k \to \mathcal{L} \xleftarrow{} k  \sqcup  j' \xleftarrow{} k  \sqcup  j \xrightarrow{} n_{2}'  \sqcup  k  \sqcup  j \to \mathcal{C}_{2} \xleftarrow{} m_{2}' \xleftarrow{} m
    % \]
    \ifdefined\ONECOLUMN
    \[
    n \xrightarrow{} n'_{1} \xrightarrow{} \mathcal{C}_{1} \xleftarrow{} m'_{1}  \sqcup  i  \sqcup  k \xleftarrow{} i \sqcup k \xrightarrow{} i'  \sqcup  k \xrightarrow{} \mathcal{L} \xleftarrow{} k  \sqcup  j' \xleftarrow{} k  \sqcup  j \xrightarrow{} n'_2  \sqcup  k  \sqcup  j \xrightarrow{} \mathcal{C}_{2} \xleftarrow{} m'_2 \xleftarrow{} m
    \]
    \else
    \[
    \adjustbox{width=\linewidth}{
    \begin{tikzcd}
        n \arrow[r] & n'_{1} \arrow[r] & \mathcal{C}_{1} & m'_{1}  \sqcup  i  \sqcup  k \arrow[l]     & i \sqcup k \arrow[l] \arrow[r] & i' \sqcup k \arrow[rd] &             \\
                    &                  &                 &                          &                         &                 & \mathcal{L} \\
        m \arrow[r] & m'_{2} \arrow[r] & \mathcal{C}_{2} & n'_{2}  \sqcup  k  \sqcup  j \arrow[l] & k \sqcup j \arrow[r] \arrow[l] & k \sqcup j' \arrow[ru] &            
    \end{tikzcd}
    }
    \]
    \fi
...
%     \[
%         f = f_1  \sqcup  \ldots  \sqcup  f_n
%     \]
%      such that each $f_i$ contains no hierarchical edges,  and for all $i \neq j$ we have $f_i \neq f_j$.
% \end{proposition}
% \begin{proof}
%     We will define the depth of an edge $e$ as the number of parents it has, \textit{i.e.}, by $|[e)|$.
%     Then we will build the required weak normal form in steps and first will build a form $f = f_1  \sqcup  \ldots  \sqcup  f_n$ such that every of $f_i$ is also of the form $g_1  \sqcup  \ldots  \sqcup  g_k$ and so on and we will assume that $f$ has at least one hierarchical edge (otherwise is is already a weak normal form).
...
%     Because we do not increase the maximum depth in $f$ we will eventually reach a maximum value of $k$ and stop because there will be no hierarchical edges of depth $max(k)$ (otherwise their children would have depth $max(k)  \sqcup  1$).
...
%     This brings us to a weak normal form $f = f_1  \sqcup  \ldots  \sqcup  f_n$ where each of $f_i$ does not contain any hierarchical edges and for $i \not = j$, $f_i \not = f_j$, the latter is guaranteed by rule (8).
% \end{proof}
...
See Figures~\ref{fig:e-graph-example-a-b},\ref{fig:e-graph-example-b-c}.
...
% NOTE This definition means that tensor identity is the unit for the semilattice, which is not necessarily the case
% \begin{definition}
