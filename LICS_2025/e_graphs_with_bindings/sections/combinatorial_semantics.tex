...
We then evoke the definition of hypergraphs and corresponding cospans as introduced in~\cite{bonchi_string_2022-1,bonchi_string_2022-2} over not uni-typed signatures.
...
\begin{enumerate}
    ...
    \item $l_{V,\mathcal{F}}(v) = l_{V,\mathcal{G}}(\phi_V(v))$
    \item $l_{E,\mathcal{F}}(e) = l_{E,\mathcal{G}}(\phi_E(e))$
\end{enumerate}
That is, a homomorphism should preserve sources and targets of edges and preserve the labels.
...
that is computed by identifying vertices in $\mathcal{F} + \mathcal{G}$ that share pre-image in $m$.
...
To make such cospans complete with respect to representing morphism of $\catname{S}(\Sigma)$, we need to impose some restriction in their form.
...
The semilattice enrichment structure was encoded using hierarchical edges in a hypergraph following the approach of a well-established area of hierarchical hypergraphs~\cite{plump:hierarchical-graphs, montanari:gs-lambda, palacz:hierarchical-transform, Gaducci:hierarchical-graphs, Ghica:hierarchical, fscd}.
The latter work in \textit{loc. cit.}, in particular, introduced a combinatorial representation of closed $\Sigma$-terms (without enrichment).
...
We next extend the definition of e-hypergraphs for $\catname{SLat}$-enriched symmetric monoidal categories~\cite{ghica2024equivalencehypergraphsegraphsmonoidal} to accommodate new structure brought by closedness and when evoking a term e-hypergraph we will mean the notion introduced in what follows rather than what was defined in \textit{loc. cit.}.
...
\end{itemize} 
$\textcolor{red}{E}$ and $\textcolor{blue}{E}$ are then induced by the corresponding relations and consist of exclusively hierarchical edges; then we have $\colorbox{yellow}{E} = E \setminus \textcolor{red}{E} \cup \textcolor{blue}{E}$.
Given an e-hypergraph $\mathcal{G}$ we can consider a corresponding \textit{underlying} hypergraph $\mathcal{G}'$ by forgetting $\textcolor{red}{<}, \textcolor{blue}{<}$ and~$\consistency$.
\end{definition}
...
$\textcolor{blue}{<}$ defines closed monoidal structure as defined in~\cite{fscd} and $(\textcolor{red}{<},\consistency)$ defines $\catname{SLat}$ structure as per~\cite{ghica2024equivalencehypergraphsegraphsmonoidal}.
