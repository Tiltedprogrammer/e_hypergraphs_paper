...
To make such cospans complete with respect to representing morphisms of $\catname{S}(\Sigma)$, we need to impose some restrictions on their form.
...
The practical meaning of the above theorem is that two $\Sigma$-terms $f$ and $g$ are equal modulo SMC equations if and only if they are represented as the same hypergraph with interfaces.
The theorem defines a full and faithful functor that is induced by the interpretation function of morphism terms that renders the latter as cospans of hypergraphs and is given in Appendix~\ref{sec:appendix:interpretation}.
...
A typical DPO square is depicted in Fig.~\ref{fig:dpo_dpoi}~(left).
...
To express the rewriting of (hyper-)graphs with interfaces,  \textit{i.e.},  the ones that represent morphisms in $\HypI{\Sigma}$, the DPO formalism has been extended to double-pushout rewriting with \textit{interfaces}  (DPOI) as shown in Fig.~\ref{fig:dpo_dpoi}~(right).
...
Notably, the laws of a semilattice enrichment in~\cite{tiurin2025equivalencehypergraphsdporewriting} and the laws of Definition~\ref{def:closed} in~\cite{fscd} were not absorbed by the combinatorial representation and instead were implemented on top as structural DPOI rewrites.

We follow an analogous approach by combining combinatorial representations of~\cite{fscd} and~\cite{tiurin2025equivalencehypergraphsdporewriting}.
We next extend the definition of e-hypergraphs for $\catname{SLat}$-enriched symmetric monoidal categories~\cite{tiurin2025equivalencehypergraphsdporewriting} to accommodate new structure brought by closedness, and when evoking a term `e-hypergraph' we will mean the structure defined as follows.


\begin{definition}
We define an e-hypergraph $\mathcal{G}$ over a closed symmetric monoidal signature $\Sigma = (\Sigma_{O}, \Sigma_{M})$ as a tuple $(V,E,s,t, l_{V}, l_{E} \textcolor{e-color}{<}, \textcolor{closed-color}{<},\consistency)$ where
\begin{itemize}
  \item $V$ is a set of vertices.
  \item $E = \colorbox{yellow}{E} \cup \textcolor{e-color}{E} \cup \textcolor{closed-color}{E}$ is a set of hyperedges that is formed of three disjoint sets of hyperedges defined below.
  ...
  \item $\textcolor{e-color}{<} \subseteq \textcolor{e-color}{E} \times V + E$ and $\textcolor{closed-color}{<} \subseteq \textcolor{closed-color}{E} \times V + E $ with $\textcolor{e-color}{E}, \textcolor{closed-color} \subset E$ are disjoint partial orders that are defined as a transitive closure of corresponding immediate predecessor relations that we denote as $\textcolor{e-color}{<^{\mu}}$ and $\textcolor{closed-color}{<^{\mu}}$.
        For $x \in V + E$, the set of predecessors of $x$ is denoted as $[x) = \{x' ~|~ \exists y_{1} = x', \ldots, y_{n} = x,  y_{i} \textcolor{e-color}{<^{\mu}} y_{i + 1} \text{ or } y_{i} \textcolor{closed-color}{<^{\mu}} y_{i+1}\; \}$.
        ...
        \begin{enumerate}
          ...
          \item edges $e$ such that there is no $x$ such that $e \textcolor{e-color}{<^{\mu}} x$ or $e \textcolor{e-color}{<^{\mu}} x$ are labelled;
          \item the relations are closed under connectivity, \emph{i.e.}, if $v \in s(e)$ then $e' \textcolor{e-color}{<^{\mu}} e$ iff $e' \textcolor{e-color}{<^{\mu}} v$, similarly for $v \in t(e)$ and $\textcolor{closed-color}{<}$.
        \end{enumerate}
  ...
  We require that $\consistency_{p} \not = (E_{p} + V_{p}) \times (E_{p} + V_{p})$ where $V_{p} = \{ v ~ | ~ p \textcolor{e-color}{<^{\mu}} v\}$ and $E_{p} = \{ e ~ | ~ p \textcolor{e-color}{<^{\mu}} e\}$.
\end{itemize} 
...
Given an e-hypergraph $\mathcal{G}$ we can consider a corresponding \textit{underlying} hypergraph $\mathcal{G}'$ by forgetting $\textcolor{e-color}{<}, \textcolor{closed-color}{<}$ and~$\consistency$.
\end{definition}

Intuitively, edges from $\textcolor{e-color}{E}$ encode equivalence classes, \emph{i.e.}, e-boxes, while edges from $\textcolor{closed-color}{E}$ encode lambda-abstraction boxes.
...
$\textcolor{closed-color}{<}$ defines closed monoidal structure as defined in~\cite{fscd} and $(\textcolor{e-color}{<},\consistency)$ defines $\catname{SLat}$ structure as per~\cite{tiurin2025equivalencehypergraphsdporewriting}.

Consider an example in the middle part of Fig.~\ref{fig:e-cospan-example}.
The e-hypergraph in the example is given by $V = \{v_{1}, \ldots w_{10}\}$, $E = \{e_{1}, \ldots, e_{7}\}$, $\textcolor{e-color}{E} = \{e_{1}\}$, $\textcolor{closed-color}{E} = \{e_{2}\}$, $l_{E} = {e_{1} \mapsto \bot, e_{2} \mapsto \bot, e_{3} \mapsto g, \ldots, e_{7} \mapsto f}$ and then we have
\[
\vspace{-1ex}
\begin{array}{cccc}
...
Edges in $\textcolor{e-color}{E}$ are depicted with a dashed box, while edges in $\textcolor{closed-color}{E}$ --- with a round solid box.
\begin{figure}
\[
\adjustbox{scale=0.6}{
...
Graphically, we depict such extended cospans as in Fig.~\ref{fig:e-cospan-example}: feet of the cospan are depicted within shaded regions.
...
In Fig.~\ref{fig:e-cospan-example} the partitions for input and output internal interfaces are given with colours.

Intuitively, such notion of isomorphism makes it possible to freely rearrange interfaces for disjoint components.
For example, in the input interface of cospan in Fig.~\ref{fig:e-cospan-example}, we can swap 
\begin{tikzpicture}
...
Consider, again, Fig. ~\ref{fig:e-cospan-example}.
...
The construction in this section closely follows the one introduced in~\cite{tiurin2025equivalencehypergraphsdporewriting} with some adjustments to account for $\textcolor{closed-color}{<}$.
...
% Consider a hypothetical DPO square below in Fig.~\ref{fig:interface_change_example} where long arrows denote matching between carriers of cospans.
...
\begin{enumerate} 
    ...
    \item for all $v,w$ in $\mathcal{G}$ in the image of $i + j$,  $v$ and $w$ share the same sequence of predecessors, and either $v,w$ are top-level, or $v \consistency w$, or there exists $e \in \textcolor{closed-color}{E}$ s.t. $v,w \textcolor{closed-color}{<^{\mu}} e$;
    \item for all $v,w$ in $\mathcal{L}^{\bot}$ in the image of $i + j$,  $v$ and $w$ share the same sequence of predecessors, and either $v,w$ are top-level or or $v \consistency w$, or there exists $e \in \textcolor{closed-color}{E}$ s.t. $v,w \textcolor{closed-color}{<^{\mu}} e$;
