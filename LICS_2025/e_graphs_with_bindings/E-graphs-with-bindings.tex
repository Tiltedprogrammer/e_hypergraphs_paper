...
\textcolor{red}{Pioneering work in this direction has been done by~\cite{koehler2022sketchguided} where
...
As e-graph endeavours to represent multiple terms at once with as much sharing as possible, using variable names for binding means that two $\alpha$-equivalent terms can not be shared as they are structurally different.
Even though such simple encoding makes expressing $\beta$-reduction as a rewrite rule in terms of explicit substitution simpler, applying such rules requires computing extra free variable predicates and populates a given e-graph with extra nodes for explicit substitution.
On the other hand, encoding binding using $\textit{de Brujin}$ while still fails to represent $\alpha$-equivalent terms as being structurally equivalent, as \textit{e.g.}, in $\lambda . \%0 ((\lambda . \%1 \%0) 42)$ the same binder is represented with $\%0$ and $\%1$, avoids explicit substitution when implementing $\beta$-reduction at the cost of including \textit{ad hoc} operators for index shifting into the rewrite rules.
...
Other works suggested an approach to add internal support for binding which however made the formulation of e-graph considerably more complicated~\cite{slotted-egraphs}.
}
\textcolor{blue}{The reference above is a workshop unpublished work, I do not know if it is good to use such reference.} 
