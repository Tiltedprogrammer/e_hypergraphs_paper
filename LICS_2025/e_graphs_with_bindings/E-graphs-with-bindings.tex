...
On one hand, as an e-graph represent a family of terms at once a $\beta$-reduction should also operate with families of terms which makes it a very expensive rewrite.
On the other hand, e-graphs should represent these families with as much sharing of subterms as possible, and a particular choice of binding encoding affects this requirement.
In this work we will focus on the second part of this challenge.
The most simple way of encoding binding in e-graph is by using explicit variable names.
A major drawback of such encoding is that it fails to share $\alpha$-equivalent terms as, $\textit{e.g.}$, $\lambda x . x$ and $\lambda y . y$ are structurally different and hence will be represented as different subterms in an e-graph that uses such encoding.
$\beta$-reduction is then implemented by introducing an explicit substitution operator into the term calculus.
When applying a $\beta$-reduction rule, such explicit substitution nodes quickly populate an e-graph which results in performance issues.
The rewrite rule itself also becomes more complicated as there is a need to compute free variable predicates in order to apply the rule.
}
\textcolor{red}{
