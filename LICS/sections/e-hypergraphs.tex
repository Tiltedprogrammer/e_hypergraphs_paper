...
\label{def:e-homo}    
An \emph{e-hypergraph $\mathcal{G}$ over a signature $\Sigma$} is a tuple $(V,E,s,t,l,<,\consistency)$,  where $(V,E,s,t)$ is an unlabelled hypergraph;  $l :E \to \Sigma + 1$ is a labelling function modified to include an extra value $\bot$ in its codomain; $<$ is a strict partial order on $V + E$ called the \textit{child relation}.  
...
\end{enumerate}
We additionally define an equivalence relation $\consistency_p$,  called the \textit{consistency relation},  on each set $\{x \in V + E ~|~ p <^\mu x\}$ of elements which share the same parent.  
...
Technically,  condition $(3)$ on the child relations says that $<^{\mu}$ respects connectivity of edges and vertices,  and similarly for the consistency relation. 
...
for rules whose source or target contain a maximal connected sub-hypergraph with no input and output vertices.  
While we believe this limitation can be overcome,  we find the technical development to be significantly simpler if we simply avoid consideration of such sub-hypergraphs.  
For this reason,  in the remainder of the paper we consider signatures which can not result in construction of terms with type $0 \to 0$,  thus restricting possible e-hypergraphs to those without such problematic edges. 
