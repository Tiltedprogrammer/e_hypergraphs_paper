...
Because the application of some rewrites can either enable or block the subsequent application of others, the choice of application order significantly impacts the quality of the resulting optimisation.  
...
Typical approaches to this problem use heuristics to determine an ordering. 
...
Considering programs as represented by terms of an arbitrary algebraic theory, we demonstrate a correspondence between e-graphs and (string diagrams for) simple Cartesian categories enriched over a category of semilattices (\textit{semilattice-enriched}).
...
A final contribution of this work is giving a concrete combinatorial representation of enriched string diagrams in terms of (hierarchical)  hypergraphs, which we call \textit{e-hypergraphs}. 
...
As a corollary, this also provides a formalisation of the rewriting theory of e-graphs as encountered in the literature. 
...
Recalling the correspondence between DAG representations of algebraic terms and  string-diagrams for morphisms of a suitable Cartesian category (see \textit{e.g.}~\cite{GhicaZanassiStringLambda})
...
Note that the motivating example of our work, e-graphs, requires the Cartesian structure to express arbitrary sharing as used in e-graphs.
...
We denote by $\catname{SLatt}$ the closed monoidal category of semilattices which is defined in the Appendix (\textcolor{red}{add definition of $\catname{SLatt}$ in the Appendix}).
...
\]
A Cartesian SMT is given by a set of generators $\Sigma_C$ which additionally contains the generators of commutative comonoids, together with the equations of the SMT of commutative comonoids, and the following additional naturality equations, for every $c \in \Sigma_C$
\[
	\scalebox{0.6}{
...
Generally,  we may define a category \textit{enriched} in any other (closed) monoidal category $M$ (\textit{e.g.},  \textbf{SLatt}).  
...
In particular, 2-functor $F$ turns every category $\mathbb{C'} \in \catname{Cat}$ into a free semilattice enriched $\mathbb{C} \in \catname{SLatt}\text{--}\catname{Cat}$ by making every hom-set of $\mathbb{C'}$ into a free semilattice on this set.
...
Both of these categories are in $\catname{PROP}^{+}$ that is obtained from $\catname{PROP}$ via the same adjunction.
\end{definition}
