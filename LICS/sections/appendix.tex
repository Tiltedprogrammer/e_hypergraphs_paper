...
We will then discuss the uniqueness of the corresponding pushout complement.
...
\begin{proof}
    ...
    Then, since $m$ and $[c_1,c_2]$ are monos, the first square can be rewritten as follows.
...
\[\begin{tikzcd}
	{a + x + y} && {i + j + x} && {i+j} \\
	\\
	{a + x + y + z} &&&& {\begin{matrix} i + j + w \\ \cong \\ i + j + z \end{matrix}}
...
\end{tikzcd}\]
    Because $a + x + y + w$ also yields a pushout it must be the case that $a+x+y+w \cong a + x + y + z$ and therefore $w \cong z$ and pushout complement for vertices is $i + j + z$ up to isomorphism.
    One can further observe that $g_{V} = h_{V,ext};h_{V,int} + id_{n}$ and that $h_{V,ext}$ is mono and $h_{V,int} = [h_1,h_2]$ where $h_1,h_2$ are mono.
    The above shows that edges and vertices in the pushout complement are uniquely determined.
    ...
    Clearly there is a morphism from $\mathcal{L}^{\bot}$ to $\mathcal{G}'$, i.e. the morphism $g' = (g_{V},g_{E})$ that has the same action on edges and vertices as the morphism $g$ does, because $v'$ such that $[v') \not = \varnothing$ can be mapped to $g_{V}(v')$ such that $[g_{V}(v')) \not = \varnothing$.
...
\begin{proof}
    ...
    \begin{itemize}
        \item If $\mathcal{C}$ is empty, then either $f = g$ syntactically and $\llbracket f \rrbracket = \llbracket g \rrbracket$ or $f = l = r = g$ for some $l = r$ or $r = l$ in $\mathcal{S}$.
            ...
            \] is an MDA e-hypergraph.
            By denoting the pushout of $\mathcal{L}_1^{\bot} \xleftarrow{} n_1 + m_1 \xrightarrow{} \mathcal{L}_{2}^{\bot}$ with $\mathcal{L}_3^{\bot}$,
            one can observe that $\llbracket f \rrbracket$ is the pushout object of $\mathcal{L} \xleftarrow{} i+j \xrightarrow{} \mathcal{L}$ as per the diagram below
            \[\begin{tikzcd}
    ...
    \end{itemize}
    The rest of the argument follows by induction on the length of $w$ and ultimately we have shown that if $f = g$ then $\llbracket f \rrbracket = \llbracket g \rrbracket$ in $\WellTypedMdaEcospans(\Sigma)/\mathcal{S}$.
