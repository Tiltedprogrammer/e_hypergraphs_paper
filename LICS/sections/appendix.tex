...
\begin{remark}
    The first assumption in~\ref{pushout:assumptions} can be dropped with extra requirements that for all egdes $e_{i}, e_{j} \in E_{Z}$:
    \begin{itemize}
        \item $[f_{E}(e_{i})) = [f_{E}(e_{j}))$ and $[g_{E}(e_{i})) = [g_{E}(e_{j}))$ \textcolor{red}{This has to be dropped}
        \item \textcolor{red}{Find appropriate conditions}
    \end{itemize}
\end{remark}
...
\end{tikzcd}\]
    ...
    \textcolor{red}{The same reasoning applies to show $E_{\mathcal{L}^{\bot}}$} is unique.
