...
For every canonical acyclic connected e-graph there is a corresponding cospan of e-hypergraphs given by a function $[]$ that is illustrated by the figure in the introduction and further developed in the Appendix.
One e-graph can be turned into another one via addition and upward merging if and only if one e-hypergraph can be rewritten into another one and vice versa.
\end{proposition}
% \begin{proof}
