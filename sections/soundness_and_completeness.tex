...
$[-]$ is full.
\begin{proof}
As we quotient by $\mathcal{S}$ instead of arbitrary cospans we can only consider their normal forms $A_1 + \ldots A_n$ where all of $A_i$ do not contain any hierarchical edges.
Because each of $A_i$ is a plain hypergraph, they have a pre-image in $\textsf{PROP}(\Sigma)$, i.e. $A_i = \llbracket \cdot \rrbracket;\mathcal{I}(a_i)$.
