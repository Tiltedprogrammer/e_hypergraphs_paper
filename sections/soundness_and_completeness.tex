...
\[
[\cdot] : \textsf{PROP}^{+}(\Sigma) \to \sfrac{\WellTypedMdaEcospans}{\mathcal{S}}
\]
where $\sfrac{\WellTypedMdaEcospans}{\mathcal{S}}$ is $\WellTypedMdaEcospans$ quotiented by structural rewrite rules which makes this category into a semilattice-enrhiched one.
By freeness, it suffices to define how the functor acts on generators \update{Identities and generators from the signature?}.
...
\begin{enumerate}
    ...
    \item $[ f + g ] = [ f ] + [ g ]$  for $f,g \in \Sigma$;
    \item \update{Symmetry?}
\end{enumerate}

this also makes the functor into a symmetric monoidal semilattice-enriched SMC one on the nose.
...
\begin{proposition}
    ...
    \begin{proof}
        Consider the commutative diagram below \update{Is it obvious that the diagram commutes?}.
...
\begin{theorem}
    $a \Rightarrow_{\langle l, r \rangle}^{+} b$ iff $[a] \Rrightarrow_{\langle [l],[r] \rangle} [b]$.
\end{theorem}