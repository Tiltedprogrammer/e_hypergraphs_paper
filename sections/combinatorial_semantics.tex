...
\begin{definition}[E-hypergraph]
    ...
    \item \update{Complement of $\conflict_{\mu}$ should be transitive}
...
\begin{enumerate}
    ...
    \[
        \langle \phi_{V};i_{V_{\mathcal{G}}}, \phi_{E};i_{E_{\mathcal{G}}} \rangle^{*}(\conflict_{\mathcal{F}}(i_{E_{\mathcal{F}}}(x)))
        \subseteq
        \conflict_{\mathcal{G}}(\phi_{E};i_{E_{\mathcal{G}}}(x))
    \]
    ...
    \[
        \langle \phi_{V};i_{V_{\mathcal{G}}}, \phi_{E};i_{E_{\mathcal{G}}} \rangle^{*}(\conflict_{\mathcal{F}}(i_{V_{\mathcal{F}}}(x)))
        \subseteq
        \conflict_{\mathcal{G}}(\phi_{V};i_{V_{\mathcal{G}}}(x))
    \]
    Or, on per-component basis, if $i_{E_{\mathcal{F}}}(x_1) \in \conflict_{\mathcal{F}}(i_{E_{\mathcal{F}}}(x_2))$ then $i_{E_{\mathcal{G}}}(\phi_{E}(x_1)) \in \conflict_{\mathcal{G}}(i_{E_{\mathcal{G}}}(\phi_{E}(x_2)))$ and similarly for other types of $x_1,x_2$.
\end{enumerate}
...
\begin{remark}
    ...
    \begin{itemize}
    \item First, we assume that $Z$ is a \textit{discrete} e-hypergraph, i.e. it has no edges.
    \item Second, we assume that $[f_{V}(v_i)) = [f_{V}(v_j))$ and $[g_{V}(v_i)) = [g_{V}(v_j))$ for all $v_i,v_j \in V_{Z}$. \update{This essentially means that $f,g$ do not map nodes to levels of different hierarchy.}
    ...
    \[
        X + Y = \{V_{X} + V_{Y}, E_{X} + E_{Y}, s_{X+Y}, t_{X+Y}, \#_{X+Y}, <_{c,X+Y} \}
    \]
    ...
    For this we will consider several cases where \update{$f$ and $g$ are mono. I require mono just to simplify things, we can get rid of this requirement if everything else is ok.}.
    ...
    \begin{enumerate}
        ...
        \item $u$ has a pre-image $z$ in $V_{Z}$, (by discreteness, $[z) = \varnothing$) and $[f_{V}(z)) \not = \varnothing$ $[g_{V}(z)) = \varnothing$.
              ...
              \]
        \item $u$ has no pre-image in $Z$ and then 
        \[
            <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}[u]) = [<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(u))]
        \]
    \end{enumerate}
    ...
    \begin{enumerate}
        \item We have the only case of $u$ having no pre-image in $E_{Z}$ because its empty.
              Consider two cases.
              \begin{itemize}
                \item  $<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(u)) = u_1$.
                    ...
                    \]
                \item $[u) = \varnothing$ and for all $v_i \in s(u)$ and $v_j \in t(u)$ $[[v_i]) \not = \varnothing$ and $[[v_j]) \not = \varnothing$.
                      Then we define 
                    \[
    ...
    \begin{remark}
        Essentially, what we build above is that $[e] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$ when there exists $v' \in V_{X} + V_{Y}$ such that $[v'] = [v]$ and $e = <_{c,X+Y}^{\mu}(v')$.
    \end{remark}
    \update{If $f$ and $g$ are not mono, the definitions stay the same as $i_{2,E}$ and $i_{1,E}$ are mono.}
    ...
    \question{I probably should've added injections for $<$ as well as it is defined on a coproduct (like I do below for $\conflict$) }
    
    \update{Check that $<_{c,\sfrac{X+Y}{\sim}}^{\mu}$ is irreflexive and assymetric and that it respects connectivity.}
    \begin{itemize}
        \item Suppose there exist $[e]$ such that $[e] = <_{c,\sfrac{X+Y}{\sim}^{\mu}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e]))$.
              ...
              \begin{itemize}
                \item Assume $<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e)) = e_1$, then  $<_{c,\sfrac{X+Y}{\sim}^{\mu}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = [e_1]$. 
                      This would mean that $e_1 = e$ which would mean that the relation $<_{c,X+Y}^{\mu}$ is not irreflexive.
                \item Assume $[e) = \varnothing$ and for all $v_{i} \in s(e)$ and $v_{j} \in t(e)$ $[[v_i]) \not = \varnothing$ $[[v_j]) \not = \varnothing$.
                      Then, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
                      \begin{itemize}
                        \item If $v$ has no pre-image in $V_{Z}$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v))] = [e]$.
                              This would mean that $e$ is a predecessor of $v$ in $X+Y$, but $v$ is either in the sources or targets of $e$ which violates respectful connectivity condition.
                        \item $v$ has a pre-image $f_{V};i_{1,V}(z)$ in $V_{Z}$ such that $[f_{V}(z)) \not = \varnothing$ and $[g_{V}(z)) = \varnothing$.
                              Then, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [i_{1,E}(e_x)] = [e]$.
                              This means that $e$ has a pre-image in $E_{X}$, but because $v$ also has a pre-image in $V_{X}$ and its set of predecessors is not empty it would mean that $e$ has a predecessor which it does not.
                        \item $v$ has a pre-image $f_{V};i_{1,V}(z)$ in $V_{Z}$ such that $[f_{V}(z)) = \varnothing$ and $[g_{V}(z)) \not = \varnothing$.
                              Then, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [i_{2,E}(e_y)] = [e]$.
                              It means that $e$ has a pre-image in $E_{Y}$, but $v$ is its source or target and has a pre-image in $V_{Y}$ which is not possible.
                        \item The remaining two cases for $v = g_{V};i_{2,V}(z)$ are symmetric.
                      \end{itemize}
              \end{itemize}$[<_{c,X+Y}^{\mu}(e)]$ and $e = <_{c,X+Y}^{\mu}(e)$ because edges are not identified.
        ...
        \item Let $e_1,e_2$ be edges. 
              ...
              \begin{itemize}
                ...
                \begin{itemize}
                    ...
                    \item $[e_1) = \varnothing$ and for all $v_i \in s(e_1)$ and $v_j \in t(e_1)$ $[[v_i]) \not = \varnothing$ and $[[v_j]) \not = \varnothing$.
                          ...
                          \begin{itemize}
                            ...
                            \item If $v$ has a pre-image $f_{V};i_{1,V}(z)$ in $V_{Z}$ then necessarily $[f_{V}(z)) = \varnothing$ (because $[e_1) = \varnothing$) and hence $[g_{V}(z)) \not = \varnothing$.
                                  Then,
                                  \[
                                    [<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])] = [i_{2,E}(e_y)] = [e_2]
                                  \]
                                  This means that $e_2$ has a pre-image in $E_{Y}$, but because $e_1$ is a predecessor of $e_2$ they both should have pre-image in either $E_{X}$ or $E_{Y}$.
                            \item Case when $v$ has a pre-image $g_{V};i_{2,V}(z)$ is symmetric to the above.
                          \end{itemize}
                    \end{itemize}
                    \item $[e_2) = \varnothing$ and for all $v_i \in s(e_2)$ and $v_j \in t(e_2)$ $[[v_i]) \not = \varnothing$ $[[v_j]) \not = \varnothing$.
                          Then, 
                          \[
                            <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X}+E_{Y}}{\sim}}([e_2])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [e_1]
                          \]
                          The cases when $[e_1) \not = \varnothing$ are similar to the above, so we will just jump to the interesting case when 
                          $[e_1) = \varnothing$ and
                          \[
                            <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X}+E_{Y}}{\sim}}([e_1])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])) = [e_2]
                          \]
                ...
                \item If $<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1)) = e_3$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = [<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1))] = [e_3] = [e_2]$ which means that $e_2$ is the predecessor of $e_1$.
                      ...
                      \begin{itemize}
                        ...
                        \item $v \sim v'$ which means there exists $z \in V_{Z}$ such that $v = f_{V};i_{1,V}(z)$ and $v' = g_{V};i_{2,V}(z)$ and $v' \in s(e_1)$.
                              ...
                              \]
                        \end{itemize}
                \item $[e_1) = \varnothing$ and for all $v_i \in s(e_1), v_j \in t(e_1)$ $[[v_i]) \not = \varnothing,\; [[v_j]) \not = \varnothing$. 
                Then, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = <_{c,\sfrac{X+Y}{\sim}}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [e_2]$ and this gives us the needed property on the nose.
              \end{itemize}
        ...
        Once again, $[v] \in s([e_1])$ means that there exists $v' \in s(e_1)$ such that $[v'] = [v]$.
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                \[ 
                    [e_2] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [i_{1,E}(e_x)]
                \]
                Because $e_{x}$ is the predecessor of $f_{V}(z)$ it should also be the predecessor of the pre-image of $e_1$ in $E_{X}$ as $v \in s(e_1)$.
                Denote the pre-image of $e_1$ in $E_{X}$ as $e_{1}^{x}$. 
                Then, $e_{x} = <_{c,X}^{\mu}(i_{E_{X}}(e_{1}^{x}))$ and 
                
                \begin{align*}
                    [i_{1,E}(e_{x})] &= [i_{1,E}(<_{c,X}^{\mu}(i_{E_{X}}(e_{1}^{x})))] = \\
                    &= [<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(i_{1,E}(e_{1}^{x})))] = [<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1))] = \\
                    &= <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = [e_2]
                \end{align*}
            ...
            \end{itemize}
            \item $v \sim v'$ and hence there exists $z \in V_{Z}$ such that $v = f_{V};i_{1,V}(z)$ and $v' = g_{V};i_{2,V}(z)$ and $v' \in s(e_1)$.
                  This implies that $e_1$ has a pre-image in $E_{Y}$ and
                  \[
                    [e_2] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))
                  \]
                  \begin{itemize}
                    \item $[f_{V}(z)) = \varnothing$ and $[g_{V}(z)) \not = \varnothing$ which implies $[g_{V};i_{2,V}(z)) = [v') \not = \varnothing$.
                    That is, there exists $e_3 = <_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v'))$ and by e-hypergraphness of $X+Y$ $e_3 = <_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1))$.
                    Then, 
                    \begin{align*}
                    [e_3] = [<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v'))] &= <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])) = \\
                    &= [<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1))] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = \\
                    &= <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V{Y}}{\sim}}([v]))
                \end{align*}
                \item $[f_{V}(z)) \not = \varnothing$ and $[g_{V}(z)) = \varnothing$ which implies $[g_{V};i_{2,V}(z)) = [v') = \varnothing$ and hence $[e_1) = \varnothing$.
                      By definition,
                      \begin{align*}
                        <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) &= <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([v'])) = \\
                        &= <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([v])) = [e_2]
                    \end{align*}
                \end{itemize}
        \item Cases when $v = g_{V};i_{2,V}(z)$ are symmetric.
        \end{itemize}    
    \end{itemize}
    \update{This will be changed to consistency relation, but the definition of quotiening will stay the same.}
    Let's now define $\conflict_{\sfrac{X+Y}{\sim}}$.
    We can consider the original conflict relation as a function $\conflict_{X+Y} : (V_{X} + V_{Y}) + (E_{X} + E_{Y}) \to 2^{(V_{X} + V_{Y}) + (E_{X} + E_{Y})}$.
    Quotienting the range of the function gives us $\conflict_{X+Y}' : V_{X} + V_{Y} + E_{X} + E_{Y} \to 2^{(\sfrac{V_{X} + V_{Y}}{\sim}) + \sfrac{(E_{X} + E_{Y})}{\sim}}$, which is essentially $\langle []_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}}, []_{E};i_{\sfrac{E_{X} + E_{Y}}{\sim}} \rangle^{*}$ (a copairing extended to sequences that we will further denote it as $\langle []_{V}^{\conflict} []_{E}^{\conflict} \rangle$) applied to the return value of $\conflict_{X+Y}$.
    We then define the new relation as
    
    \[
        \conflict_{\sfrac{X+Y}{\sim}}(i_{E_{X} + E_{Y}}([x]_{E})) = (\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\bigcup_{x' \in E_{X} + E_{Y} | [x']_{E} = [x]_{E}} i_{E_{X} + E_{Y}}(x')))^{\Rightarrow}
    \]
    \[
        \conflict_{\sfrac{X+Y}{\sim}}(i_{V_{X} + V_{Y}}([x]_{V})) = (\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\bigcup_{x' \in V_{X} + V_{Y} | [x']_{V} = [x]_{V}} i_{V_{X} + V_{Y}}(x')))^{\Rightarrow}
    \]
        where $i_*$ are injections into $\sfrac{V_{X} + V_{Y}}{\sim} + \sfrac{E_{X} + E_{Y}}{\sim}$ and $\Rightarrow$ is a closure by (1) from~\ref{def:e-hypergraph-req}.
        That is, the relation above is the smallest relation that contains $\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\bigcup_{x' \in E_{X} + E_{Y} | [x']_{E} = [x]_{E}} i_{E_{X} + E_{Y}}(x'))$ and such that if $[x_1] <_{c,\sfrac{X+Y}{\sim}}([x_2])$ and $i_{\sfrac{E_{X} + E_{Y}}{\sim}}[x_1] \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}[x_3])$ then $i_{\sfrac{E_{X} + E_{Y}}{\sim}}[x_2] \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}[x_3])$.
        And similarly for nodes.

          
    Again, we need to check that this is a well-defined function and that our initial $([]_{V},[]_{E})$ respects the conflict.

    \begin{itemize}
        \item Assume that $t_1, t_2 \in E_{X+Y}$ and $t_1 \sim t_2$ then $\conflict_{\sfrac{X+Y}{\sim}}(i_{E_{X} + E_{Y}}[t_1]) = \conflict_{\sfrac{X+Y}{\sim}}(i_{E_{X} + E_{Y}}[t_2])$.
              $t_1 \sim t_2$ implies there exists $z \in E_{Z}$ such that $t_1 = f_{E};i_{1,E}(z)$ and $t_2 = g_{E};i_{2,E}(z)$
              \begin{align*}
                \conflict_{\sfrac{X+Y}{\sim}}(i_{E_{X} + E_{Y}}([t_1])) = (\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\bigcup_{t_i \in \{t_1, t_2\}} i_{E_{X} + E_{Y}}(t_i)))^{\Rightarrow}
                %      \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\conflict_{X+Y}(i_{E_{X} + E_{Y}}(t_1))) = \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\conflict_{X+Y}(f_{E};i_{1,E};i_{E_{X} + E_{Y}}(z)) = \\
                %      \text{By homomorphism-ness of $f_{E};i_{1,E}$} \\
                %      = \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\langle f_{V};i_{1,V};i_{V_{X} + V_{Y}} ,f_{E};i_{1,E};i_{E_{X} + E_{Y}} \rangle^{*}(\conflict_{Z}(i_{E_{Z}}(z)))
              \end{align*}
              Similarly,
              \[
                \conflict_{\sfrac{X+Y}{\sim}}(i_{E_{X} + E_{Y}}([t_2])) = (\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\bigcup_{t_i \in \{t_1, t_2\}} i_{E_{X} + E_{Y}}(t_i)))^{\Rightarrow}
              \]
              and we get well-definedness on the nose.
            %   \begin{align*}
            %     \conflict_{\sfrac{X+Y}{\sim}}(i_{E_{X} + E_{Y}}[t_2]) = \conflict_{X+Y};\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(i_{E_{X} + E_{Y}}(t_2)) = \\
            %          \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\conflict_{X+Y}(i_{E_{X} + E_{Y}}(t_2))) = \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\conflict_{X+Y}(g_{E};i_{2,E};i_{E_{X} + E_{Y}}(z)) = \\
            %          \text{By homomorphism-ness of $g_{E};i_{2,E}$} \\
            %          = \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\langle g_{V};i_{2,V};i_{V_{X} + V_{Y}} ,g_{E};i_{2,E};i_{E_{X} + E_{Y}} \rangle^{*}(\conflict_{Z}(i_{E_{Z}(z)))
            %   \end{align*}
            %   Then,
            %   $[f_{E};i_{1,E}(z')]_{E} = [g_{E};i_{2,E}(z')]_{E}$ for all $z' \in \{z'' \in \conflict_{Z}(z) \text{ s.t. } z'' \in E_{Z}\}$
            %   and
            %   $[f_{V};i_{1,V}(z')]_{V} = [g_{V};i_{2,V}(z')]_{V}$ for all $z' \in \{z'' \in \conflict_{Z}(z) \text{ s.t. } z'' \in V_{Z}\}$
            %   hence
            %   \begin{align*}
            %     \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\langle g_{V};i_{2,V};i_{V_{X} + V_{Y}} ,g_{E};i_{2,E};i_{E_{X} + E_{Y}} \rangle^{*}(\conflict_{Z}(i_{E_{Z}}(z))) = \\
            %      = \langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle^{*}(\langle f_{V};i_{1,V};i_{V_{X} + V_{Y}} ,f_{E};i_{1,E};i_{E_{X} + E_{Y}} \rangle^{*}(\conflict_{Z}(i_{E_{Z}}(z)))
        ...
        \item Putting $\phi = ([]_{V},[]_{E})$ into the definition of a homomorphism gives us
            \[
             \langle []_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}}, []_{E};i_{\sfrac{E_{X} + E_{Y}}{\sim}} \rangle^{*}(\conflict_{X+Y}(i_{E_{X+Y}}(x)))
             \subseteq
             \conflict_{\mathcal{\sfrac{X+Y}{\sim}}}([]_{E};i_{\sfrac{E_{X} + E_{Y}}{\sim}}(x))
             \]
            By definition, 
            \[
                \conflict_{\mathcal{\sfrac{X+Y}{\sim}}}([]_{E};i_{\sfrac{E_{X} + E_{Y}}{\sim}}(x)) = (\langle []_{V}^{\conflict}, []_{E}^{\conflict} \rangle (\bigcup_{x' \in E_{X} + E_{Y} | [x']_{E} = [x]_{E}} \conflict_{X+Y}(i_{E_{X} + E_{Y}}(x))))^{\Rightarrow}
            \]
            and by recalling that $[]_V^{*} = []_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}}$ and similarly for $[]_{E}^{*}$ we get the inclusion.
    \end{itemize}
    All is left is to check that $\sfrac{X + Y}{\sim}$ is indeed an e-hypergraph.
    For this we need to check conditions (1) - (7) in~\ref{def:e-hypergraph-req}.
    % \begin{enumerate}
    %     \item Assume all $x_1, x_2, x_3$ are in $E_{X+Y}$.
    %      Given $[x_1] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}([x_2])$ 
    %      and 
    %      $i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_1]) \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_3]))$ then it must be the case that
    %     $i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_2]) \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_3]))$.
        
    %     \[
    %         i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_1]) \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_3]))
    %     \]
    %     means that there exist $x',x'' \in E_{X} + E_{Y}\; | \;[x'] = [x_3] \text{ and } x'' \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(x')) \text{ and } [x''] = [x_1]$.
    %     We then begin by considering cases for $<$ and $[x_1], [x_2], [x_3], [x']$.
    %     \begin{itemize}
    %         \item $[x_1] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}([x_2]) = [<_{c,X+Y}^{\mu}(x_2)]$.
    %         We have two options.  
    %             \begin{itemize}
    %                 \item $x_2$ has a pre-image $z_{x_2}$ in $E_{Z}$ such that $[z_{x_2}) \not = \varnothing$.
    %                 \begin{itemize}
    %                 \item Suppose $x_1 = <_{c,X+Y}^{\mu}(x_2)$, $x' = x_3$, $x'' = x_1$ hence $i_{E_{X} + E_{Y}}(x_1) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(x_3))$.
    %                       Then, because $X+Y$ is an e-hypergraph it must be the case that $i_{E_{X} + E_{Y}}(x_2) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(x_3))$.
    %                       And, by homomorphism-ness of $[]$ we get $i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_2]_{E}) \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_3]_{E}))$. 
    %                 \item Suppose $x_1 = <_{c,X+Y}^{\mu}(x_2)$, $x' \sim x_3$, $x'' = x_1$ hence there exists $z \in E_{Z}$ such that $x' = f_{E};i_{1,E}(z)$ and $x_3 = g_{E};i_{2,E}(z)$.
    %                       Hence $i_{E_{X} + E_{Y}}(x_1) \in \conflict_{X+Y}(f_{E};i_{1,E};i_{E_{X} + E_{Y}}(z))$.
    %                       Then, because $X+Y$ is an e-hypergraph it must be the case that $i_{E_{X} + E_{Y}}(x_2) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(f_{E};i_{1,E}(z)))$.
    %                       And, by homomorphism-ness of $[]$ we get $i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_2]_{E}) \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([f_{E};i_{1,E}(z)]_{E})) = \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_3]_{E}))$.
    %                 \item Suppose $x_1 = <_{c,X+Y}^{\mu}(x_2)$, $x' = x_3$, $x'' \sim x_1$ hence there exists $z \in E_{Z}$ such that $x'' = f_{E};i_{1,E}(z)$ and $x_1 = g_{E};i_{2,E}(z)$.
    %                       Hence $i_{E_{X} + E_{Y}}(f_{E};i_{1,E}(z)) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(x_3))$.
    %                       By definition $x_2 = g_{E};i_{2,E}(z_{x_2})$ (because $x_1$ and $x_2$ should have pre-image in either $X$ or $Y$, not both).
    %                       This means there exists $z_2 <_{c,Z}^{\mu}(z_{x_2})$ and $g_{E};i_{2,E}(z_2) <_{c,X+Y}^{\mu}(g_{E};i_{2,E}(z_{x_2})) = <_{c,X+Y}^{\mu}(x_2)$ and because there can be only one imemdiate predecessor we have $x_1 = g_{E};i_{2,E}(z_2)$ and $z = z_2$.
    %                       \question{What next?}
    %                       Because $X+Y$ is an e-hypergraph, we have $i_{E_{X} + E_{Y}}(f_{E};i_{1,E}(z')) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(x_3))$.
    %                       Then, by homomorphism-ness we get $i_{\sfrac{E_{X} + E_{Y}}{\sim}}([f_{E};i_{1,E}(z')]) = i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_2]) \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([x_3]))$.
    %                 \item Suppose $x_1 = <_{c,X+Y}^{\mu}(x_2)$, $x' \sim x_3$, $x'' \sim x_1$ hence there exist $z_1 \in E_{Z}$ such that $x'' = f_{E};i_{1,E}(z_1)$ and $x_1 = g_{E};i_{2,E}(z_1)$ and $z_2 \in E_{Z}$ such that $x' = f_{E};i_{1,E}(z_2)$ and $x_3 = g_{E};i_{2,E}(z_2)$.
    %                       Hence, $i_{E_{X} + E_{Y}}(x'') \in \conflict_{X+Y}(i_{E_{X} + E_{Y}})(x')$ and $i_{E_{X} + E_{Y}}(f_{E};i_{1,E}(z_1)) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}})(f_{E};i_{1,E}(z_2))$.
    %                       There also should exist $z_3$ such that $z_1 <_{c,Z}^{\mu}(z_3)$ and $g_{E};i_{2,E}(z_1) <_{c,Z}^{\mu}(g_{E};i_{2,E}(z_3))$
    %             \end{itemize}
    %             $x_2$ has no pre-image in $E_{Z}$.
    %         \end{itemize}
    %         \end{itemize}
    % \end{enumerate}

    \begin{enumerate}
        \item (1) is given by construction.
        \item There are separate cases for nodes and edges.
              \begin{itemize}
                \item Being not mutually exclusive means it is possible that $[e]_{E} <_{c,\sfrac{X+Y}{\sim}}([v]_{V})$ and $i_{\sfrac{V_{X}+V_{Y}}{\sim}}[v] \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X}+E_{Y}}{\sim}}[e])$.
                \item Or, $[e_1]_{E} <_{c,\sfrac{X+Y}{\sim}}([e_2]_{E})$ and $i_{\sfrac{E_{X}+E_{Y}}{\sim}}[e_1] \in \conflict_{\sfrac{X+Y}{\sim}}(i_{\sfrac{E_{X}+E_{Y}}{\sim}}[e_2])$.
               \end{itemize}
              The first means that either
              \begin{itemize}
                \item $[e] = [<_{c,X+Y}(v)]$ if $v$ has no pre-image in $V_{Z}$ and there exist $v'$ and $e'$  such that $[v'] = [v]$, $[e'] = [e]$ and $i_{V_{X} + V_{Y}}(v') \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(e'))$.
                      As there is no pre-image in $V_{Z}$, $v = v'$. Then we have the following case.
                    \begin{itemize}
                        \item $e = <_{c,X+Y}(v)$ and $e' = e$.
                              As we have $i_{V_{X} + V_{Y}}(v) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(e))$ and this is not possible as $X+Y$ is an e-hypergraph.
                    \end{itemize}
                \item $v$ has a pre-image $z \in V_{Z}$, let's say, $f_{V};i_{1,V}(z) = v$, and $[f_{V}(z)) = \varnothing$.
                      Then, either $v' = v$ and $i_{V_{X} + V_{Y}}(v) \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(e))$, which is not possible as it requires that $v$ has a predecessor in $X$.
                      Or, $v' \sim v$ (meaning that $v' = g_{V};i_{2,V}(z)$) and $i_{V_{X} + V_{Y}}(v') \in \conflict_{X+Y}(i_{E_{X} + E_{Y}}(e))$.
                      Then, $<_{c,\sfrac{X+Y}{\sim}}([v]) = [g_{V};<_{c,Y};i_{2,E}(z)] = [e]$. Once again, it means that $e = g_{V};<_{c,Y};i_{2,E}(z)$.
                      Overall, it means that $g_{V}(z) \in \conflict_{Y}(i_{E_{Y}}(g_{V};<_{c,Y}(z)))$ which is not possible as $Y$ is an e-hypergraph.
                \item The case when $v = g_{V};i_{2,V}(z)$ is symmetric.
              \end{itemize}
    \end{enumerate}
    Let's now prove the universal property.
    $u_{E}([i_{1,E}(x)]) = j_{1,E}(x)$ and $u_{E}([i_{2,E}(y)]) = j_2(y)$.
    ...
    \update{TODO: $\conflict$}
    % We can think of relation $<_{c,X+Y}$ as of a function $<_{c} : V_{X+Y} + E_{X+Y} \to {E_{X+Y}}^{*}$, i.e. a function that for each node or edge returns the sequence of their predecessors.
    % Then we can define the quotienting as before
    % \[
    % \sfrac{(<_{c,X+Y})}{\sim}([u]) = [<_{c,X+Y}(u)]_{E}^{*}\\
    % \]
    % or, if we consider a function that returns the immediate predecessor
    % \[
    % \sfrac{(<_{c,X+Y}^{\mu})}{\sim}([u]) = [<_{c,X+Y}^{\mu}(u)]_{E}
    % \]
    % To check that the function is well-defined we need to check that if $t_1 \sim t_2$ then $\sfrac{<_{c,X+Y}^{\mu}}{\sim}([t_1]) = \sfrac{<_{c,X+Y}^{\mu}}{\sim}([t_2])$ or $[<_{c,X+Y}^{\mu}(t_1)] = [<_{c,X+Y}^{\mu}(t_2)]$.
    % Once again, $t_1 \sim t_2$ (suppose these are nodes) implies there exists $z$ such that $t_1 = f_{V}(z);i_{1,V}(z)$ and $t_2 = g_{V};i_{2,V}(z)$.
    % Hence we have
    % \[
    %     [<_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))]
    % \]
    % \[
    %     [<_{c,X+Y}^{\mu}(g_{V};i_{2,V}(z))]
    % \]
    % and two cases
    % \begin{itemize}
    %     \item $[z) \not = \varnothing$ in which case we can use the fact that $f_{V};i_{1,V}$ and $g_{V};i_{2,V}$ are homomorphisms.
    %           This gives us 
    %             \[
    %                 f_{V};i_{1,V}(<_{c,Z}^{\mu}(z)) = <_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))
    %             \] 
    %             and
    %             \[
    %                 g_{V};i_{2,V}(<_{c,Z}^{\mu}(z)) = <_{c,X+Y}^{\mu}(g_{V};i_{2,V}(z))
    %             \]
    %             and finally,
    %             \[
    %                 [<_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))] = [<_{c,X+Y}^{\mu}(g_{V};i_{2,V}(z))]
    %             \] 
    %             as they share the same pre-image.
    %     \item Another case is when $[z) = \varnothing$. 
    %           Assume $[f(z)) = \varnothing$, in which case $<_{c,Z}^{\mu}(z)$ and $<_{c,X}^{\mu}(f_{V}(z))$ are undefined.
    %           This takes us to the situation where the lefthand-side of what we want to prove is undefined while the right-hand side is defined.
    %           \question{Can I somehow require that in such case I take the right-hand side as a definition of the lefthand side? 
    %           Otherwise the relation we have is too weak because it tells us only to identify things with the pre-image in $Z$ but eventually we also want to identify things that do not have such a pre-image.
    %           }
    %           What I want is to define 
    %           \[
    %             \sfrac{<_{c,X+Y}^{\mu}}{\sim}([f_{V};i_{1,V}(z)]) = [<_{c,X+Y}^{\mu}(g_{V};i_{2,V}(z))] \text{ if } [f_{V};i_{1,V}(z)) = \varnothing
    %           \]
    % \end{itemize}

    % \update{Approach 2}

    % We can extend the set of edges in an e-hypergraph with a special edge $\bot$.
    % And allow that it can be mapped to any other edge under a homomorphisms, i.e. we will not require that $s;\phi^{*}(\bot) = \phi;s;(\bot)$.
    % \question{but this might break quotienting of sources and targets}
    % This also means that set of edges now is $E_{\mathcal{G}} + \bot$.
    % \question{What to do with $\bot$ after a coproduct?}.
    % This gives us the ability to map this $\bot$ using homomorphisms.

    % The pushout of e-hypergraphs $X$ and $Y$ is computed in two steps.
    % First, a coproduct of $X+Y$ is computed which is 
    
    % \[
    %     X + Y = \{V_{X} + V_{Y}, E_{X} + E_{Y} + \bot_{X} + \bot_{Y}, s_{X+Y}, t_{X+Y}, \#_{X+Y}, <_{c,X+Y} \}
    % \]

    % \[
    %     R_{E} = \{ (x_i, y_j) \in (E_{X} + E_{Y} + \bot_{X} + \bot_{Y}) \times (E_{X} + E_{Y} + \bot_{X} + \bot_{Y} | \exists z \in E_{Z} + \bot_{Z} \; . \; x_i = f_{E};i_{1,E}(z) \text{ and } y_j = g_{E};i_{2,E}(z))\} 
    % \]

    % Consider $<_{c,X+Y}^{\mu}$ as a function that returns the immediate predecessor, i.e. it has a type $V_{X} + V_{Y} + E_{X} + E_{Y} \to E_{X} + E_{Y} + \bot_{X} + \bot_{Y}$.
    % And define its quotient
    % \[
    % \sfrac{<_{c,X+Y}^{\mu}}{\sim}([u]) = [<_{c,X+Y}^{\mu}(u)]_{E}
    % \]

    % We then need to check that whenever $t_1 \sim t_2$ $[<_{c,X+Y}^{\mu}(t_1)] = [<_{c,X+Y}^{\mu}(t_2)]$.
    % $t_1 \sim t_2$ implies that there exists at least one $z$ such that $t_1 = f_{V};i_{1,V}(z)$ and $t_2 = g_{V};i_{2,V}(z)$.
    % Hence we have
    % \[
    %     [<_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))]
    % \]
    ...
    % \]
    % and two cases
    % We then can use the fact that $f_{V};i_{1,V}$ and $g_{V};i_{2,V}$ are homomorphisms.
    % This gives us 
    % \[
    %     f_{V};i_{1,V}(<_{c,Z}^{\mu}(z)) = <_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))
    % \] 
    ...
    %     [<_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))] = [<_{c,X+Y}^{\mu}(g_{V};i_{2,V}(z))]
    % \] 
    % as they share the same pre-image ($<_{c,Z}^{\mu}(z)$).
    
    % Now let's check that $\sfrac{s_{X+Y}}{\sim}$ is well-defined.
    % We will consider a case where $f_{E}(\bot_{Z}) = \bot_{X}$ and $g_{E}(\bot_{Z}) \not = \bot$.
    % We then need to check that $[s_{X+Y}(f_{E};i_{1,E}(z))]^{*} = [s_{X+Y}(g_{E};i_{2,E}(z))]$
    % \begin{itemize}
    %     \item $z = \bot_{Z}$ then $[{}] = \ldots$
    % \end{itemize}

    % \update{end of Approach 2}
