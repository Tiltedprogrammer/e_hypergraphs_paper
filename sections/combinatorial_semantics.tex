...
\begin{enumerate}
    ...
    \item $\phi(<_{c,\mathcal{F}}^{\mu}(x)) = <_{c,\mathcal{G}}^{\mu}(\phi(x))$.
...
\begin{remark}
    ...
    First, we assume that $Z$ is a \textit{discrete} e-hypergraph, i.e. it has no edges.
    ...
    Recall that $<_{c}$ is essentially a transitive clousure of $<_{c}^{\mu}$ and hence we can first define $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([u]_{E})$ and $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([u]_{V})$.
    ...
    Assume that $u$ is an element of $V_{X+Y}$.
    \begin{enumerate}
        ...
        \item $u$ has a pre-image $z$ in $V_{Z}$ (by discreteness, $[z) = \varnothing$) and $[f_{V}(z)) = \varnothing$ $[g_{V}(z)) \not = \varnothing$.
              The last condition means that there exists $e_y = <_{c,Y}^{\mu}(g_{V}(z))$.
              We then define $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([u]) = [g_{V};<_{c,Y}^{\mu};i_{2,E}(z)] = [i_{2,E}(e_y)]$.
        \item $u$ has a pre-image $z$ in $V_{Z}$, (by discreteness, $[z) = \varnothing$) and $[f_{V}(z)) \not = \varnothing$ $[g_{V}(z)) = \varnothing$.
              The one before the last condition means that there exists $e_x = <_{c,X}^{\mu}(f_{V}(z))$.
              We then define $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([u]) = [f_{V};<_{c,X}^{\mu};i_{1,E}(z)] = [i_{1,E}(e_x)]$.
        \item $u$ has no pre-image in $Z$ and then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([u]) = [<_{c,X+Y}^{\mu}(u)]$
    \end{enumerate}
    ...
    \begin{enumerate}
        \item We have the only case of $u$ having no pre-image in $E_{Z}$ because its empty.
              Hence, we define $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([u]) = [<_{c,X+Y}^{\mu}(u)]$.
    \end{enumerate}
    \question{I decided to keep Z discrete, otherwise the number of cases to consider within the proofs is too large.
    This also means that we now can identify only nodes.
    I will make changes to account for this later.
    }
    ...
    Suppose $t_1, t_2$ are nodes and $t_1 \sim t_2$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([t_1]) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}([t_2])$.
    $t_1 \sim t_2$ implies there is $z$ such that $t_1 = f_{V};i_{1,V}(z)$ and $t_2 = g_{V};i_{2,V}(z)$ and either $[f_{V}(t_1)) = \varnothing$ or $[g_{V}(z)) = \varnothing$.
    Assume the latter, then, by definition.
    \[
        <_{c,\sfrac{X+Y}{\sim}}^{\mu}([t_1]) = [f_{V};<_{c,X}^{\mu};i_{1,E}(z)]    
    \]
    and
    \[
        <_{c,\sfrac{X+Y}{\sim}}^{\mu}([t_2]) = [f_{V};<_{c,X}^{\mu};i_{1,E}(z)]    
    \]
    ...
    \begin{enumerate}
        ...
        \item $[<_{c,X+Y}^{\mu}(e)]_{E} = <_{c,\sfrac{X+Y}{\sim}}^{\mu}([e]_{E})$ for $e$ such that $[e) \not = \varnothing$.
              This is homomorphic by definition. 
        \item $[<_{c,X+Y}^{\mu}(v)]_{E} = <_{c,\sfrac{X+Y}{\sim}}^{\mu}([v]_{V})$.
                ...
                \item $v$ has a pre-image $z$ in $V_{Z}$ such that $v = f_{V};i_{1,V}(z)$.
                    \begin{itemize}
                        ...
                        \item If $[f_{V}(z)) \not = \varnothing$ and $[g_{V}(z)) = \varnothing$, then,
                              by definition, we have $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([v]) = [f_{V};<_{c,X};i_{1,E}(z)] = [i_{1,E}(<_{c,Y}^{\mu}(f_{V}(z)))] = [i_{1,E}(e_y)]$.
                              Because $i_{1,E}$ is a homomorphism, we also have $[<_{c,X+Y}^{\mu}(f_{V};i_{1,V}(z))] = [i_{1,E}(<_{c,Y}^{\mu}(f_{V}(z)))] = [i_{1,E}(e_y)]$.
                    \end{itemize}
                \item The case when $v = g_{V};i_{2,V}(z)$ is symmetric.
                \item If $v$ has no pre-image in $V_{Z}$ then homomorphism-ness of $[]_{V}$ is similar to the case for $[]_{V}$ above.
              \end{itemize}
    \end{enumerate}
    ...
    \begin{itemize}
        \item Suppose there exist $[e]$ such that $[e] = <_{c,\sfrac{X+Y}{\sim}^{\mu}}([e]) = [<_{c,X+Y}^{\mu}(e)]$ and $e = <_{c,X+Y}^{\mu}(e)$ because edges are not identified.
              This is not possible as $<_{c,X+Y}^{\mu}$ is irreflexive. We need not to check this requirement for nodes because a node can not be a predecessor.
        \item Let $e_1,e_2$ be edges. 
              Then, if $[e_1] <_{c,\sfrac{X+Y}{\sim}}^{\mu}([e_2])$ it must not be the case that $[e_2] <_{c,\sfrac{X+Y}{\sim}}^{\mu}([e_1])$.
              We have $[e_1] = [<_{c,X+Y}^{\mu}(e_2)]$ and if it was the case that $[e_2] <_{c,\sfrac{X+Y}{\sim}}([e_1])$ then $[e_2] = <_{c,X+Y}^{\mu}(e_1)$ which would mean that $<_{c,X+Y}$ is not assymetric.
              Once again, we do not need to check for nodes as a node can not be on the lefthand side of the relation.
        \item We need to show that if $[v] \in s([e_1])$ and $[e_2] <_{c,\sfrac{X+Y}{\sim}}^{\mu}([e_1])$ then $[e_2] <_{c,\sfrac{X+Y}{\sim}}^{\mu}([v])$.
              We have $e_2 = <_{c,X+Y}^{\mu}(e_1)$ and $[v] \in [s(e_1)]^{*}$ meaning 
              that there exists $v' \in s(e_1)$ such that $[v] = [v']$.
              \begin{itemize}
                        \item $v' = v$. Then, given $e_2 = <_{c,X+Y}^{\mu}(e_1)$ it must be the case that $e_2 = <_{c,X+Y}^{\mu}(v)$ and $[e_2] = [<_{c,X+Y}^{\mu}(v)] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}([v])$.
                        \item $v' \sim v$ and there exists $z \in V_{Z}$ such that $v = f_{V};i_{1,V}(z)$ and $v' = g_{V};i_{2,V}(z)$ and $v' \in s(e_1)$ and because $e_2 <_{c,X+Y}^{\mu}(e_1)$ it must be the case that $e_2 <_{c,X+Y}^{\mu}(v')$. This also means that necessarily $[g_{V}(z)) \not = \varnothing$ and $[f_{V}(z)) = \varnothing$.
                              If $[f_{V}(z)) = \varnothing$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}([v]) = [g_{V};<_{c,Y};i_{2,E}(z)] = [<_{c,Y}^{\mu};i_{2,E}(g_{V}(z))] = [<_{c,X+Y}^{\mu}(i_{2,V}(g_{B}(z)))] = [<_{c,X+Y}^{\mu}(v')] = [e_2]$.
                        \item Case when $v' = f_{V};i_{1,V}(z)$ is symmetric.
               \end{itemize}
            \end{itemize}
