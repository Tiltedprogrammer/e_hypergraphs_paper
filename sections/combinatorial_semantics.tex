...
\begin{definition}[E-hypergraph]
    ...
    \begin{enumerate}
        ...
        \item For each $v \in V_{\mathcal{G}}$ $v \consistency^{\mu} v$
        \item For each $e \in E_{\mathcal{G}}$ $e \consistency^{\mu} e$
        \item If $v \in s(e)$ then $v \consistency^{\mu} e$
        \item If $v \in t(e)$ then $v \consistency^{\mu} e$
        \item The relation is symmetric and transitive
...
\begin{definition}[Pushout in $\catname{EHyp_{\Sigma}}$]
    ...
    We have the only case of $u$ having no pre-image in $E_{Z}$ because it is empty.
              ...
              \end{enumerate}
    \update{We will also build this relation in steps, first we do steps for nodes (1) - (3), then steps (1) - (2) for edges and finally, step (4) for nodes.}

    \begin{remark}
        Essentially, what we build above is that if the relation is undefined, it is inherited from either incident nodes or edges.
    \end{remark}
    ...
    We then define the new relation for edges as
    \begin{enumerate}
        \item If $\consistency_{X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e)) \not = \varnothing$, then
                \begin{align*}
                    \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{E_{X} + E_{Y}}([e]_{E})) =
                    (\langle []_{V}^{\consistency}, []_{E}^{\consistency}\rangle^{*}(\consistency_{X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e))))^{c}
                \end{align*}
        \item $\consistency_{X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e)) = \varnothing$ and there exists $[v]$ in $s([e])$ or $t([e])$ such that $\consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]) \not = \varnothing$, then
        \[
            \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{E_{X} + E_{Y}}([e]_{E})) = \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{V_{X} + V_{Y}}([v]_{V}))
        \]
    \end{enumerate}
    ...
    \[
        \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{V_{X} + V_{Y}}([v]_{V})) = (\langle []_{V}^{\consistency}, []_{E}^{\consistency} \rangle^{*}(\bigcup_{v' \in V_{X} + V_{Y} | [v']_{V} = [v]_{V}} \consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v'))))^{c}
    \]
    \item If for all $v' \in V_{X+Y}$ such that $v' \sim v$, $\consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v')) = \varnothing$ and there exists $e \in E_{X+Y}$ such that either $[v] \in s([e])$ or $[v] \in t([e])$ and $\consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) \not = \varnothing$, then
    \[
        \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{V_{X} + V_{Y}}([v]_{V})) = \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{E_{X} + E_{Y}}([e]_{E}))
    \]
    \end{enumerate}
        where $i_*$ are injections into $\sfrac{V_{X} + V_{Y}}{\sim} + \sfrac{E_{X} + E_{Y}}{\sim}$ and $c$ is a reflexive, symmetric and transitive closure.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item  Assume that $v_1, v_2 \in V_{X+Y}$ and $v_1 \sim v_2$ then $\consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{E_{X} + E_{Y}}[v_1]) = \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{E_{X} + E_{Y}}[v_2])$.
              ...
              \begin{align*}
                \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{V_{X} + V_{Y}}([v_1])) = (\langle []_{V}^{\consistency}, []_{E}^{\consistency} \rangle^{*}(\bigcup_{v_i \in \{v_1, v_2\}} \consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_i))))^{c}
              ...
              Similarly,
              \[
                \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{V_{X} + V_{Y}}([v_2])) = (\langle []_{V}^{\consistency}, []_{E}^{\consistency} \rangle^{*}(\bigcup_{v_i \in \{v_1, v_2\}} \consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_i))))^{c}
              \]
              and we get well-definedness on the nose.
              We do not need to check well-definedness for edges as $e_1 \sim e_2$ implies $e_1 = e_2$.
            \item Now suppose that both $\consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_1)) = \varnothing$ and $\consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_2)) = \varnothing$ and there exists $\consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) \not = \varnothing$ such that $[v_1] \in s([e])$ (and automatically $[v_2] \in s([e])$).
                Then,
                  \[
                    \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2]))
                  \]
            \end{itemize}
        \item Putting $\phi = ([]_{V},[]_{E})$ into the definition of a homomorphism gives us
            ...
            By definition, 
            \[
                \consistency_{\sfrac{X+Y}{\sim}}^{\mu}([]_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}}(x)) = (\langle []_{V}^{\consistency}, []_{E}^{\consistency} \rangle^{*} (\bigcup_{x' \in V_{X} + V_{Y} | [x']_{V} = [x]_{V}} \consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(x))))^{c}
            \]
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                It can not be the case that the node has no pre-image as $[[v_i]) \not = \varnothing$ and $[e) = \varnothing$ implies that step (4) is needed but we only completed steps (1)-(3).
            \end{itemize}
            ...
            \begin{itemize}
                \item $v'_1$ has a pre-image $f_{V};i_{1,V}(z)$ and hence $[f_{V}(z)) = \varnothing$ and $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'_1])) = [i_{2,E}(e_{y})]$.
                      ...
                      \[
                        <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'_1])) = [i_{2,E}(e_{y})] = [i_{2,E}(e'_{y})] = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'_2])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2]))
                      \]
            \end{itemize}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item $v \in s(e_1)$ and $v \in s(e_2)$.
                  ...
                  \begin{itemize}
                    ...
                    \item $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [i_{1,E}(e_{x})]$ and 
                          ...
                          $e_1$ and $e_2$ should both have pre-image in either $E_{Y}$ or $E_{X}$. The only possible case then is that $v_2 \in s(e_2)$ and $v'_1 \in s(e_1)$ and $v_1 \sim v'_1$.
                          Then $[v_1') = \varnothing$ and $[v_2) = \varnothing$. Show it is not possible.
                          \begin{itemize}
                            \item $v_1 = f_{V};i_{1,V}(z)$ and $[f_{V}(z)) \not = \varnothing$.
                                  Then, necessarily, $v'_1 \in s(e_1)$ and $v_1 \sim v'_1$ and $v'_1 = g_{V};i_{2,V}$ and hence $e_1$, $e_2$ have pre-image in $E_{Y}$.
                                  If $v_2 \in s(e_2)$ then it should have a pre-image in $V_{Y}$ and it cant be the case that $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}([v_2])}) = [i_{2,E}(e_y)]$.
                    ...
                    \item Other cases are not possible, for example, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X}+V_{Y}}{\sim}}([v_1])) = [i_{1,E}(e_{x})]$ and $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X}+V_{Y}}{\sim}}([v_2])) = [<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}})(v_2)]$
                          would mean that $e_2$ has a predecessor because $v_2$ does.
                  \end{itemize}
            ...
            \item $v' \in s(e_1)$ and $v'' \in s(e_2)$ and $v' \sim v$, $v'' \sim v$. 
                  The last bit also means that $v'' \sim v'$, but because $f$ and $g$ are mono we have at most 2 elements in each equivalence class.
        \end{itemize}
    \end{proof}
\end{lemma}
                    ...
                    \item If $<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1)) = e_3$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = [<_{c,X+Y}^{\mu}(i_{E_{X} + E_{Y}}(e_1))] = [e_3] = [e_2]$ which means that $e_2$ is the predecessor of $e_1$.
                          ...
                          \begin{itemize}
                            ...
                            \item $v \sim v'$ which means there exists $z \in V_{Z}$ such that $v = f_{V};i_{1,V}(z)$ and $v' = g_{V};i_{2,V}(z)$ and $v' \in s(e_1)$.
                                  ...
                                  \]
                            \end{itemize}
                    \item $[e_1) = \varnothing$ and for some $[v_i] \in s([e_1]), [v_j] \in t([e_1])$ $[[v_i]) \not = \varnothing,\; [[v_j]) \not = \varnothing$. 
                    Then, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])) = [e_2]$.
                    We now need to check that $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
                    \begin{itemize}
                        \item If $v$ has no pre-image in $V_{Z}$ and $<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v)) = \varnothing$, then, by definition
                              \[
                                <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))
                              \]
                    \end{itemize}
                    We have that $[v] \in s([e_1])$ and $[v'] \in s([e_1])$ (or $[v'] \in t([e_1])$). 
                    This means there exists $v_1$ such that $[v] = [v_1]$ and $v_1 \in s(e_1)$ and there exists $v_2$ such that $[v_2] = v'$ and $v_2 \in s(e_1)$.
                    \begin{itemize}
                        \item $v = v_1$ and $v \in s(e_1)$ and $v' = v_2$ and $v' \in s(e_1)$. 
                              Because $[e_1) = \varnothing$, then $[v) = \varnothing$ and $[v') = \varnothing$ because the relation respects connectivity.
                              \begin{itemize}
                                \item If $v = f_{V};i_{1,V}(z)$ then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [i_{2,E}(e_y)]$.
                                      If $v' = f_{V};i_{1,V}(z')$ then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [i_{2,E}(e'_y)]$.
                                      We then have $e'_y = e_y$ because by assumption all nodes from $V_{Z}$ have the same immediate predecessor under $g$.
                                \item If $v = f_{V};i_{1,V}(z)$ then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [i_{2,E}(e_y)]$ and $v'$ has no pre-image in $V_{Z}$ then there necessarily exists an incident edge $[e']$ such that ...
                                \item                                        
                              \end{itemize} 
                    \end{itemize}
                  \end{itemize}
...
\begin{lemma}
    ...
    The properties (1),(2),(5) are satisfied by construction.
    Hence we need to check wether properties (3),(4) and (6) are satisfied.
    \begin{proof}
        \begin{itemize}
            \item Suppose $[v] \in s([e])$, then it must be the case that $i_{\sfrac{E_{X} + E_{Y}}{\sim}}([e]) \in \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
                  $[v] \in s([e])$ means there exists $v' \in s(e)$ such that $[v] = [v']$.
                  We then have two cases.
                  \begin{itemize}
                    \item $v = v'$ and then $v \in s(e)$. 
                    This also means that $i_{E_{X} + E_{Y}}(e) \in \consistency_{X+Y}(i_{V_{X} + V_{Y}}(v))$.
                    By applying $([]_{V},[]_{E})$ we get 
                    \[
                       i_{\sfrac{E_{X}+E_{Y}}{\sim}}([e]) \in \langle []_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}},[]_{E};i_{\sfrac{E_{X} + E_{Y}}{\sim}}) \rangle^{*}(\consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v))) \subseteq \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}[v])
                    \]
                        \item $v' \sim v'$ and $v' \in s(e)$ and hence $i_{E_{X} + E_{Y}}(e) \in \consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v'))$.
                        By applying $([]_{V},[]_{E})$ we get 
                        \[
                        i_{\sfrac{E_{X}+E_{Y}}{\sim}}([e]) \in \langle []_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}},[]_{E};i_{\sfrac{E_{X} + E_{Y}}{\sim}}) \rangle^{*}(\consistency_{X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v'))) \subseteq \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}[v'])
                        \]      
                        ...
                        and hence
                        \[
                            i_{\sfrac{E_{X}+E_{Y}}{\sim}}([e]) \in \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}[v])
                        \]
                  \end{itemize}
                  \item The case when $[v] \in t([e])$ is symmetric.
                  \item If $i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1]) \in \consistency_{\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2]))$ then $[[v_1]) = [[v_2])$.
                        The latter means that either $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X}+V_{Y}}{\sim}}([v_1])) = <_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X}+V_{Y}}{\sim}}([v_2]))$ or both functions are undefined.
                        By definition,
                        \[
                        ...
                        \]
                        which means that either $v_1 \consistency_{X+Y}^{\mu} v_2$ or $v'_1 \consistency_{X+Y}^{\mu} v_2$ and $v'_1 \sim v_1$.
                        \begin{itemize}
                            \item $v_1 \consistency_{X+Y}^{\mu} v_2$ implies $[v_1) = [v_2)$.
                            \begin{itemize}
                                \item If $v_1$ does not have a pre-image in $V_{Z}$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_1))]$
                                \begin{itemize}
                                    \item If $v_2$ does not have a pre-image in $V_{Z}$, then $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = [<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_2))]$ and by assumption $<_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_1)) = <_{c,X+Y}^{\mu}(i_{V_{X} + V_{Y}}(v_2))$ and hence their images are also equal.
                                    \item If $v_2$ has an image $f_{V};i_{1,V}(z)$ and $[f_{V}(z)) = \varnothing$ and hence $[v_2) = \varnothing$ and $[v_1) = \varnothing$.
                                          Then, $<_{c,\sfrac{X+Y}{\sim}}^{\mu}(i_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = [i_{2,E}(e_{y})]$.
                                \end{itemize}
                            \end{itemize}
                        \end{itemize}
        \end{itemize}
