...
First, a coproduct of $X$ and $Y$ is taken which is $X + Y = \{V_{X} + V_{Y}, E_{X} + E_{Y}, s_{X} + s_{Y}, t_{X} + t_{Y} \}$.
Then, consider a relation 
\[
    S = \{(x_i,y_j), (y_j, x_i) \in (X+Y) \times (X+Y)\; | \; \exists z \in Z \; . \; x_i = f(z) \text{ and } y_j = g(z) \text{where $x_i \in X$ and $y_j \in Y$ }\}
% \cup \{(x,x) \in (X+Y) \times (X+Y) \text{ where $x \in X$}\} \cup \{(y,y) \in (X+Y) \times (X+Y) \text{ where $y \in Y$}\}$ 
\]
and let relation $R$ be its reflexive and transitive closure. $R$ is then an equivalence relation since $S$ is symmetric.

A quotient of $X+Y$ by this relation denoted by 
\[
    \sfrac{X+Y}{\sim R} = \{\sfrac{V_{X} + V_{Y}}{\sim R}, \sfrac{E_{X} + E_{Y}}{\sim R}, \sfrac{s_{X} + s_{Y}}{\sim R}, \sfrac{t_{X} + t_{Y}}{\sim R}\}
\]
where $\sfrac{s_{X} + s_{Y}}{\sim R} : \sfrac{E_{X} + E_{Y}}{\sim R} \to \sfrac{V_{X} + V_{Y}}{\sim R}$ now maps equivalence classes to equivalence classes.
...
To show that $\sfrac{X + Y}{\sim R}$ is a pushout we need to argue that for each $Q$ and morphisms $j_1$ and $j_2$ such that the above square commutes there is a unique $u$ such that $j_1 = \sfrac{i_1}{\sim R} ; u$ and $j_2 = \sfrac{i_2}{\sim R} ; u$.
We will define such $u$ as $u([x]) = j_1(x)$ and $u([y]) = j_2(y)$ and claim that is it a well-defined function by showing that if $t_1 = t_2$ then $u(t_1) = u(t_2)$.
There are three options
\begin{itemize}
    \item $t_1 = [x]$ and $t_2 = [x]$, where $[x]$ is a singleton equivalence class.
          The property then holds on the nose.
    \item $t_1 = [y]$ and $t_2 = [y]$, where $[y]$ is a singleton equivalence class.
    The property then holds on the nose.
    \item $t_1 = [x]$ and $t_2 = [y]$ and there exists at least one $z$ in $Z$ such that $x = f(z)$ and $y = g(z)$.
          By applying $j_1$ to the first we get $j_1(f(z)) = j_1(x)$ and similarly for $j_2$: $j_2(g(z)) = j_2(y)$ and $j_1(x) = j_2(y)$ by commutativity.
          Hence, $u([x]) = u([y])$. 
\end{itemize}
...
\begin{remark}
    ...
    \[
    \sfrac{(<_{c,X} + <_{c,Y})}{\sim R} \subseteq \sfrac{(V_{X} + V_{Y} + E_{X} + E_{Y})}{\sim R} \times \sfrac{(E_{X} + E_{Y})}{\sim R} \to \sfrac{(E_{X} + E_{Y})}{\sim R}
    \]
