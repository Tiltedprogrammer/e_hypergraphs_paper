...
Next $\sfrac{i_{1,V}}{\sim R_{V}} : V_{X} \to \sfrac{V_{X} + V_{Y}}{\sim R_{V}} = i_{1,V};s'$ where $s' : V_{X}+V_{Y} \to \sfrac{V_{X} + V_{Y}}{\sim R_{V}}$.
and 
$\sfrac{i_{1,E}}{\sim R_{E}} : E_{X} \to \sfrac{E_{X} + E_{Y}}{\sim R_{E}} = i_{1,E};s'$, similarly for $\sfrac{i_{2,E}}{\sim R_{E}}$ and $\sfrac{i_{2,V}}{\sim R_{V}}$.
We need to show
\begin{enumerate}
...
$s_{X};(\sfrac{i_{1,V}}{\sim R_{V}})^{*}(e) = s_{X};i_{1,V}^{*};s'^{*}(e)$ which is a homomorphism because $i_{1,V}$ and \update{$s'$ are}.
