...
\begin{remark}
    ...
    We then quotient $X + Y$ by $R_{V},R_{E}$ and claim that it is a pushout.
    \[
        \sfrac{X + Y}{\sim (R_{V},R_{E})} = \{\sfrac{V_{X} + V_{Y}}{\sim R_{V}}, \sfrac{E_{X} + E_{Y}}{\sim R_{E}}, \sfrac{s_{X+Y}}{\sim (R_{V},R_{E})}, \sfrac{t_{X,Y}}{\sim ()}, \sfrac{\#_{X+Y}}{\sim (R_{V},R_{E})}, \sfrac{<_{c,X + Y}}{\sim (R_{V},R_{E})} \}    
    \]
    ...
    \[
    \sfrac{(<_{c,X+Y})}{\sim} \subseteq \sfrac{(V_{X} + V_{Y})}{\sim R_{V}} + \sfrac{(E_{X} + E_{Y})}{\sim R_{E}} \times \sfrac{(E_{X} + E_{Y})}{\sim R_{E}}
    \]
    such that for each $(x_1, x_2) \in <_{c, X+Y}$ $\sfrac{<_{c,X+Y}}{\sim}$ contains $([x_1],[x_2])$.
