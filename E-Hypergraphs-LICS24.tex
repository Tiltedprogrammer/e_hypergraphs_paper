...
The correspondence of the previous section further extends to include the equations of a monoidal theory: the equations of a theory are considered as rewrite rules on string diagrams,  which are then modelled as DPO-rewrites on hypergraphs with interfaces. 
...
To mimic rewrites in $\textbf{PROP}(\Sigma)$ with the help of DPOI rewriting in $\catname{Hyp_{\Sigma}}$, we need to put some restrictions both on pushout complement and on matching as not all of them correspond to valid rewrites in $\textbf{PROP}(\Sigma)$. CHRIS: "LEAD TO"? BE PRECISE! :) 
...
\begin{definition}
Given $G \xleftarrow{} n+m$ and $H \xleftarrow{} n + m$ in $\catname{Hyp_{\Sigma}}$, $G$ rewrites convexly into $H$ with interface $n + m$ --- notation $(G \xleftarrow{} n + m ) \Rrightarrow_{\mathcal{R}}  (H \xleftarrow{} n + m )$ --- if there exist rule $L \xleftarrow{} i + j \xrightarrow{} R$ in the set of rewrite rules $\mathcal{R}$ and object $C$ and cospan arrows $i+j \xrightarrow{} C \xleftarrow{} n+m$ such that the DPOI diagram above commutes and its marked squares are pushouts and the following conditions hold
...
By resorting to convex matching and boundary complement we virtually require that the original cospans above are monogamous, e.g., $n \xrightarrow{} G \xleftarrow{} m$.
...
\]
We will denote the unique immediate predecessor $e$ of $x$ by writing $e <^{\mu} x$.
...
The consistency relation must satisfy the following properties. 
    \begin{enumerate}
        ...
        \item If $v \in s(e)$ then $v \consistency e$
        \item If $v \in t(e)$ then $v \consistency e$
...
\end{definition}
Condition $(3)$ on the child relations says that $<^{\mu}$ respects connectivity.
