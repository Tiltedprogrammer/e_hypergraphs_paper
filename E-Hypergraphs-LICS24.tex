...
The former means that rewrite rules become of the form
\[
\mathcal{L} \xleftarrow{} i' + j' \xleftarrow{} i + j \xrightarrow{} i'' + j'' \xrightarrow{} \mathcal{R}
\] where we use the fact that cospans 
\[
...
\]
encode the same data and we will further omit the arrows $0 \to 0 \to \mathcal{L}$.
That is, a rewrite rule is a pair of cospans of e-hypergraphs with matching \textit{external} interfaces and arbitrary \textit{internal} interfaces.
...
\]
not necessarily coincide with the internal interfaces of the initial cospan.
...
To account for such changes in the interfaces we require additional structure on DPOI rewriting for $\catname{EHyp({\Sigma})}$ which is summarised in the definition below.
...
\]
\end{definition}
To guarantee that the rewriting yields a monogamous directed acyclic e-hypergraph we also need to introduce a revised version of a boundary complement.
...
\[
    i + j \to \mathcal{L}^{\bot} \to G^{\urcorner}
\] 
...
is a \textit{boundary} complement if $[c_1,c_2]$ is mono 
 and there exist $d_1 : n \to C$ and $d_2 : k \to C$ 
...
Then there is a morphism $n \to l$ because there is a morphism $n \to n'$ and the external interfaces of $n'$ and $l$ are the same.
Similarly for $k \to r$.
\end{definition}
...
The conditions (3)-(4), in particular, prohibit a match $\{f \mapsto f, g \mapsto g\}$ in $\mathcal{L} = f \otimes g$ and $\mathcal{G} = f + g$.
...
It will be analogous to the one from Definition~\ref{def:convex_dpo} but again, to make sure the resulting cospan of e-hypergraph is monogamous we will have to construct internal interfaces separately.
