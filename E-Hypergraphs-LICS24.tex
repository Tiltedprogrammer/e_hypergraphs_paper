...
The \emph{in-degree} of a vertex $v$ in a hypergraph $\mathcal{G}$ is the number of hyperedges $e \in {E_\mathcal{G}}$ such that $v \in s(e)$.  Similarly, the out-degree of $v$ is the number of edges $e \in E_\mathcal{G}$ such that $v \in t(e)$.
...
Then, given $\mathcal G \xleftarrow{} n+m$ and $\mathcal H \xleftarrow{} n + m$ in $\catname{Hyp(\Sigma)}$, $\mathcal G$ rewrites convexly into $\mathcal H$ with interface $n + m$ --- notation $(\mathcal G \xleftarrow{} n + m ) \Rrightarrow_{\mathfrak{R}}  (\mathcal H \xleftarrow{} n + m )$ --- if there exist rule $\mathcal L \xleftarrow{} i + j \xrightarrow{} \mathcal R$ in $\mathfrak{R}$ and object $C$ and cospan arrows $i+j \xrightarrow{} \mathcal C \xleftarrow{} n+m$ such that the DPOI diagram of Definition \ref{def:dpoi} commutes and its marked squares are pushouts 
...
These edges can be designated as "parents" to sub-e-hypergraphs,  which interpret the various components of the e-boxes.  These components are distinguished by the consistency relation, which partitions the maximal sub-e-hypergraph below the hierarchical edge.  
...
\begin{definition}[Category of e-hypergraphs with interfaces]
    The category of \textit{e-hypergraphs with extended interfaces} $\Ecospans$ has discrete e-hypergraphs as objects,  with homsets $\Ecospans(n,m)$ consisting of isomorphism classes of \textit{extended cospans}, defined as follows:  
    \[
    n \xrightarrow{f_{ext}} n' \xrightarrow{f_{int}} \mathcal{G} \xleftarrow{g_{int}} m' \xleftarrow{g_{ext}} m
    \]
    where $\mathcal{G}$ is an e-hypergraph,  and $n, n', m, m'$ are discrete e-hypergraphs,  $f_{ext},f_{int},g_{ext},g_{int}$ are monomorphisms in $\catname{EHyp(\Sigma)}$,  and the image of $f_{ext};f_{int}$ and of $g_{ext};g_{int}$ consist exclusively of top-level vertices,  and that that vertices in the strictly internal interface (defined below) are not top-level.  We will sometimes write $\mathcal G$ to denote the extended cospan,  where it is clear from context $\mathcal G$ is equipped with extended interfaces. 
...
Given an edge $e \in E_\mathcal{G}$ such that $l(e) = \bot$,  we call the \textit{inputs of $e$} the intersection of the strictly internal input interface of $\mathcal{G}$ with the immediate successors of $e$,  and analogously for the \emph{outputs of $e$}.  
    ...
    Two extended cospans are isomorphic if there exist isomorphisms $\alpha$, $\beta$ and $\gamma$ making the following diagram commute.
    \[
    \scalebox{0.75}{
...
Unlike in the previous section,  in order to give an equivalence with $\textbf{PROP}^+(\Sigma)$,  we must first develop the theory of DPOI rewriting for e-hypergraphs.  This is beacuse the equations for semilattice enrichment are not intended to be subsumed by the e-hypergraph representation.  Instead,  we will treat them via rewriting,  as we do with equations of the signature.  This is because the semilattice equations involve sharing and copying, operations on string diagrams which are not usually quotiented away, cf. string diagrams for Cartesian categories.  An ideal combinatorial representation would factor out the three semilattice equations which \textit{do not} involve sharing and copying (namely,  associativity, commutativity, and idempotence) --- however, a limitation of the current work is that we do not achieve this.  Presently,  we will treat these equations via rewriting as well. 
...
\begin{remark}
There is a limitation of extended DPOI rewriting as defined in this section: it is unable to express rewriting
for rules whose target contains an edge with no incident vertices.  
While this limitation can be overcome,  we find the technical development to be significantly simpler if we simply avoid consideration of such edges.  
For this reason,  in the remainder of the paper we consider signatures which do not contain $c: 0 \to 0$,  thus restricting possible e-hypergraphs to those without such problematic edges. 
...
\] and mono $f : \mathcal{L} \to \mathcal{G}$ in $\catname{EHyp(\Sigma)}$, a pushout complement $i + j \to \mathcal{L}^{\bot} \to \mathcal{G}$
...
Intuitively the above definition means that when the occurence of $\mathcal{L}$ within $\mathcal{G}$ is top-level then $\mathcal{L}$'s external interfaces become the part of $\mathcal{L}^{\bot}$'s external interfaces and when the occurence is nested, those interfaces become the part of $\mathcal{L}^{\bot}$'s strict internal interfaces.
In both cases when removing the occurence of $\mathcal{L}$ from $\mathcal{G}$, $\mathcal{L}$'s strict internal interfaces are removed from strict internal interfaces of $\mathcal{G}$ to form internal interfaces of the corresponding mda-cospans above.
...
More precisely, when removing the occurence of $\mathcal{L}$ from $\mathcal{G}$, \emph{i.e.}, by computing the pushout complement, the internal interfaces of the resulting e-hypergraph should be modified to \textit{exclude} the vertices corresponding to $\mathcal{L}$'s strictly internal interfaces (since the vertices they map to have been removed). 
...
such that the two marked squares are pushouts and the following conditions hold:
	...
	\]
	\item the map $f_1 = [g_1,h_1]: n'' \to \mathcal H$ in the diagram above consists of $g_1$ as defined in Definition \ref{def:boundary_new} of boundary complement,  and $h_1$ which is the restiction of the composite $i'' \to \mathcal R \to \mathcal H$ to $i'' \setminus i$,  and similarly for the map $f_2: k'' \to \mathcal H$.  
    \end{enumerate}
...
\end{definition}
The boundary complement conditions, in particular, prohibits finding a match for $\mathcal L$ in $\mathcal G$,  below.  
\[
	\mathcal{L} = \scalebox{0.8}{\tikzfig{combinatorial_semantics/f_times_g}} \qquad \mathcal{G} = \scalebox{0.6}{\tikzfig{combinatorial_semantics/f_plus_g_inline}}
\]
...
\end{align*}
where $c \in \Sigma$ and $+$ in $\WellTypedMdaEcospans$ is defined by Figure \ref{fig:A+B}.  It uses the notation of dashed blue boxes for hierarchical edges, with its children drawn inside,  and maximally consistent components seperated by another dashed line.  We will omit the subscripts in $\llbracket - \rrbracket_{n,m}$ where it is clear from context. 
\end{definition}
...
We define the appropriate notion of quotient by EDPOI rewrites below.  Let the function $\llbracket - \rrbracket$ extend in the obvious way to apply to equations (\textit{i.e.}, pairs of $\Sigma^+$-terms). 
...
\end{proposition}    
The proof follows the familar argument (\textit{i.e.},  termination of distributivity rewrites) that $\Sigma^+$-terms can be rewritten via the semilattice-enrichment equations into a similar normal form.  
...
\]
Observe that $\llbracket f_1 \rrbracket + \ldots + \llbracket f_n \rrbracket$ and $\llbracket g_1 \rrbracket + \ldots + \llbracket g_m \rrbracket$ are weak normal forms.  We consider two cases: where $n=m$ and where $n\neq m$.  In the first case,  it is immediate that $\llbracket f \rrbracket \neq \llbracket g \rrbracket $,  since equal terms have weak normal forms with the same number of components.  Otherwise,  if $n = m$,  assume for contradiction that $\llbracket f \rrbracket = \llbracket g \rrbracket$.  We have that there exists a permutation $\sigma$ such that for every index $i$ we have $\llbracket f_i \rrbracket \neq \llbracket g_{\sigma(i)} \rrbracket $,  and thus by  $(*)$ that $f_i = g_{\sigma(i)}$,  implying that $f = g$ which is a contradiction.  Therefore $\llbracket f \rrbracket \neq \llbracket g \rrbracket$ as required. 
...
\begin{remark}
    Both $<$ and $\consistency$ can be considered as functions defined on $V_{\mathcal{F}} + E_{\mathcal{F}}$, \textit{i.e.}, on the coproduct of vertices and edges.
...
\begin{proof}
    ...
    % &\}
    % \end{align*}
and let relations $R_{V},R_{E}$ be their reflexive symmetric and transitive closures respectively.
...
We then quotient the set vertices and edges in $X + Y$ by $R_{V}$ and $R_{E}$ respectively.
    \begin{align*}
        ...
        & \}    
    \end{align*}
We will refer to $\sim (R_{V},R_{E})$ just as $\sim$, \textit{i.e.}, by writing $\sfrac{V_{X} + V_{Y}}{\sim}$ and concrete relation will be clear from the context.
We have 
\[
...
Recall that $<$ is essentially a transitive clousure of $<^{\mu}$ and hence we can first define 
    ...
    \begin{enumerate}
        ...
        %       The last requirement means that there necessarily exist $x = <_{X}^{\mu}(f_{E}(z)) = f_{E}(<_{Z}^{\mu}(z))$ and $y = <_{Y}^{\mu}(g_{E}(z)) = g_{E}(<_{Z}^{\mu}(z))$.
        %       We then define $<_{\sfrac{X+Y}{\sim}}^{\mu}([u]) = [<_{X+Y}^{\mu}(u)]$
        \item $u$ has a pre-image $z$ in $V_{Z}$ (by discreteness, $[z) = \varnothing$) and $[f_{V}(z)) = \varnothing$, $[g_{V}(z)) \not = \varnothing$.
              The last condition means that there exists $e_y = <_{Y}^{\mu}(\iota_{V_{Y}}(g_{V}(z)))$.
              We then define 
              \[
                <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[u]) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)] = [\iota_{2,E}(e_y)]
              \]
        \item $u$ has a pre-image $z$ in $V_{Z}$, (by discreteness, $[z) = \varnothing$) and $[f_{V}(z)) \not = \varnothing$ $[g_{V}(z)) = \varnothing$.
              The one before the last condition means that there exists $e_x = <_{X}^{\mu}(\iota_{V_{X}}(f_{V}(z)))$.
              We then define 
              \[
                <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[u]) = [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)] = [\iota_{1,E}(e_x)]
              \]
        \item $u$ has no pre-image in $V_{Z}$ and $<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(u)) = e'$
        \[
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[u]) = [<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(u))] = [e']
        \]
        \item $u$ has no pre-image in $V_{Z}$ and $<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(u))$ is undefined (\textit{i.e.}, $[u) = \varnothing$). 
              If there exists $[v]$ such that $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [e']$ and such that there is an \textit{undirected} path from $[u]$ to $[v]$, then we define
              \[ 
    ...
    Now, assume that $u$ is an element of $E_{X+Y}$.
    We have the only case of $u$ having no pre-image in $E_{Z}$ because it is empty.
    Consider two cases.
    \begin{enumerate}
    \item  $<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(u)) = u_1$.
            Then we define
        \[
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}[u]) = [<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(u))]
        \]
    \item $[u) = \varnothing$ and there exists $[v]$ such that $<_{\sfrac{X+Y}{\sim}}^{\mu}([v]) = [e']$ and such that there is an undirected path from $[u]$ to $[v]$, we define 
        \[
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}[u]) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v])
        \] 
    \end{enumerate}
    ...
    Let's show that the definition does not depend on a particular choice of $[v]$ in (4).
    Suppose, $[u) = \varnothing$ and there exist $[v_1]$ and $[v_2]$ such that $<_{\sfrac{X+Y}{\sim}}^{\mu}([v_1]) = [e_1]$ and $<_{\sfrac{X+Y}{\sim}}^{\mu}([v_2]) = [e_2]$ and $[e_1] \not = [e_2]$ and such that there is a path from $[u]$ to $[v_1]$ and from $[u]$ to $[v_2]$.
    First, both $v_1$ and $v_2$ should have a pre-image in $V_{Z}$, otherwise it would mean that there is a path from, \textit{e.g.},  $u$ to $v_1$ and that $[v_1) \not = \varnothing$ which would imply that $[u) \not = \varnothing$ and we get contradiction.
    Then consider sevaral cases.
    \begin{itemize}
        \item $v_1 = f_{V};\iota_{1,V}(z_1)$ and $v_2 = f_{V};\iota_{1,V}(z_2)$.
        \begin{itemize}
            \item $u$ has a pre-image in $V_{X}$. Then $[f_{V}(z_1)) = \varnothing$ and $[g_{V}(z_1)) \not = \varnothing$, similarly for $z_2$.
                  Hence, 
                  \[
                    <_{\sfrac{X+Y}{\sim}}^{\mu}([v_1]) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z_1)] = [e_1]
                  \]
                  and
                  \[
                    <_{\sfrac{X+Y}{\sim}}^{\mu}([v_2]) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z_2)] = [e_2]
                  \]
                  Then, $[e_1] \not = [e_2]$ implies
                  \[
                    [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z_1)] \not = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z_2)]
                  \] and because $[]_{E}$ and $\iota_{2,E}$ are injective, it further means that
                  \[
                    <_{Y}^{\mu}(g_{V};\iota_{V_{Y}}(z_1)) \not = <_{Y}^{\mu}(g_{V};\iota_{V_{Y}}(z_2))
                  \]
                  which contradicts assumption (3) from~\ref{pushout:assumptions}.
                  \item Case when $u$ has a pre-image in $V_{Y}$ is symmetric.
                \end{itemize}
                \item $v_1 = f_{V};\iota_{1,V}(z_1)$ and $v_2 = g_{V};\iota_{1,V}(z_2)$
                \begin{itemize}
                    \item $u$ has a pre-image in $V_{X}$. Then, $[f_{V}(z_1)) = \varnothing$, $[g_{V}(z_1)) \not = \varnothing$, $[f_{V}(z_2)) = \varnothing$, $[g_{V}(z_2)) \not = \varnothing$.
                          This contradicts the assumption (4) from~\ref{pushout:assumptions}.
                    \item Case when $u$ has a pre-image in $V_{Y}$ is symmetric.
                \end{itemize}
                \item The remaining two cases are symmetric.
    \end{itemize}

    Let's check that this is a well-defined function.
    Suppose $t_1, t_2$ are vertices and $t_1 \sim t_2$, then $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}+V_{Y}}{\sim}}[t_1]) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[t_2])$.
    $t_1 \sim t_2$ implies there is $z$ such that $t_1 = f_{V};\iota_{1,V}(z)$ and $t_2 = g_{V};\iota_{2,V}(z)$ and two cases
    \begin{itemize}
    ...
    \[
        <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[t_1]) = [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)]
    \]
    and
    \[
        <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[t_2]) = [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)]
    \]
    so we have well-defined-ness on the nose. 
    We do not need to check well-defined-ness for edges as $e_1 \sim e_2$ just implies $e_1 = e_2$ for edges.

    We also need to makes sure that $[]_{E} : E_{X} + E_{Y} \to \sfrac{E_{X} + E_{Y}}{\sim}$ and $[]_{V} : V_{X} + V_{Y} \to \sfrac{V_{X} + V_{Y}}{\sim}$ are homomorphisms with respect to $<^{\mu}$.
    \begin{enumerate}
         \item $[<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e))]_{E} = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([e]_{E}))$ for $e$ such that $[e) \not = \varnothing$.
              Note that by requiring $[e) \not = \varnothing$ we use the first definition for edges.
              This is homomorphic by definition. 
        \item $[<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v))]_{E} = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]_{V}))$.
              Let's make case analysis on $[v]_{V}$.
              \begin{itemize}
                \item $v$ has a pre-image $z$ in $V_{Z}$ such that $v = f_{V};\iota_{1,V}(z)$.
                    \begin{itemize}
                        \item If $[f_{V}(z)) = \varnothing$ and $[g_{V}(z)) \not = \varnothing$, then, because $\iota_{1,V}$ is a coproduct injection, it must be the case that $[\iota_{1,V}(f_{V}(z))) = \varnothing$ and we do not require any equalities for such $v$.
                        \item If $[f_{V}(z)) \not = \varnothing$ and $[g_{V}(z)) = \varnothing$, then,
                              by definition, we have 
                              \begin{align*}
                                <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{V_{X} + V_{Y}}[v]) &= [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)]\\ 
                                &= [\iota_{1,E}(<_{X}^{\mu}(f_{V};\iota_{V_{X}}(z)))]\\
                                &= [\iota_{1,E}(e_x)] ~.
                            \end{align*}
                            Because $\iota_{1,E}$ is a homomorphism, we also have 
                            \[
                                [<_{X+Y}^{\mu}(f_{V};\iota_{1,V};\iota_{V_{X} + V_{Y}}(z))] = [\iota_{1,E}(<_{X}^{\mu}(f_{V};\iota_{V_{X}}(z)))] = [\iota_{1,E}(e_x)] ~.
                            \]
                    \end{itemize}
                \item The case when $v = g_{V};\iota_{2,V}(z)$ is symmetric.
    ...
    \begin{enumerate}
        \item If $\consistency_{X+Y}(\iota_{E_{X} + E_{Y}}(e)) \not = \varnothing$, then
                ...
                \end{align*}
        \item $\consistency_{X+Y}(\iota_{E_{X} + E_{Y}}(e)) = \varnothing$ and there exists $[v]$ such that there is an undirected path from $[e]$ to $[v]$ and such that $\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) \not = \varnothing$, then
        \[
            \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e]_{E})) = \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{V_{X} + V_{Y}}([v]_{V}))
        \]
    \end{enumerate}
    ...
    \[
        \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]_{V})) = [[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v' \in V_{X} + V_{Y} | [v']_{V} = [v]_{V}} \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v')))
    \]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \]
              ...
              \begin{align*}
                \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = ([[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v_i \in \{v_1, v_2\}} \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v_i))))^{c}
              \end{align*}
              Similarly,
              \[
                \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = ([[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v_i \in \{v_1, v_2\}} \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v_i))))^{c}
              \]
    ...
    We will need to check that this a well-defined function, while homomorphism-ness follows from homomorphism-ness of $j_{1,V}$ and $j_{2,V}$.

    Let's say that $t_1 \sim t_2$ then we need to show that $u_{V}([t_1]) = u_{V}([t_2])$.
    Because $t_1 \sim t_2$ it means that there exists $z \in V_{Z}$ such that $t_1 = f_{V};\iota_{1,V}(z)$ and $t_2 = g_{V};\iota_{2,V}(z)$.
    Then, we have
    \[
...
\begin{corollary}
    ...
    \begin{align*}
         [v') \not = \varnothing,\\
         <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))
    \end{align*}
\end{corollary}
\begin{corollary}
...
\begin{corollary}
    ...
    If $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)) = \varnothing$ and $\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) \not = \varnothing$ then there exists $v'$ such that there is a path from $v$ to $v'$ and there exists $v'' \sim v'$ such that
    \[
        \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v'')) \not = \varnothing
    \]
    \[
        \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))
    \]
\end{corollary}

\begin{corollary}
    \label{corollary:consistency_edges}
    If $\consistency_{X+Y}(\iota_{E_{X} + E_{Y}}(e)) = \varnothing$ and $\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) \not = \varnothing$ then there exists $v$ such that there is a path from $e$ to $v$ and there exists $v' \sim v$ such that
    \[
        \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v')) \not = \varnothing
    \]
    \[
        \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))
    \]
\end{corollary}
...
\begin{proof}
    ...
    \begin{itemize}
      \item Assume $<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e)) = e_1$, then  $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = [e_1]$. 
            This would mean that $e_1 = e$ as $[]_{E}$ is injective, which would mean that the relation $<_{X+Y}^{\mu}$ is not irreflexive because $e_1$ is the predecessor of $e$.
      \item Assume $[e) = \varnothing$, then, by Corollary~\ref{corollary:edges} there exists $v$ such there is a path from $e$ to $v$ and there exists $v' \sim v$ and $[v') \not = \varnothing$ and therefore $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
            \begin{itemize}
              \item $v$ has a pre-image in $V_{Z}$ such that $v = f_{V};\iota_{1,V}(z)$ and since there is a path from $e$ to $v$ $[f_{V}(z)) = \varnothing$ and $v' = g_{V};\iota_{2,V}(z)$, $[g_{V}(z)) \not = \varnothing$.
                    Then, 
                    \[
                      <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)] = [e]
                    \]
                    ...
                    Contradiction.
              \item Case when $v = g_{V};\iota_{2,V}(z)$ is symmetric.
            \end{itemize}
    \end{itemize}                  
...
\label{lemma:child_assymetric}
    $<_{\sfrac{X+Y}{\sim}}^{\mu}$ is assymetric.
\end{lemma}
\begin{proof}
    ...
    \begin{itemize}
      \item If $<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_2)) = e_3$ then $[e_1] = [e_3]$ which yields $e_1 = e_3$.
            Having at the same time $[e_2] = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1]))$ gives us also two cases.
      \begin{itemize}
          \item $<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1)) = e_4$ then $[e_2] = [e_4]$ and $e_2 = e_4$.
          ...
          \item $[e_1) = \varnothing$ and then by Corollary~\ref{corollary:edges} there exists $v$ such that there is a path from $e_1$ to $v$ and there exists $v' \sim v$ such that $[v') \not = \varnothing$.
                Then, $[e_2] = [<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])]$.
                \begin{itemize}
                  \item If $v$ has a pre-image in $V_{Z}$ such that $f_{V};\iota_{1,V}(z)$ (and hence $e_1$ has a pre-image in $E_{X}$) then necessarily $[f_{V}(z)) = \varnothing$ (because $[e_1) = \varnothing$) and hence $[g_{V}(z)) \not = \varnothing$.
                        Then,
                        \[
                          [<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])] = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)] = [e_2]
                        \]
                  ...
                  \item Case when $v = g_{V};\iota_{2,V}(z)$ is symmetric to the above.
                \end{itemize}
          \end{itemize}
          \item $[e_2) = \varnothing$ and by Corollary~\ref{corollary:edges} there exits $v_2$ such that there is a path from $e_2$ to $v_2$ and there exists $v'_2 \sim v_2$ such that $[v'_2) \not = \varnothing$. 
                Then, 
                \[
                  <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}+E_{Y}}{\sim}}([e_2])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = [e_1]
                \]
                The case when $[e_1) \not = \varnothing$ is similar to the above, so we will just jump to the interesting case when 
                $[e_1) = \varnothing$ and
                \[
                  <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X}+E_{Y}}{\sim}}([e_1])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [e_2]
                \]
                The cases we have are the following.
                \begin{itemize}
                  \item $v_2 = f_{V};\iota_{1,V}(z_2)$ and necessarily $[f_{V}(z_2)) = \varnothing$ and $v'_2 = g_{V};\iota_{2,V}(z_2)$.
                  Then, 
                  \[
                    <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z_2)] = [e_1]
                  \] which also implies that $e_1$ has a pre-image in $E_{Y}$.
                  \begin{itemize}
                      \item $v_1 = g_{V};\iota_{2,V}(z_1)$ (because there is a path from $e_1$ to $v_1$) and necessarily $[g_{V}(z_1)) = \varnothing$ (because $[e_1) = \varnothing$) and $v'_1 = f_{V};\iota_{1,V}(z_1)$.
                      Then,
                      \[
                        <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z_1)] = [e_2]
                      \]  
                      However, it means that $[f_{V}(z_2)) = \varnothing$ and $[f_{V}(z_1)) \not = \varnothing$ which violates assumption (3) in Theorem~\ref{th:existence_of_pushouts}.
                  \end{itemize}
                  \item Case when $v_2 = g_{V};\iota_{2,V}(z_2)$ is symmetric.
                \end{itemize}
    \end{itemize} 
...
\end{lemma}
\begin{proof}
        We need to show that if $[v] \in s([e_1])$ and $[e_2] = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1]))$ then $[e_2] = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v])$.
        We have two cases.
                  \begin{itemize}
                    \item If $<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1)) = e_3$, then $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = [<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1))] = [e_3] = [e_2]$ which means that $e_2$ is the predecessor of $e_1$.
                          ...
                          \begin{itemize}
                            \item $v = v'$ then $v \in s(e_1)$ and because $e_2$ is the predecessor of $e_1$ it is also a predecessor of $v$ because $X+Y$ is an e-hypergraph.
                                That is, $e_2 = <_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v))$ and we have $[e_2] = [<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v))] = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
                            \item $v \sim v'$ which means there exists $z \in V_{Z}$ such that $v = f_{V};\iota_{1,V}(z)$ and $v' = g_{V};\iota_{2,V}(z)$ and $v' \in s(e_1)$.
                                  Once again, $e_2 = <_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1))$ implies $e_2 = <_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v'))$ and 
                                  \begin{align*}
                    ...
                    \item $[e_1) = \varnothing$ and by Corollary~\ref{corollary:edges} there exists $v_1$ such that there is a path from $e_1$ to $v_1$ and there exists $v'_1 \sim v_1$ such that $[v'_1) \not = \varnothing$.
                          Then, $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1]))$.
                          Suppose $v_1 = f_{V};\iota_{1,V}(z)$ and then $v'_1 = g_{V};\iota_{2,V}(z)$ and $[g_{V}(z)) \not = \varnothing$ and 
                          \[
                            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)] = [e_2]
                          \] and hence $e_2$ has a pre-image in $E_{Y}$.
                          \begin{itemize}
                            \item $v = v'$ and then $v \in s(e_1)$ and $[v) = \varnothing$.
                                  Because there is a path from $e_1$ to $v_1$ there is also a path from $v$ to $v_1$ and hence $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [e_2]$
                            \item $v \sim v'$ and $v' \in s(e_1)$ and $[v') = \varnothing$.
                                  By the same argument, 
                                  \begin{align*}
                                    <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))\\
                                                                                                          &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1]))\\ 
                                                                                                          &= [e_2]
                                   \end{align*}
                          \end{itemize}                    
                  \end{itemize}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    \item If $v$ has no pre-image in $V_{Z}$ and $[v) \not = \varnothing$, then
                    \begin{align*}
                        [e_2] &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))\\
                              &= [<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v))]
                    \end{align*}
                          This essentially means that $e_2$ is the predecessor of $v$ and, by e-hypergraphness of $X+Y$, $e_2 = <_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1))$ and 
                          \begin{align*}
                            [e_2] &= [<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1))]\\ 
                                  &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1]))
                          \end{align*}
                    ...
                    \item $v$ has no pre-image in $V$ and $[v) = \varnothing$.
                          ...
                          \begin{align*}
                            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))\\
                             &= [e_2]
                          \end{align*}
                          Because $v \in s(e_1)$, there should also be a path from $e_1$ to $v'$ and 
                          \begin{align*}
                            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1])) &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))\\
                             &= [e_2]
                          \end{align*}
                    \item $v$ has a pre-image $f_{V};\iota_{1,V}(z)$ and $[f_{V}(z)) \not = \varnothing$ and $[g_{V}(z)) = \varnothing$, then
                    \begin{align*} 
                        [e_2] &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))\\
                              &= [f_{V};\iota_{V_{X}};<_{X}^{\mu};\iota_{1,E}(z)]
                    \end{align*}
                    This means that the pre-image of $e_2$ is the predecessor of $f_{V}(z)$ and hence $e_2$ should be the predecessor of $f_{V};\iota_{1,V}(z)$ and hence of $e_1$ since $v_1 \in s(e_1)$ and $X+Y$ is an e-hypergraph.
                    That is, $e_2 = <_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1))$ and 
                    \begin{align*}
                        [e_2] &= [<_{X+Y}^{\mu}(\iota_{E_{X} + E_{Y}}(e_1))]\\
                               &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e_1]))
                    \end{align*}
                    \item $v$ has a pre-image $f_{V};\iota_{1,V}(z)$ and $[f_{V}(z)) = \varnothing$ and $[g_{V}(z)) \not = \varnothing$, then
                    \begin{align*}
                        [e_2] &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))\\
                              &= [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)]
                    \end{align*} 
                    This means that $e_2$ has a pre-image in $E_{Y}$ and $[e_1) = \varnothing$ since $e_1$ has a path to $v$.
                    Then, by definition, 
                    \begin{align*}
                        <_{\sfrac{X+Y}{\sim}}^{\mu}([e_1]) &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v])\\
                                                           &= [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)]\\
                                                           &= [e_2]
                    \end{align*}
                    \item Cases when $v$ has a pre-image $g_{V};\iota_{2,V}(z)$ are symmetric.
                \end{itemize}
                \item $v \sim v'$.
                    Follows by the same argument as above by recalling that $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
            \end{itemize}    
\end{proof}
...
\end{lemma}
\begin{proof}
    Suppose $[v] \in s([e])$ and $[[v]) = [[e]) \not = \varnothing$, then it must be the case that $\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e]) \in \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$.
    $[v] \in s([e])$ means there exists $v' \in s(e)$ such that $v \sim v'$.
    We then have two cases.
    \begin{itemize}
      \item $v = v'$ and then $v \in s(e)$.
      \begin{itemize}
          \item $[v) \not = \varnothing$ and then $[e) \not = \varnothing$ and necessarily $\iota_{E_{X} + E_{Y}}(e) \in \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v))$.
                  By applying $([]_{V},[]_{E})$ we get 
                  \begin{align*}
                  \iota_{\sfrac{E_{X}+E_{Y}}{\sim}}([e]) &\in \\
          ...
          \item $[v) = \varnothing$ and then $[e) = \varnothing$ then there exists $v'$ such that there is a path from $e$ to $v'$ and $v'' \sim v'$ such that $[v'') \not = \varnothing$.
                It is then also the case that $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)) = \varnothing$ and $\consistency_{X+Y}(\iota_{E_{X} + E_{Y}}(e)) = \varnothing$ and $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v'')) \not = \varnothing$.
                Then, by definition, 
                \begin{align*}
                \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) &= \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))\\
                 &= \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'']))
                ...
                and by reflexivity 
                \[
                    \iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e]) \in \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])) = \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))
                \]
      \end{itemize}
      \item $v \sim v'$ follows by the same argument by using functionality, 
          \[
              \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v']) = \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v])
          \]
     \end{itemize}
...
\label{lemma:consistency_child}
    $\consistency_{\sfrac{X+Y}{\sim}}$ is defined only for nodes and edges that share the same non-empty set of parents.
\end{lemma}
\begin{proof}
  If $\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1]) \in \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2]))$ then
        $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}+V_{Y}}{\sim}}([v_1])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}+V_{Y}}{\sim}}([v_2]))$.
        By definition, one case is when
        \begin{align*}
            &\consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2]))\\
            &= ([[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*} (\bigcup_{v' \in V_{X} + V_{Y} | [v']_{V} = [v_2]_{V}} \consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v'))))^{c}
        \end{align*}
        which means that either $v_1~\consistency_{X+Y}~v_2$ or $v'_1 ~\consistency_{X+Y}~ v_2$ and $v'_1 \sim v_1$, or $v_1 ~\consistency_{X+Y}~ v_2'$ and $v_2' \sim v_2$, or $v_1' ~\consistency_{X+Y}~ v_2'$ and $v_1' \sim v_1$, $v_2' \sim v_2$.
        Suppose, $v_1' ~\consistency_{X+Y}~ v_2$ and $v_1' \sim v_1$.
        It implies that $[v_1') = [v_2) \not = \varnothing$.
        If $v_1$ has no pre-image in $V_{Z}$, then $v_1 = v'_1$ and we have
        \begin{align*}
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1']))\\
             &= [<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v_1'))]\\
             &= [<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v_2))]\\
             &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2]))
        \end{align*}
        If $v_1$ has a pre-image in $V_{Z}$ and $v_1 = f_{V};\iota_{1,V}(z)$ and $[f_{V}(z)) = \varnothing$ (because $[v'_1) = [v_2) \not = \varnothing$) and $v'_1 = g_{V};\iota_{2,V}(z)$ and $[g_{V}(z)) \not = \varnothing$.
        Then,
        \[
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_1])) = [g_{V};\iota_{V_{Y}};<_{Y}^{\mu};\iota_{2,E}(z)] = [\iota_{2,E}(<_{Y}^{\mu}(\iota_{V_{Y}}(v_2)))]
        \]
        and
        \[
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = [<_{X+Y}^{\mu}(\iota_{V_{X} +V_{Y}}(v_2))] = [\iota_{2,E}(<_{Y}^{\mu}(\iota_{V_{Y}}(v_2)))]
        \]
        The rest cases are analogous.
        Another case is when there exists $v_3$ such that there is a path from $v_2$ to $v_3$ and there is $v_3' \sim v_3$ such that $\consistency_{X+Y}(v_3') \not = \varnothing$ and
        \[
            \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_2])) = \consistency_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v_3]))
        \]
        Then the argument is the same but with using $v_3$ in place of $v_2$.
  Case for edges is analogous.
\end{proof}


\begin{proposition}[$\sfrac{X+Y}{\sim}$ is an e-hypergraph]
For this we need to check that the defined relations --- $<_{\sfrac{X+Y}{\sim}}$ and $\consistency_{\sfrac{X+Y}{\sim}}$ --- satisfy all the needed properties.
\end{proposition}
\begin{proof}
...
\]
    ...
    \begin{enumerate}
        \item $[m_{V}(f_{V}(v_i))) = [m_{V}(f_{V}(v_j)))$ for all $v_i,v_j$ in $V_{\mathcal{K}}$.
        \item $\consistency(m_{V}(f(v_i))) = \consistency(m_{V}(f_{V}(v_j)))$ for all $v_i,v_j$ in $V_{\mathcal{K}}$.
...
\begin{proof}
    ...
    In particular, they mean that after removing the image of $\mathcal{L}$ from $\mathcal{K}$ we have the following
    \begin{itemize}
        \item $[c_{V}(v_{i})) = [c_{V}(v_{j}))$ for all $v_{i},v_{j}$ in $V_{\mathcal{K}}$.
        \item $\consistency(c_{V}(v_i)) = \consistency(c_{V}(v_j))$ for all $v_i,v_j$ in $V_{\mathcal{K}}$.
    \end{itemize}
    This means that $\mathcal{G}$ is isomorphic to the pushout constructed accroding to Theorem~\ref{th:existence_of_pushouts}.
\end{proof}
...
\label{prop:boundary_complement}
    The boundary complement in~\ref{def:boundary_new} when exists is unique
    \begin{proof}
        The pushout in $EHyp_{\Sigma}$ should also be a pushout on the underlying sets of vertices and hyperedges.
    ...
    \begin{figure}[h!]
        ...
        \end{subfigure}
    \end{figure}
    $V$s and $E$s are sets of vertices and hyperedges respectively, marked squares are pushouts and $\mathcal{L}^{\bot}$s are pushout complements.
...
\[\begin{tikzcd}
	{i + j + x + y} && {i+j + x} && {i+j} \\
	\\
	{i + j + x + y + z} &&&& {\begin{matrix} i + j + z \\ \cong \\ i + j + w \end{matrix}}
...
\end{proposition}
\begin{proof}
    To prove this statement we essentially need to show that the functor $\llbracket - \rrbracket$ is well-defined, \textit{i.e.}, if $g = h$ in $\textsf{PROP}^{+}(\Sigma)$ then $\llbracket g \rrbracket = \llbracket h \rrbracket$ in $\WellTypedMdaEcospans / \mathcal{S}$ or $\llbracket g \rrbracket \Rrightarrow_{\mathcal{S}}^{*} \llbracket h \rrbracket$ in $\WellTypedMdaEcospans$.
    $g = h$ implies that there is a sequence of SMC and enrichment equations such that when applied to $g$ it is turned into $h$.
    ...
    \]
    We then further note that if $l$ is a subterm of $g$ then the nodes in the image of input and output external interfaces of $\llbracket l \rrbracket$ in $\llbracket g \rrbracket$ all share the same set of parents and are pairwise consistent if the set of parents is non-empty.
    ...
    Therefore, there exists a pushout complement $\mathcal{C}$ which is $\llbracket g \rrbracket$ with $\llbracket l \rrbracket$ removed.
