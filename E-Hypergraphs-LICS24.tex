...
\begin{theorem}[Corollary 26~\cite{bonchi_string_2022-2}]\label{thm:prop-equiv}
...
We use the notation $n \setminus m$ to denote the discrete e-hypergraph with $n - m$ nodes,  in particular with the nodes of $m$ removed from $n$ when nodes of $m$ is a sub-e-hypergraph of $n$.
...
Because extended cospans have a more general notion of interface,  including \textit{internal} nodes,  DPOI rewriting as presented in Section \ref{sec:combinatorial-semantics} needs some adjustments. This is the purpose of this section of the paper. 
...
Before introducing our definition of \textit{extended} DPOI rewriting,  we must modify the definition of boundary complement to guarantee that the rewriting yields a monogamous directed acyclic e-hypergraph. 
...
The appropriate notion of \textit{match} for our hierarchical setting additionally requires that its image is a \textit{down-closed} graph,  as defined below. 
...
Observe that the strictly internal interface $i' \setminus i$ of $\mathcal L$ is mapped to the internal interface $n'$ of $\mathcal G$,  since $\mathcal L$ is a down-closed subgraph of $\mathcal G$.  This induces an identification of $i' \setminus i$ in  $n'$. 
Thus,  $f_1$ is given by $g_{ext}$ with its codomain restricted to $n' \setminus (i' \setminus i)$ --- noting the image of $g_{ext}$ indeed lies within $n' \setminus (i' \setminus i)$.  The map $g_1$ is then derived from the restriction of $g_{int}$ to type $n' \setminus (i' \setminus i) \to \mathcal G$ by further observing that $\mathcal L^\bot$ can be identified within $\mathcal G$ --- and in particular has the internal interface of $\mathcal G$ minus $i' \setminus i$.  A similar argument defines $f_2$ and $g_2$. 
...
This is because its input internal interface may contain nodes that previously were a part of the output internal interface and vice versa. 
...
\]
Observe  that therefore $\llbracket f_1 \rrbracket + \ldots + \llbracket f_n \rrbracket$ and $\llbracket g_1 \rrbracket + \ldots + \llbracket g_m \rrbracket$ are weak normal forms.  We conisder two cases: where $n=m$ and where $n\neq m$.  In the first case,  it is immediate that $\llbracket f \rrbracket \neq \llbracket g \rrbracket $,  since their weak normal forms cannot be equal.  Otherwise,  if $n = m$,  and $\llbracket f \rrbracket = \llbracket g \rrbracket$,  we have that there exists a permutation $\sigma$ such that for every index $i$ we have $\llbracket f_i \rrbracket \neq \llbracket g_{\sigma(i)} \rrbracket $,  and thus by  $(*)$ that $f_i = g_{\sigma(i)}$,  implying that $f = g$ which is a contradiction. 

To see that the functor is full,  consider a morphism $f$ in $\WellTypedMdaEcospans$.  We can find an equivalent weak normal form $f_1 + \ldots + f_n$,  where each $f_i$ contains no hierarchical edges.  Thus,  by $(*)$ we can find morphisms $g_i$ in $\textbf{SMT}(\Sigma, \mathcal E)$ such that $\llbracket g_i \rrbracket = f_i$.  Thus,  we can construct a morphism $g_1 + \ldots + g_n$ such that $\llbracket g_1 + \ldots + g_n \rrbracket = \llbracket g_1 \rrbracket + \ldots \llbracket g_n \rrbracket = f_1 + \ldots + f_n$,  as required. 
\end{proof}
...
\begin{remark}
    Both $<$ and $\consistency$ can be considered as functions defined on $V_{\mathcal{F}} + E_{\mathcal{F}}$, \textit{i.e.}, on the coproduct of verticies and edges.
...
We then quotient the set verticies and edges in $X + Y$ by $R_{V}$ and $R_{E}$ respectively.
    \begin{align*}
...
Recall that $<$ is essentially a transitive clousure of $<^{\mu}$ and hence we can first define 
    ...
    \begin{enumerate}
        ...
        \item $u$ has no pre-image in $V_{Z}$ and $<_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(u))$ is undefined (\textit{i.e.}, $[u) = \varnothing$). 
              If there exists $[v]$ such that $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([v])) = [e']$ and such that there is an \textit{undirected} path from $[u]$ to $[v]$, then we define
              \[ 
                <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[u]) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}[v])
              \]
    \end{enumerate}
    ...
    % Essentially, this interleaving means that verticies inherit the relation from incident edges and vice versa.
    ...
    Suppose $t_1, t_2$ are verticies and $t_1 \sim t_2$, then $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X}+V_{Y}}{\sim}}[t_1]) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[t_2])$.
    ...
    \[
        \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]_{V})) = [[]_{V}^{\consistency}, []_{E}^{\consistency}]^{*}(\bigcup_{v' \in V_{X} + V_{Y} | [v']_{V} = [v]_{V}} \consistency_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v')))
    \]
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \]
              $v_1 \sim v_2$ implies there exists $z \in V_{Z}$ such that $v_1 = f_{V};\iota_{1,V}(z)$ and $v_2 = g_{V};\iota_{2,V}(z)$ and supposing that for any of $v_i$ $\consistency_{X+Y}^{\mu}(\iota_{V_{X} + V_{Y}}(v_i)) \not = \varnothing$, then
              \begin{align*}
        ...
        \item Putting $\phi = ([]_{V},[]_{E})$ into the definition of a homomorphism gives us
            \begin{align*}
             [[]_{V};\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}, []_{E};\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}]^{*}(&\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(x)))\subseteq
            ...
            \end{align*}
            and by recalling that $[]_V^{\consistency} = []_{V};i_{\sfrac{V_{X} + V_{Y}}{\sim}}$ and similarly for $[]_{E}^{\consistency}$ we get the inclusion 
            \[
                []_{V};\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}, []_{E};\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}]^{*}(\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(x))) \subseteq \consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([x]))
...
By construction we have the following corollaries for $<_{\sfrac{X+Y}{\sim}}$.
...
\begin{lemma}
    $<_{\sfrac{X+Y}{\sim}}^{\mu}$ is irreflexive, assymetric and respects connectivity.
    \begin{proof}
...
\begin{proof}
    ...
    \]
    We then further note that if $l$ is a subterm of $g$ then the nodes in the image of input and output external interfaces of $\llbracket l \rrbracket$ in $\llbracket g \rrbracket$ all share the same set of parents and are pairwise consistent.
    ...
    Then necessarily verticies in $\mathcal{C}$ in the image of $i+j$ share the same set of parents and are pairwise consistent as they do in $\mathcal{G}$ so.
