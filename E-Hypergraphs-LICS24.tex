...
%To see that the functor is faithful,  consider $f$ and $g$ of $\textbf{SMT}^{+}(\Sigma, \mathcal {E} )$ such that $\llbracket f \rrbracket = \llbracket g \rrbracket$.  We aim to show that $f = g$.  First,  find weak normal forms of $\llbracket f \rrbracket$ and $\llbracket g \rrbracket$,  namely $\phi_1 + \ldots + \phi_ni$ and $\psi_1 + \ldots + \psi_n$.  We now need a further observation that if $\llbracket f \rrbracket \Rrightarrow_{{\llbracket \mathcal{S,E} \rrbracket}^+} \llbracket g \rrbracket$,  then there exist rewrites $\phi_i \Rrightarrow_{\mathcal E} \psi_i$.  Thus,  by $(*)$ we have that $\llbracket \phi_i \rrbracket^{-1} = \llbracket \psi_i \rrbracket^{-1}$.  Noting that $f = \llbracket \phi_1 \rrbracket^{-1} + \ldots + \llbracket \phi_n \rrbracket^{-1}$ and $g = \llbracket \psi_1 \rrbracket^{-1} + \ldots + \llrbacket \psi_n \rrbracket^{-1}. 
