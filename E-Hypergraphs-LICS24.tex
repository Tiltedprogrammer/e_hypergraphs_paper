...
\] 
Analogous to Section \ref{sec:combinatorial-semantics},  we observing that the following extended cospans express the same data
\[
...
\]
is a \textit{boundary} complement if $[c_1,c_2]$ is mono and there exist $d_1 : n \to \mathcal{L}^\bot$ and $d_2 : k \to \mathcal{L}^\bot$ making the above triangle commute,  such that
    \[
    ...
    \]
is a \textit{not necessarily well-typed} extended mda-cospan.
