...
\begin{enumerate} 
    ...
    \]
\end{enumerate}
where $f_1$ is given by $g_{ext}$ with its codomain restricted to $n' \setminus (i' \setminus i)$ --- noting the image of $g_{ext}$ indeed lies within $n' \setminus (i' \setminus i)$ --- and 
similarly for $f_2$. The map $g_1$ is derived from $g_{int}$ as follows. 
 %because the image of $m$ is a convex down-closed e-hypergraph, 
Note that the internal interface $i'$ of $\mathcal L$ is mapped to the internal interface $n'$ of $\mathcal G$,  since $\mathcal L$ is a down-closed subgraph of $\mathcal G$.  This induces an identification of $i'$ in  $n'$ which restricts to an identification of $i'\setminus i$ in $n'$. 
This gives rise to a restriction of $g_{int}$ of type $n' \setminus (i' \setminus i) \to \mathcal G$.  Since $\mathcal L^\bot$ can be identified within $\mathcal G$ --- and in particular has the internal interface of $\mathcal G$ minus $i' \setminus i$ --- this observation induces the map $g_1:  n' \setminus (i' \setminus i) \to \mathcal{L}^\bot$ from $g_{int}$.  A similar argument defines $g_2$. 
...
such that the two marked squares are pushouts and the following conditions hold:
	...
	\]
	\item the map $f = [f_1,f_2]: n'' \to \mathcal H$ in the diagram above is defined as in Definition \ref{def:boundary_new} of boundary complement,  and $f_2$ as the restiction of the composite $i'' \to \mathcal R \to \mathcal H$ and,  and similarly for the map $g: k'' \to \mathcal H$.  
    \end{enumerate}
...
\label{pushout:assumptions}
    ...
    \item $\consistency(f_{V};\iota_{V_{X}} (v_i)) = \consistency(f_{V};\iota_{V_{X}}(v_j))$ and $\consistency(g_{V};\iota_{V_{Y}}(v_i)) = \consistency(g_{V};\iota_{V_{Y}}(v_j))$ for all $v_i,v_j$ in $V_{Z}$.
\end{enumerate}    
...
\]
    ...
    \begin{enumerate}
        ...
        \item $\consistency(m_{V};\iota_{V_{\mathcal{G}}}(v_i)) = \consistency(m_{V};\iota_{V_{\mathcal{G}}}(v_j))$ for all $v_i,v_j$ that have a pre-image in $V_{\mathcal{K}}$.
