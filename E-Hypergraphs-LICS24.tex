...
We use the notation $n \setminus m$ to denote the discrete hypergraph with $n - m$ nodes,  in particular with removal of the nodes of $m$ removed from $n$ when nodes of $m$ are identified within $n$.
...
\end{proposition}    
Note that at similar argument can be used to rewrite $\Sigma^+$-terms into a similar form by using only the semilattice-enrichment equations.  
