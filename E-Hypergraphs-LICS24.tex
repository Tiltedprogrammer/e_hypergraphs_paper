...
Because extended cospans have a more general notion of interface,  we need to make some adjustments to DPOI rewriting as presented in Section \ref{sec:combinatorial-semantics}.
The first thing is that we need to account for is the existence of \textit{internal} interfaces,  and in particular that internal interfaces are not necessarily preserved during rewriting.
To illustrate this, consider the following rewrite rule
...
\] such that there is a matching $m : \mathcal{L} \to \mathcal{G}$.
When removing the occurence of $\mathcal{L}$ from $\mathcal{G}$, e.g. by computing the pushout complement, the internal interface of $\mathcal{G}$ should be modified.  This is because the verticies of $\mathcal{L}$ corresponding to strict internal interface of $\mathcal{L}$ (and hence to strict internal interface of $\mathcal{G}$) should be removed from $\mathcal{G}$ and therefore from $\mathcal{G}$'s internal interface.
Then, when gluing $\mathcal{R}$ into the created context, we also introduce strict internal interfaces of $\mathcal{R}$ into this context.
...
To account for such changes in the interfaces we require additional structure on DPOI rewriting for $\catname{EHyp({\Sigma})}$.
...
Note the addition of internal interfaces and how we use the same trick to go from cospan $n_{ext} \xrightarrow{} g_i \xrightarrow{} G \xleftarrow{} g_o \xleftarrow{} m_{ext}$ to $G \xleftarrow{} g_{i} + g_{o} \xleftarrow{} n_{ext} + m_{ext}$.
...
Then, we explicitly show how the internal interfaces of the pushout complement are constructed.
...
For monogamous extended cospans $x_{ext} \xrightarrow{} l_{i} \xrightarrow{} L \xleftarrow{} l_{o} \xleftarrow{} y_{ext}$ and $n_{ext} \xrightarrow{} g_{i} \xrightarrow{} G \xleftarrow{} g_{o} \xleftarrow{} m_{ext}$ and mono $f : L \to G$, a pushout complement $x_{ext} + y_{ext} \to C \to G^{\urcorner}$ as depicted in the square below
\[
\scalebox{0.75}{
...
\begin{definition}[Convex down-closed match] 
    We call $m : L \to G$ in $\catname{EHyp(\Sigma)}$ a convex down-closed match if
...
The whole process of rewriting including the construction of the internal interfaces of the resulting e-hypergraph is summarised in the following definition.
...
if 
\begin{align*}    
    G \xleftarrow{[ a_2,b_2 ]} g_i + g_{o}& \xleftarrow{[a_1, b_1]} n_{ext} + m_{ext} \\ &\Rrightarrow^{+}\\ H \xleftarrow{[a_2,b_2]} h_i + h_{o}& \xleftarrow{[a_1, b_1]} n_{ext} + m_{ext}
\end{align*}

under a rewrite rule
\[
    \langle l_{i} + l_{o} \xrightarrow{[a_2, b_2]} L \xleftarrow{[a_1, b_1]} x_{ext} + y_{ext},
    r_{i} + r_{o} \xrightarrow{[a_2, b_2]} R \xleftarrow{[a_1, b_1]} x_{ext} + y_{ext} 
    \rangle
\]
...
Recall further that (typed) $\Sigma^+$-terms are those categorical combinator terms freely constructed from generators $c \in \Sigma$, $\textsf{id}_1$, $\sigma_{1,1}$, $(;\!)$ and $\otimes$,  and $f+g$, respectively.  Write $\Sigma^+(n,m)$ for the set of $\Sigma^+$-terms of type $n \to m$.  We wish to specify a set of rewrite rules corresponding to those in Figure \ref{fig:string-equations}.  In order to do this,  we specify a function which maps  uninterpreted $\Sigma^+$-terms into $\WellTypedMdaEcospans$ morphisms,  and which also extends the action of the free functor from $\textbf{PROP}(\Sigma)$ into $\WellTypedMdaEcospans$ induced by the interpretation of $\Sigma$ as itself (detailed in \cite{bonchi_string_2022-2}). 
...
Let $\llbracket - \rrbracket_{n,m}: \Sigma^+(n,m) \to \WellTypedMdaEcospans(n,m)$ be the function defined by (mutual) induction on $\Sigma$-terms by 
\begin{align*}
	\llbracket \id \rrbracket_{n,m} &= \id  &	\llbracket \phi ; \psi \rrbracket_{n,m} &= \llbracket \phi \rrbracket^+_{n,m} ; \llbracket \psi \rrbracket_{n,m} \\
...
\[
	f \sim g \quad \text{if} \quad f \Rightarrow^*_{\llbracket \mathcal E \rrbracket^+} g ~ . 
\]
\end{definition}
In particular,  let $\mathcal{S}$ be the set equations of Figure \ref{fig:string-equations},  \textit{i.e.},  the semilattice equations to be applied to $\Sigma^+$-terms to recover $\textbf{PROP}^+(\Sigma)$. 
This quotient in fact turns the interpretation function into a proper functor $\llbracket - \rrbracket: \textbf{PROP}^+(\Sigma) \to \WellTypedMdaEcospans/{\mathcal{S}}$,  which can further be quotiented to account for extra equations arising in symmetric monoidal theories. 
Note that not all pushouts in $\textbf{MEHypI}(\Sigma)$ are guaranteed to exist: however,  pushouts for rewrites generated from $\Sigma$-equations and those generated from the semilattice equations on $\Sigma^+$-terms do exist.  
%Let $\mathcal E$ be an arbitrary set of $\Sigma$-equations. 
Thus we have the following result. 
\begin{proposition}[Soundness]
The category $\WellTypedMdaEcospans/{\mathcal{S}}$ a semilattice-enriched PROP. 
\end{proposition}
...
The equations of Figure~\ref{fig:string-equations} -- excepting the commutativity equation -- can all be directed in an obvious way to form a terminating rewrite system which results in a cospan satisfying the statement below. 
...
This normal form can be used to prove the following completeness theorem. 
...
\end{theorem}
\begin{proof}
The equivalence is given by the functor $\llbracket - \rrbracket$.  First, note that the functor is surjective on objects,  so it suffices to prove the functor is both full and faithful.  In each case,  the result follows by a normal form argument in combination with relying on the equivalence $(*)$ $\textbf{SMT}(\Sigma, \mathcal E) \cong \textbf{MHypI}(\Sigma)/\mathcal{E}$ of symmetric monoidal theories and (appropriately quotiented) plain MDA hypergraphs which is given by the restriction of $\llbracket - \rrbracket$ to $$\textbf{SMT}(\Sigma, \mathcal E)$ which is a subcategory of $\textbf{SMT}^{+}(\Sigma, \mathcal {E} )$. 

To see that the functor is faithful,  consider $f$ and $g$ of $\textbf{SMT}^{+}(\Sigma, \mathcal {E} )$ such that $\llbracket f \rrbracket = \llrbracket g \rrbracket$.  We aim to show that $f = g$.  First,  find weak normal forms of $\llbracket f \rrbracket$ and $\llrbracket g \rrbracket$,  namely $\phi_1 + \ldots + \phi_ni$ and $\psi_1 + \ldots + \psi_n$.  We now need a further observation that if $\llbracket f \rrbracket \Rrightarrow_{{\llbracket \mathcal{S,E} \rrbracket}^+} \llrbracket g \rrbracket$,  then there exist rewrites $\phi_i \Rrightarrow_{\mathcal E} \psi_i$.  Thus,  by $(*)$ we have that $\llbracket \phi_i \rrbracket^{-1} = \llbracket \psi_i \rrbracket^{-1}$.  Noting that $f = \llbracket \phi_1 \rrbracket^{-1} + \ldots + \llbracket \phi_n \rrbracket^{-1}$ and $g = \llbracket \psi_1 \rrbracket^{-1} + \ldots + \llrbacket \psi_n \rrbracket^{-1}. 
CHRIS: THIS NEEDS FIXING. 

To see that the functor is full,  consider a cospan $f$ in $\WellTypedMdaEcospans$.  We can find an equivalent weak normal form $f_1 + \ldots + f_n$,  where each $f_i$ is a plain MDA hypergraph.  Thus,  by $(*)$ we can find morphisms $g_i$ such that $\llbracket g_i \rrbracket = f_i$.  Thus,  we have a morphism $g_1 + \ldots + g_n$ such that $\llbracket g_1 + \ldots + g_n \rrbracket = \llbracket g_1 \rrbracket + \ldots \llbracket g_n \rrbracket = f_1 + \ldots + f_n$,  as required. 
\end{proof}
...
For any $\phi, \psi \in \textbf{SMT}^{+}(\Sigma, \mathcal{E})$,  we have
\[
...
[cite, cite, cite].  We consider that our development of hierarchical "e-box" structure within string diagrams and their associated (e-)hypergraphs ( categorically,  semilattice enrichment) is essentially orthogonal to these developments, and so ought to be easily combined to give notions of e-hypergraphs specifically suited to the optimization of these particular monoidal theories.  A particularly interesting extension that we intend to consider is the combination of our "e-structure" with (Cartesian) \textit{closed} categories, allowing the natural combination of e-graph optimization techniques with the $\lambda$-calculus.  The corresponding combinatorial representation would build on the recent work \cite{ghica_hierarchical_2023, alvarez-picallo_rewriting_2022, alvarez-picallo_functorial_2021}.  String diagrams for these various examples are included in Figure \ref{fig:applications}.  There is also the question of developing a corresponding term calculus \cite{Heijltjes:FMCII}. % hierarchical  
