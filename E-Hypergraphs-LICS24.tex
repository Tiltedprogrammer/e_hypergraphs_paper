...
requires the use of a categorical \textit{trace}, generating a cycle. In this paper, we focus on \textit{acyclic} e-graphs and their corresponding categories; but the above example illustrates that the extension to the cyclic case seems to be routine. As it would involve needless additional presentational complexity we leave it as further work. 
...
\begin{definition}[Category of e-hypergraphs with interfaces]
    The category of \textit{e-hypergraphs with extended interfaces} $\Ecospans$ has discrete e-hypergraphs as objects,  with hom-sets $\Ecospans(n,m)$ consisting of isomorphism classes of \textit{extended cospans}, defined as follows:  
    \[
...
We will do the same for the corresponding pushout complement and show its uniqueness.
These condition will play a crucial role in showing that for each equation in $\textsf{PROP}^{+}(\Sigma)$ there is a sequence of rewrites in $\WellTypedMdaEcospans$.
...
Recall that $<$ is essentially a transitive closure of $<^{\mu}$ and hence we can first define 
    ...
    We also need to make sure that $[]_{E} : E_{X} + E_{Y} \to \sfrac{E_{X} + E_{Y}}{\sim}$ and $[]_{V} : V_{X} + V_{Y} \to \sfrac{V_{X} + V_{Y}}{\sim}$ are homomorphisms with respect to $<^{\mu}$.
    \begin{enumerate}
    ...
    \begin{enumerate}
        \item If $\consistency_{X+Y}(\iota_{E_{X} + E_{Y}}(e)) \not = \varnothing$, then
                ...
                \end{align*}
        \item $\consistency_{X+Y}(\iota_{E_{X} + E_{Y}}(e)) = \varnothing$ and there exists $[v]$ such that there is an undirected path from $[e]$ to $[v]$ and such that 
        \[
    ...
    \begin{itemize}
      ...
      \begin{itemize}
          ...
          \item $[e_1) = \varnothing$ and then by Corollary~\ref{corollary:edges} there exists $v$ such that there is a path from $e_1$ to $v$ and there exists $v' \sim v$ such that $[v') \not = \varnothing$.
                ...
                \end{itemize}
          \end{itemize}
          \item $[e_2) = \varnothing$ and by Corollary~\ref{corollary:edges} there exists $v_2$ such that there is a path from $e_2$ to $v_2$ and there exists $v'_2 \sim v_2$ such that $[v'_2) \not = \varnothing$. 
                Then, 
                \[
...
\label{lemma:consistency_child}
    $\consistency_{\sfrac{X+Y}{\sim}}$ is defined only for vertices and edges that share the same non-empty set of parents.
\end{lemma}
\begin{proof}
...
We will then discuss the existence and uniqueness of the corresponding pushout complement by first recalling the result for plain hypergraphs.
...
\begin{proof}
    ...
    Then, no-dangling and no-identification conditions are necessary for pushout complement to exist~\cite{dpo}.
    ...
    \end{itemize}
    This means that $\mathcal{G}$ is isomorphic to the pushout constructed according to Theorem~\ref{th:existence_of_pushouts}.
\end{proof}
...
\label{prop:boundary_complement}
    ...
    \[
        \tikzfig{appendix/pushout_edges}
    \]
    where $V$s and $E$s are sets of vertices and hyperedges respectively, marked squares are pushouts and $\mathcal{L}^{\bot}$s are pushout complements.
...
\[\begin{tikzcd}
	...
	\arrow["c"', from=1-5, to=3-5]
\end{tikzcd}\]
    Because $i + j + x + y + w$ also yields a pushout it must be the case that $i+j+x+y+w \cong i + j + x + y + z$ and therefore $w \cong z$ and pushout complement for vertices is $i + j + z$ up to isomorphism.
...
In this section we will elaborate on soundness of our translation $\llbracket - \rrbracket : \textsf{PROP}^{+}(\Sigma) \to \WellTypedMdaEcospans$.
