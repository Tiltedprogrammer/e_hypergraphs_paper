...
Thus,  we wish a rewrite rule to be a pair of \textit{extended} cospans of e-hypergraphs with matching \textit{external} (but not necessaril internal) interfaces,  as follows. 
\[
...
\begin{remark}
There is a limitation of EDPOI rewriting as defined in this section: it is unable to express rewriting
for rules whose target contains an edge with no incident vertices.  
While this limitation can be overcome,  we find the technical development (relating to the child and consistency relations) to be significantly simpler if we simply avoid consideration of such edges.  
...
To guarantee that the rewriting yields a monogamous directed acyclic e-hypergraph we must revise the definition of boundary complement.
...
We are now ready to define \textit{convex extended DPOI (EDPOI) rewriting} for $\catname{EHyp({\Sigma})}$.  for e-hypergraphs with interfaces,  which we will later use to show the equivalence between $\textsf{PROP}^+(\Sigma)$ and $\WellTypedMdaEcospans$.
...
CHRIS: TODO: COMBINE TWO EDPOI DEFINITIONS
...
\]
    ...
    \begin{enumerate}
        ...
        \item all $v,w$ that have a pre-image in $i+j$ share the same set of parents;
        \item $m(v)  \consistency m(w)$ for all $v,w$ that have a pre-image in $i+j$.
        \item $i + j \to \mathcal{C} \to \mathcal{G}$ is a boundary complement in the leftmost pushout.
    \end{enumerate}
and
