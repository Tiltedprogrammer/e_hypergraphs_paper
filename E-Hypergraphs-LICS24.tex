...
\begin{enumerate} 
    \item the image of $m$ is a convex down-closed e-hypergraph
    \item $[c_1,c_2]$ is mono
    \item there exist $d_1 : n \to \mathcal{L}^\bot$ and $d_2 : k \to \mathcal{L}^\bot$ making the above triangle commute
    \item all $v,w$ in $\mathcal{G}$ with the pre-image in $i + j$ share the same set of parents
    \item either for all $v,w$ in $\mathcal{G}$ with the pre-image in $i+j$, $v \consistency w$, or for all $v,w$ in $\mathcal{G}$ with the pre-image in $i+j$ $\consistency$ is undefined
...
Similarly for $g_2$.
$f_1$ is given by $g_{ext}$ with its codomain restricted to $n' \setminus (i' \setminus i)$ --- noting the image of $g_{ext}$ lies within $n' \setminus (i' \setminus i)$.
Similarly for $f_2$.
...
such that the two marked squares are pushouts and the following conditions hold:
	...
	\]
		...
		&f_2 : n' \setminus (i' \setminus i) \to \mathcal{C} \to \mathcal{H} & \\
		\end{align*}
    \end{enumerate}
...
$[f_1,f_2]: n'' \to \mathcal{H} $
Since both $f_1$ and $f_2$ are monos with disjoint image (?), their copairing is also a mono.  Note $f_1$ is the obvious restriction,  $f_2$ is defined as $g_2$ in boundary complement. 
CHRIS: FIX (4) ABOVE:  
