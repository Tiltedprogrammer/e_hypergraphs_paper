...
DAN: I THINK THE STRING DIAGRAMS EQUATIONS SHOULD BE ACCOMPANIED BY THE TERM EQUATIONS. THE NUMBER OF EQUATIONS IN THE DEFINITIONS AND IN THE FIGURE IS NOT THE SAME, WHICH IS A BIT CONCERNING. 
...
\end{enumerate}
We denote by $\catname{Hyp(\Sigma)}$ the category of hypergraphs over $\Sigma$. DAN: WHAT ARE THE MORPHISMS?
\end{definition}
...
DAN: I AM NOT SURE 'DISCRETE HYPERGRAPHS' ARE DEFINED?
...
DAN: THERE IS LITERATURE ON HIERARCHICAL HYPERGRAPHS (DETLEF PLUMP AND UGO MONTANARI I THINK HAVE PAPERS) SOME OF IT IS REFERENCED FROM MY PREVIOUS PAPERS ON HIERARCHICAL HYPERGRAPHS. MAKE SURE YOU CITE THEM, THESE PEOPLE MAY GET ANNOYED. WE DID NOT INVENT HIERARCHICAL HYPERGRAPHS. ALSO MY PAPER WITH KOKO WHICH INTRODUCES THEM ONLY COMBINATORIALLY. 
...
DAND: THIS IS VERY LONG... SHOULD IT BE BROKEN DOWN INTO SEVERAL DEFINITIONS? 
...
The first thing is that we need to account for internal interfaces after all.
The main issue is then that internal interfaces may not be preserved during rewriting.
...
When removing the occurence of $\mathcal{L}$ from $\mathcal{G}$, e.g. by computing the pushout complement, the internal interface of $\mathcal{G}$ should be modified as the verticies of $\mathcal{L}$ corresponding to strict internal interface of $\mathcal{L}$ (and hence to strict internal interface of $\mathcal{G}$) should be removed from $\mathcal{G}$ and therefore from $\mathcal{G}$'s internal interface.
...
To account for such changes in the interfaces we put some additional structure on DPOI rewriting for the case of $\catname{EHyp({\Sigma})}$.
...
For monogamous extended cospans $x_{ext} \xrightarrow{} l_{i} \xrightarrow{} L \xleftarrow{} l_{o} \xleftarrow{} y_{ext}$ and $n_{ext} \xrightarrow{} g_{i} \xrightarrow{} G \xleftarrow{} g_{o} \xleftarrow{} m_{ext}$ and mono $f : L \to G$, a pushout complement $x_{ext} + y_{ext} \to C \to G^{\urcorner}$ as depicted below
\[
\scalebox{0.75}{
