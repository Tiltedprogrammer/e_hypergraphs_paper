...
Recall that $<$ is essentially a transitive closure of $<^{\mu}$ and hence we can first define 
    ...
    Otherwise, we leave $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{V_{X} + V_{Y}}{\sim}([u]))$ undefined.
    ...
    \begin{enumerate}
        \item If there exists $v$ such that $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)) \not = \varnothing$ and $u \sim v$, we let
            ...
            \end{align*}
        \item $u$ has no pre-image in $V_{Z}$ and $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(u)) = \varnothing$ and there exists $v$ such that $\consistency_{X+Y}^{\hashtag}(\iota_{V_{X} + V_{Y}}(v)) \not = \varnothing$ and such that there is an undirected path from $[u]$ to $[v]$.
        Then we let
        \[
    ...
    to be the reflexive, symmetric and transitive closures of $\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}$.

    More precisely, 
    \[
      (\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])))^{c}
    \]
    where $c$ denotes a closure, is the smallest set such that
    \begin{itemize}
        \item $[v] \in (\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])))^{c}$ if $\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) \not = \varnothing$
        \item if $[u] \in \consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))$ then 
        \[
            [v] \in (\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([u])))^{c}
        \]
        ...
        \[
            [x_i] \in \consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([x_{i+1}]))
        \] or 
        \[
            [x_{i+1}] \in \consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([x_{i}]))
        \] for $i < n$ (respectively, when $[x_{i}] \in \sfrac{E_{X} + E_{Y}}{\sim}$) both
        \[
            [x_1] \in (\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([x_{n}])))^{c}
        \]
        and
        \[
            [x_n] \in (\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([x_{1}])))^{c}
        \]
    \end{itemize}
    Similarly for
    \[
        (\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}([e])))^{c}
    \]
    ...
    \begin{itemize}
        ...
        \item Suppose $<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))$ such that there is a path from $[v]$ to $[v']$ and $v$ has no pre-image in $V_{Z}$.
              ...
              \[
                <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([v]))) = u_{E}(<^{\mu}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v]))
              \]
        \item Suppose $v = f_{V};\iota_{1,V}(z)$ and $[f_{V}(z)) = \varnothing$. Then 
        \[
            <^{\mu}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v]) = [g_{V};\iota_{V_{Y}};<^{\mu}_{Y};\iota_{2,E}(z)]
        \]
        Because $j_2$ is a homomorphism, it must be the case that
        \[
         j_{2,E}(g_{V};\iota_{V_{Y}};<^{\mu}_{Y}(z)) = <^{\mu}_{Q}(g_{V};j_{2,V};\iota_{V_{Q}}(z))
        \]
        By definition,
        \begin{align*}
            <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([v]))) &= <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([\iota_{1,V}(f_{V}(z))])))\\
            &= <^{\mu}_{Q}(\iota_{V_{Q}}(j_{1,V}(f_{V}(z))))\\
            &\text{by commutativity}\\
            &= <^{\mu}_{Q}(\iota_{V_{Q}}(j_{2,V}(g_{V}(z))))\\
            &= j_{2,E}(g_{V};\iota_{V_{Y}};<^{\mu}_{Y}(z))
        \end{align*}
        And also
        \begin{align*}
            u_{E}([g_{V};\iota_{V_{Y}};<^{\mu}_{Y};\iota_{2,E}(z)]) &= j_{2,E}(g_{V};\iota_{V_{Y}};<^{\mu}_{Y}(z))\\
            &= <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([v])))
        \end{align*}
        \item Suppose $v = f_{V};\iota_{1,V}(z)$ and $[f_{V}(z)) \not = \varnothing$. Then 
        \[
            <^{\mu}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}[v]) = [f_{V};\iota_{V_{X}};<^{\mu}_{X};\iota_{1,E}(z)]
        \]
        Because $j_1$ is a homomorphism, it must be the case that
        \[
         j_{1,E}(f_{V};\iota_{V_{X}};<^{\mu}_{X}(z)) = <^{\mu}_{Q}(f_{V};j_{1,V};\iota_{V_{Q}}(z))
        \]
        By definition,
        \begin{align*}
            <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([v]))) &= <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([\iota_{1,V}(f_{V}(z))])))\\
            &= <^{\mu}_{Q}(\iota_{V_{Q}}(j_{1,V}(f_{V}(z))))\\
            &= j_{1,E}(f_{V};\iota_{V_{X}};<^{\mu}_{X}(z))
        \end{align*}
        And also
        \begin{align*}
            u_{E}([f_{V};\iota_{V_{X}};<^{\mu}_{X};\iota_{1,E}(z)]) &= j_{1,E}(f_{V};\iota_{V_{X}};<^{\mu}_{X}(z))\\
            &= <^{\mu}_{Q}(\iota_{V_{Q}}(u_{V}([v])))
        \end{align*}
        \item The case when $v = g_{V};\iota_{2,V}(z)$ is symmetric.
        \item Suppose that $v$ has no pre-image in $V_{Z}$ and $v = \iota_{1,V}(v')$ such that $[v') \not = \varnothing$. Then,
        \begin{align*}
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) &= <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([\iota_{1,V}(v')]))\\
            &= [<_{X+Y}^{\mu}(\iota_{V_{X}+V_{Y}})(\iota_{1,V}(v'))]\\
            &= [\iota_{1,E}(<_{X}^{\mu}(\iota_{V_{X}}(v')))]
        \end{align*}
        By definition,
        \begin{align*}
            u_{E}([\iota_{1,E}(<_{X}^{\mu}(\iota_{V_{X}}(v')))]) &= j_{1,E}(<_{X}^{\mu}(\iota_{V_{X}}(v')))\\
            &= <_{Q}^{\mu}(\iota_{V_{Q}}(j_{1,V}(v')))\\
            &= <_{Q}^{\mu}(\iota_{V_{Q}}(u_{V}([\iota_{1,V}(v')])))\\
            &= u_{E}(<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])))
        \end{align*}
        \item The last case to consider is when the predecessor of $[v]$ is defined by the predecessor of $[v']$ such that there is a path from $[v]$ to $[v']$. That is,
        \[
            <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) = <_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v']))
        \]
        Because $u$ preserves sources and targets, there is a path between $u_{V}([v])$ and $u_{V}([v'])$.
        Then
        \begin{align*}            
        &u_{E}(<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])))\\
        &= u_{E}(<_{\sfrac{X+Y}{\sim}}^{\mu}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v'])))\\
        &\text{(by reusing the results above for $v'$)}\\
        &= <_{Q}^{\mu}(\iota_{V_{Q}}(u_{V}([v'])))\\
        &\text{(because $Q$ is an e-hypergraph and $u_{V}(v)$ and $u_{V}(v')$ are connected)}\\
        &= <_{Q}^{\mu}(\iota_{V_{Q}}(u_{V}([v])))
        \end{align*}
    \end{itemize}
    The cases for edges are analogous.
    ...
    \begin{itemize}
        ...
        $\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])) \not = \varnothing$
        \item Suppose $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)) \not = \varnothing$ and $v = \iota_{1,V}(v_{X})$, then
        \begin{align*}
            &\consistency^{\hashtag}_{\sfrac{X+Y}{\sim}}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))\\
            &= [[]_{V};\iota_{\sfrac{V_{X} + V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}]^{*}(\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)))\\
            &= [[]_{V};\iota_{\sfrac{V_{X} + V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}]^{*}(\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(\iota_{1,V}(v_{X}))))\\
            &= [[]_{V};\iota_{\sfrac{V_{X} + V_{Y}}{\sim}},[]_{E};\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}]^{*}([\iota_{1,V};\iota_{V_{X} + V_{Y}},\iota_{1,E};\iota_{E_{X} + E_{Y}}]^{*}(\consistency_{X}(\iota_{V_{X}}(v_{X}))))\\
            &= [\iota_{1,V};[]_{V};\iota_{\sfrac{V_{X} + V_{Y}}{\sim}},\iota_{1,E};[]_{E};\iota_{\sfrac{E_{X} + E_{Y}}{\sim}}]^{*}(\consistency_{X}(\iota_{V_{X}}(v_{X})))
        \end{align*}
        and
        \begin{align*}
            &[u_{V};\iota_{V_{Q}}, u_{E};\iota_{E_{Q}}]^{*}(\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])))\\
            &=\\
            &[\iota_{1,V};[]_{V};u_{V};\iota_{V_{Q}},\iota_{1,E};[]_{E};u_{E};\iota_{E_{Q}}]^{*}(\consistency_{X}(\iota_{V_{X}}(v_{X})))\\
            &=\\
            &[j_{1,V};\iota_{V_{Q}},j_{1,E};\iota_{E_{Q}}](\consistency_{X}(\iota_{V_{X}}(v_{X})))\\
            &\subseteq\\
            &\consistency_{Q}(j_{1,V};\iota_{V_{Q}}(v_{X}))\\
            &=\\
            &\consistency_{Q}(u_{V};\iota_{V_{Q}}([v]))
        \end{align*}
        \item The case when $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)) \not = \varnothing$ and $v = \iota_{2,V}(v_{Y})$ is symmetric.
        \item Suppose $\consistency_{X+Y}(\iota_{V_{X} + V_{Y}}(v)) = \varnothing$ and $v = f_{V};\iota_{1,V}(z)$, then
              \begin{align*}
                &\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v]))\\
                &=\\
                &[[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}(\consistency_{X+Y}(\iota_{2,V};\iota_{V_{X} + V_{Y}}(g_{V}(z))))\\
                &=\\
                &[[]_{V}^{\consistency},[]_{E}^{\consistency}]^{*}([\iota_{2,V};\iota_{V_{X} + V_{Y}},\iota_{2,E};\iota_{E_{X}+E_{Y}}]^{*}(\consistency_{Y}(\iota_{V_{Y}}(g_{V}(z)))))
            \end{align*}
            and
            \begin{align*}
                &[u_{V};\iota_{V_{Q}},u_{E};\iota_{E_{Q}}]^{*}(\consistency_{\sfrac{X+Y}{\sim}}^{\hashtag}(\iota_{\sfrac{V_{X} + V_{Y}}{\sim}}([v])))\\
                &=\\
                &[\iota_{2,V};[]_{V};u_{V};\iota_{V_{Q}},\iota_{2,E};[]_{E};u_{E};\iota_{E_{Q}}]^{*}(\consistency_{Y}(\iota_{V_{Y}}(g_{V}(z))))\\
                &=\\
                &[j_{2,V};\iota_{V_{Q}},j_{2,E};\iota_{E_{Q}}]^{*}(\consistency_{Y}(\iota_{V_{Y}}(g_{V}(z))))\\
                &\text{(by homomorphism-ness of $j_2$)}\\
                &\subseteq\\
                &\consistency_{Q}(j_{2,V};\iota_{V_{Q}}(g_{V}(z)))\\
                &\consistency_{Q}(j_{1,V};\iota_{V_{Q}}(f_{V}(z)))\\
                &=\\
                &\consistency_{Q}(u_{V};\iota_{V_{Q}}([f_{V};\iota_{1,V};(z)]))\\
                &=\\
                &\consistency_{Q}(u_{V};\iota_{V_{Q}}([v]))
            \end{align*}
        \item The remaining case when $v = g_{V};\iota_{2,V}(z)$ is symmetric.
        \end{itemize}
...
\begin{figure}
    \[
        \scalebox{0.5}{
        \tikzfig{appendix/pushout_example}
        }
\]
\caption{Pushout in $\catname{EHyp_{\Sigma}}$}
