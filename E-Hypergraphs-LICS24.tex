...
\]
 and the following conditions hold:
    \begin{enumerate}
        ...
        \item all elements in the image of $i+j$ share the same set of parents;
        \item $m(v)  \consistency m(w)$ for all $v,w$ in the image of $i+j$.
...
\end{definition}
The conditions (3) and (4) on a match, in particular, prohibit finding a match for $\mathcal L$ in $\mathcal G$,  below.  
\[
